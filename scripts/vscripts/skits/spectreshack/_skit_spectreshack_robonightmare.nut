...
void function OnBoopTriggerEnter_Thread( entity trigger, entity ent, SkitInstance si )
{
	...
	while ( ent.Player_IsFreefalling() )
	{
		WaitFrame()
	}
	...
	boopDirection.z = 1.0
		...
		if ( drawDebugs )
		{
			...
			DebugDrawLineRGB( boopOrigin, boopOrigin + (boopDirection * 100), int(COLOR_WHITE.x), int(COLOR_WHITE.y), int(COLOR_WHITE.z), true, 30.0 )
		}
	#endif // DEV
