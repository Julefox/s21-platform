...
void function UpdateLootRuiWithData( entity player, var rui, LootData data, int lootContext, LootRef lootRef, bool isInMenu )
{
	...
	if ( data.lootType == eLootType.MAINWEAPON )
	{
		...
		{
			printt( "Warning: RuiSetString skinName failed on loot tooltip rui: " + e1 )
		}
		try
		{
		...
		{
			printt( "Warning: RuiSetInt skinTier failed on loot tooltip rui: " + e2 )
		}
		...
		if ( IsValidItemFlavorGUID( skinGUID ) )
		{
			...
			if ( ItemFlavor_HasQuality( weaponSkin ) )
			{
				...
				{
					printt( "Warning: RuiSetString skinName failed on loot tooltip rui: " + e3 )
				}
				try
				{
				...
				{
					printt( "Warning: RuiSetInt skinTier failed on loot tooltip rui: " + e4 )
				}
			}
		}
