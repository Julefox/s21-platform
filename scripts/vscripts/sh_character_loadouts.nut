...
void function CharacterLoadouts_GiveWeaponLoadoutToPlayer( entity player, WeaponLoadout weaponLoadout, string debugLoadoutName = "", bool doFirstDeploy = true )
{
	...
	for ( int i = 0; i < weaponLoadout.weaponRefs.len(); i++ )
	{
		...
		if ( IsValidItemFlavorGUID( weapon.e.skinItemFlavorGUID, eValidation.DONT_ASSERT ) )
		{
		...
		if ( IsValidItemFlavorGUID( weapon.e.charmItemFlavorGUID, eValidation.DONT_ASSERT ) )
		{
...
array< LootData > function Remove_Weapons( entity player, bool devOut = false )
{
	...
	// Slot wipe: TakeWeaponNow(ref) can miss kit-suffix / ghost primaries so
	// PRIMARY_1 stays full and the next GiveWeapon_NoDeploy throws "No room".
	// FR death skips ClearPlayerInventory, so restore depends on this being solid.
