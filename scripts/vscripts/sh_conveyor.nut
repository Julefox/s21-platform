...
void function DEBUG_Conveyor_DestroyDeployables ()
{
	...
	foreach ( string name in file.pathNames )
	{
		...
		{
			array<entity> children = crate.GetChildren()
...
{
	array<entity> children = script_mover.GetChildren()
	if ( children.len() > 0 )
	{
		foreach ( entity child in children )
		{
			...
			foreach ( string abilityName in DEPLOYABLE_ABILITY_NAMES)
			{
				...
				if ([BASE_WALL_SCRIPT_NAME, MOUNTED_TURRET_PLACEABLE_SCRIPT_NAME, DEATH_TOTEM_TARGETNAME].contains(abilityName))
					continue
				target = FindScriptNameInChildren( child, abilityName )
				if ( target != null )
				{
