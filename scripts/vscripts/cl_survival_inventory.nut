...
struct {
	...
	bool inventoryInited = false
} file


void function Cl_Survival_InventoryInit()
{
	// Safe to call from survival client init AND Flowstate custom modes
	// (fs_1v1 never runs ClGamemodeSurvival_Init which normally owns this).
	// Must run after Sh_InitToolTips (GameMode_RunClientInits), not SharedInit.
	if ( file.inventoryInited )
		return
...
void function Survival_UseInventoryItem( string ref, string secondRef )
{
	...
	{
		file.specialItemUseFunctions[ ref ]( GetLocalViewPlayer(), ref, secondRef )
	}
...
void function TEMP_UpdateTeammateRui( var elem, bool isCompact )
{
	...
	while ( true )
	{
		...
		if ( teammateIndex < team.len() )
		{
			...
			entity ent = ( teammateIndex < 0 )? player: team[ teammateIndex ]
			ItemFlavor character = FS_Hud_GetCharacterFlavor( ent )
			asset legendIcon      = CharacterClass_GetGalleryPortrait( character )
