		...
		for ( int capsuleIdx = 0; capsuleIdx < numCapsules; capsuleIdx++ )
		{
			for ( int breakableThingIndex = 0; breakableThingIndex < 5; breakableThingIndex++ )
			{
				...
				TraceResults collTrace = TraceHullEntsOnly(
					...
					DebugDrawCircleRGB( capsuleCurr, up, capsuleMins.x, int( COLOR_GREEN.x ), int( COLOR_GREEN.y ), int( COLOR_GREEN.z ), true, stepDuration + 0.08 )
					DebugDrawCircleRGB( capsuleCurr + <0, 0, capsuleMaxs.z>, up, capsuleMins.x, int( COLOR_GREEN.x ), int( COLOR_GREEN.y ), int( COLOR_GREEN.z ), true, stepDuration + 0.08 )
					DebugDrawCircleRGB( capsuleDest, up, capsuleMins.x, int( COLOR_MAGENTA.x ), int( COLOR_MAGENTA.y ), int( COLOR_MAGENTA.z ), true, stepDuration + 0.08 )
					DebugDrawCircleRGB( capsuleDest + <0, 0, capsuleMaxs.z>, up, capsuleMins.x, int( COLOR_MAGENTA.x ), int( COLOR_MAGENTA.y ), int( COLOR_MAGENTA.z ), true, stepDuration + 0.08 )
				#endif
...
{
	#if BLOCKABLE_DOOR_DEBUG
		DebugDrawCircleRGB( circleCenterIn, <0, 0, 0>, circleRadius, int( COLOR_WHITE.x ), int( COLOR_WHITE.y ), int( COLOR_WHITE.z ), true, 0.6 )
	#endif
	bool intersect = true
	...
	{
		#if BLOCKABLE_DOOR_DEBUG
			DebugDrawLineRGB( arcCornerIn, arcCornerIn + arcRadius * startAngPlaneAlongDir, 255, 120, 180, true, 0.6 )
		#endif
		intersect = false
	}
	else
	{
		#if BLOCKABLE_DOOR_DEBUG
			DebugDrawLineRGB( arcCornerIn, arcCornerIn + arcRadius * startAngPlaneAlongDir, 120, 255, 180, true, 0.6 )
		#endif
	}
	...
	{
		#if BLOCKABLE_DOOR_DEBUG
			DebugDrawLineRGB( arcCornerIn, arcCornerIn + arcRadius * endAngPlaneAlongDir, 255, 120, 30, true, 0.6 )
		#endif
		intersect = false
	}
	else
	{
		#if BLOCKABLE_DOOR_DEBUG
			DebugDrawLineRGB( arcCornerIn, arcCornerIn + arcRadius * endAngPlaneAlongDir, 120, 255, 30, true, 0.6 )
		#endif
	}
	...
	{
		#if BLOCKABLE_DOOR_DEBUG
			DebugDrawCircleRGB( arcCornerIn, <0, 0, 0>, arcRadius, 255, 40, 40, true, 0.6 )
		#endif
		intersect = false
	}
	else
	{
		#if BLOCKABLE_DOOR_DEBUG
			DebugDrawCircleRGB( arcCornerIn, <0, 0, 0>, arcRadius, 40, 255, 40, true, 0.6 )
		#endif
	}
...
{
	DoorData data = CreateDoorDataFromDoor( door, baseAngles, baseModel )
	entity rebuiltDoor = PasReinforce_OnDoorDestroyed( door )
	file.rebuiltDoorToData[ rebuiltDoor ] <- data
	...
	if ( IsCodeDoor( door ) )
	{
		...
		{
			data.hasLinkDoor = true
			data.linkDoorData = CreateDoorDataFromDoor( data.linkDoor, linkDoorBaseAngles, linkDoorBaseModel )
		}
	}

	if ( !( data.realm in file.recreateDoorDataByRealm) )
		file.recreateDoorDataByRealm[ data.realm ] <- []

	file.recreateDoorDataByRealm[ data.realm ].append( data )
}
...
void function BlockableDoor_OnDamage( entity door, var damageInfo )
{
	...
	else if ( IsBitFlagSet( DamageInfo_GetCustomDamageType( damageInfo ), DF_EXPLOSION ) )
	{
		...
		if ( IsBitFlagSet( DamageInfo_GetCustomDamageType( damageInfo ), DF_GIB ) )
		{
			damageInflicted = float( door.GetMaxHealth() )
		}
	}
	...
	#if BLOCKABLE_DOOR_DEBUG
		DebugDrawLineRGB( doorCenter, doorCenter + effectDir * 50.0, 200, 200, 50, true, 3.0 )
	#endif
...
bool function Survival_DoorSliding_CanUseFunction( entity playerUser, entity doorModel, int useFlags )
{
	...
	{
		DebugDrawLineRGB( playerPos, doorModelPos, 200, 200, 50, true, 1.0 )
