...
void function DEV_ShowVaults()
{
	foreach ( entity lootEnt in SURVIVAL_Loot_GetAllLoot() )
	{
		if ( IsValid( lootEnt ) && GetVaultPanelForLoot( lootEnt ) )
			DebugDrawLineRGB( lootEnt.GetOrigin(), lootEnt.GetOrigin() + <0, 0, 64>, 255, 0, 0, true, 60.0 )
	}
}
...
void function DEV_ShowVaultKeys()
{
	foreach( vaultKey in file.dev_VaultKeys )
	{
		if( IsValid( vaultKey ) )
		{
			DebugDrawSphereRGB( vaultKey.GetOrigin(), 64, 102, 0, 204, true, 60 )
		}
		else
		{
...
void function DEV_ShowVaultPanelInfo_Single( entity panel, bool showRmDimsThroughGeo = true, bool showVolsThroughGeo = false, float duration = 60 )
{
	...
	DebugDrawSphereRGB( loc, 64, 255, 255, 0, true, duration )
	...
	DebugDrawSphereRGB( dims.center, 64, 255, 255, 0, showRmDimsThroughGeo, duration )
	DebugDrawSphereRGB( dims.endFront, 128, 0, 255, 0, showRmDimsThroughGeo, duration )
	DebugDrawSphereRGB( dims.endBack, 128, 255, 0, 0, showRmDimsThroughGeo, duration )
	DebugDrawLineRGB( dims.center, dims.endFront, 0, 255, 0, showRmDimsThroughGeo, duration )
	DebugDrawLineRGB( dims.center, dims.endBack, 255, 0, 0, showRmDimsThroughGeo, duration )
	...
	foreach( specialVol in file.noPlaceVolumes_ByPanel[ panel ] )
	{
		...
		DebugDrawCylinderRGB( specialVol.GetOrigin() - < 0, 0, dims.heightBelow >, < -90, 0, 0 >, dims.radius, volHeightTotal, 255, 0, 0, showVolsThroughGeo, duration )
	}
}
#endif // SERVER && DEV
#endif
...
bool function PingVaultUnderAim( entity vault )
{
	...
	if( VaultPanels_PingFromMap_Enabled() )
		Remote_ServerCallFunction( FUNCNAME_PingVaultFromMap, vault ) //const string FUNCNAME_PingVaultFromMap = "VaultPanel_ClientToServer_PingVaultFromMap"
