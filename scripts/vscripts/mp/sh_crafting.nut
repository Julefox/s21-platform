...
void function Crafting_Init()
{
	...
	file.isEnabled = true
		...
		//AddCallback_OnQuickchatEvent( eCommsAction.REPLY_CRAFTING_NEXT_HARVESTER_OR_WORKBENCH, PingNextGoalForPlayer ) //S21 comms action
		//AddCallback_OnQuickchatEvent( eCommsAction.REPLY_CRAFTING_PING_ALL_WORKBENCHES, PingAllWorkbenches ) //S21 comms action
...
//	ent.Destroy
	...
	// The harvester is reused rather than recreated here, so its links and parent are
	// still intact. Only restore them if it ever becomes a new entity again.
	if ( harvester != ent )
	{
...
void function Crafting_ShowCraftingLocations( float lifetime = 5.0)
{
	...
	foreach( location in ReplicatorLocations )
	{
		...
		DebugDrawSphereRGB( origin, 128, 255, 255, 0, true, lifetime )
		DebugDrawLineRGB( origin, origin + <0,0,12000>, 255, 255, 0, true, lifetime )
	...
	{
		vector origin = location.GetOrigin()
		DebugDrawSphereRGB( origin, 64, 0, 255, 0, true, lifetime )
		DebugDrawLineRGB( origin, origin + <0,0,6000>, 0, 255, 0, true, lifetime )
...
void function Crafting_RingLogic_Thread(entity player)
{
	...
	while ( DeathField_PointDistanceFromFrontier( player.GetOrigin(), player.DeathFieldIndex() ) > 0 )
	{
		WaitFrame()
	}
	player.Signal( "CraftingPlayerDetachImmediate" )
...
bool function Crafting_Workbench_IsNotBusy( entity player, entity ent, int useFlags )
{
	...
	if( Crafting_CraftersDisabledInDeathField() )
	{
		bool inDeathField = DeathField_PointDistanceFromFrontier( ent.GetOrigin(), player.DeathFieldIndex() ) <= 0
		if ( inDeathField )
		{
			return false
		}
