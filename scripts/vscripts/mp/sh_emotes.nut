...
void function ShEmotes_Init()
{
	...
	// Dummy / podium ModelPerformEmote EndSignals these even when live player emotes are off.
	...
	AddCallback_OnPlayerRespawned( CharacterEmote_OnPlayerRespawned )
...
void function PlayerPerformEmote( entity player, ItemFlavor flavor )
{
		...
		function() : ( player, wasAlreadyInShoulderMode, forceStandHandle ) //, emote3pSound )
		{
			if ( IsValid( player ) )
			{
				...
				if ( GetPlayerIsEmoting( player ) )
				{
					...
					player.ContextAction_ClearEmoting()
	...
	player.ContextAction_SetEmoting()
	...
	if ( !wasAlreadyInShoulderMode )
	{
		...
		player.SetTrackEntityOffsetHeight( cameraHeightOffset + CAM_BASE_OFFSET_HEIGHT )
		player.SetTrackEntityOffsetRight( 12.0 )
		player.SetTrackEntityBlendInTimes( 0.2,0.0,0.0 )
		player.SetTrackEntityBlendOutTime( CAM_OUTRO_EASE_TIME )
...
void function CharacterEmote_OnPlayerRespawned( entity player )
{
	SetPlayerCanGroundEmote( player, true )
}
