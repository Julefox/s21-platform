...
{
	MapZones_RegisterDataTable( $"datatable/map_zones/zones_mp_rr_tropic_island_mu2.rpak" )
	//mp_rr_tropic_island_SurvivalPreprocess()
	...
	{
		#if SERVER
			AddCallback_EntitiesDidLoad( IMC_Armory_EntitiesDidLoad )
		#endif
	...
	SURVIVAL_SetFlightAngleAdjustment( -20.0 )
	Survival_SetMapFloorZ( -500 )
...
//	PrecacheParticleSystem( FX_LIGHTING_STRIKE_SKYBOX )
	...
	{
		//BAROMETER
		view.origin = < -10496.057617, -19960.699219, 288.261658 >
		view.angles = <  -13.739991, -132.692108, 0 >
		view.fov = 90

		//Pylon
		//view.origin = < 7277.240234, -24218.728516, 859.480164 >
		//view.angles = < -18.249617, 98.252014, 0 >
		//view.fov = 90
	}
	else //SURVIVAL INTRO CAMERAS
	{
		int select = RandomInt( 6 )
		switch( select )
		{
			case 0:
				view.origin = < 13040.264648, -15305.111328, 1395.929565 >
				view.angles = < -8.060658, -144.466553, 0.000000 >
				break
				//PYLON

			case 1:
				view.origin = <  15930.752930, -26232.435547, 1270.571289 >
				view.angles = < 6.349490, -82.016464, 0.000000 >
				view.fov = 90
				break
				//ECHO HQ

			case 2:
				view.origin = < 32634.236328, -30567.572266, 1096.903076 >
				view.angles = <  -4.650510, 124.783607, 0.000000 >
				view.fov = 90
				break
				//STORM WALL

			case 3:
				view.origin = < 7207.047363, -35576.238281, 2919.248535 >
				view.angles = <  28.789480, -178.456345, 0.000000 >
				view.fov = 90
				break
				//ECHO SALVAGE

			case 4:
				view.origin = < -11736.392578, -2008.676147, 3022.890625 >
				view.angles = <  24.609474, 104.323631, 0.000000 >
				view.fov = 90
				break
				//AMP STATION B

			case 5:
				view.origin = < 25878.992188, 38081.335938, 11935.330078 >
				view.angles = <  37.589657, 64.314598, 0.000000 >
				view.fov = 90
				break
				//AMP STATION A
		}
	...
	Flyers_SetFlyersToSpawn( flyerCount )
		...
		ForcedSpawn_AddSpawnPoint( "checkpoint_north", < -19148, 20583, 2864 > ) //1
		ForcedSpawn_AddSpawnPoint( "checkpoint_south", < -24525, 10687, 78 > ) //2
		ForcedSpawn_AddSpawnPoint( "trident", < -11690, 28966, 3890 > ) //3
		ForcedSpawn_AddSpawnPoint( "north_pad", < -26248, 37267, 4483 > ) //4
		ForcedSpawn_AddSpawnPoint( "downed_beast", < -34939, 21666, 3922 > ) //5
		ForcedSpawn_AddSpawnPoint( "the_mill", < -38548, 1850, 1400 > ) //6
		ForcedSpawn_AddSpawnPoint( "cenote_cave", < -30535, -19148, 273 > ) //7
		ForcedSpawn_AddSpawnPoint( "barometer_south", < -21045, -29351, 239 > ) //8
		ForcedSpawn_AddSpawnPoint( "barometer_north", < -11348, -15130, 239 > ) //9
		ForcedSpawn_AddSpawnPoint( "ceto_station", < -12158, 594, 3040 > ) //10
		ForcedSpawn_AddSpawnPoint( "cascade_falls", < 1602, 12170, 4190 > ) //11
		ForcedSpawn_AddSpawnPoint( "command_center", < 20673, 17470, 13342 > ) //12
		ForcedSpawn_AddSpawnPoint( "the_wall", < 4924, 36186, 6519 > ) //13
		ForcedSpawn_AddSpawnPoint( "zeus_station", < 29742, 43198, 11561 > ) //14
		ForcedSpawn_AddSpawnPoint( "lighting_rod", < 31732, 28031, 12044 > ) //15
		ForcedSpawn_AddSpawnPoint( "cliff_side", < 42165, 13334, 9609 > ) //16
		ForcedSpawn_AddSpawnPoint( "storm_catcher", < 27196, 12879, 8671 > ) //17
		ForcedSpawn_AddSpawnPoint( "prowler_nest", < 26835, -175, 3118 > ) //18
		ForcedSpawn_AddSpawnPoint( "launch_pad", < 26673, -12762, 526 > ) //19
		ForcedSpawn_AddSpawnPoint( "devastated_coast", < 32993, -23089, 1077 > ) //20
		ForcedSpawn_AddSpawnPoint( "echo_hq", < 16640, -33897, 2605 > ) //21
		ForcedSpawn_AddSpawnPoint( "coastal_camp", < 1427, -35774, 2474 > ) //22
		ForcedSpawn_AddSpawnPoint( "the_pylon", < 9993, -16610, 4006 > ) //23
		ForcedSpawn_AddSpawnPoint( "jurassic", < -747, -7619, 1761 > ) //24
		ForcedSpawn_AddSpawnPoint( "lift", < 10533, 3106, 2877 > ) //25
