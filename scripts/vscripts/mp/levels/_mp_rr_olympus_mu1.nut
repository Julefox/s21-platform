...
void function InitWarpNode( entity infotarget )
{
	...
	if( infotarget.GetLinkEntArray().len() == 2 )
	{
		...
		DebugDrawLineRGB( infotarget.GetOrigin(), infotarget.GetLinkEntArray()[0].GetOrigin(), 255, 0, 255, true, 999 )
		DebugDrawLineRGB( infotarget.GetOrigin(), infotarget.GetLinkEntArray()[1].GetOrigin(), 0, 255, 0, true, 999 )
	}
	else if( infotarget.GetLinkEntArray().len() == 1 )
	{
		...
		{
			DebugDrawLineRGB( infotarget.GetOrigin(), link.GetOrigin(), 0, 0, 255, true, 999 )
		}
	} else if( infotarget.GetLinkEntArray().len() == 0 )
	{
		...
		foreach( link in infotarget.GetLinkEntArray() )
		{
			DebugDrawLineRGB( infotarget.GetOrigin(), link.GetOrigin(), 0, 255, 0, true, 999 )
		}
	}
	#endif
}
...
void function DEV_StartNodesLinksShow()
{
	foreach( node in file.nodes )
	{
		...
		DebugDrawLineRGB( node.GetOrigin(), node.GetLinkEntArray()[0].GetOrigin(), 255, 0, 255, true, 3.0 )
		DebugDrawLineRGB( node.GetOrigin(), node.GetLinkEntArray()[1].GetOrigin(), 0, 255, 0, true, 3.0 )
