...
// InitOctaneTownTakeover / PlaceOctaneTownTakeoverLoot now defined in
// mp_rr_canyonlands_common.nut (so hu can reach them too)
...
// InitOctaneTownTakeover / PlayerTookBurnTriggerDamage now defined in
// mp_rr_canyonlands_common.nut (identical bodies) so hu can reach them too.
...
void function InitWraithAudioLog()
{
	...
	AddCallback_OnUseEntity_ClientServer( audioLogTarget, WraithTTAudioLog_OnUse )
}
...
void function InitIndividualLeviathan( entity leviathan )
{
	...
	// TODO: implement -- DisableSkydiveEndOnEntity undefined in this engine build (every other
	// call site in the codebase is also disabled; SetLeviathanMover in mu3_common.nut's own
	// inline "fix" is unproven -- that function isn't defined anywhere either)
	//DisableSkydiveEndOnEntity( leviathan, true )
...
bool function Leviathan_CanLookAtEnt( entity leviathan, vector pos, float careChance, float maxDistCareChance, bool DEBUG )
{
	...
	{
		if ( DEBUG )
			DebugDrawLineRGB( center, pos, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), true, 5 )
		return false
	}
	...
	{
		if ( DEBUG )
			DebugDrawLineRGB( center, pos, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 5 )
		return false
	}
	...
	if ( dot < 0 )
	{
		...
		{
			if ( DEBUG )
				DebugDrawLineRGB( center, pos, 255, 128, 0, true, 5 )
			return false
		}
	}

	if ( DEBUG )
		DebugDrawLineRGB( center, pos, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 5 )
