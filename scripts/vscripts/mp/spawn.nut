...
global function Spawn_SetSpawnPointOverride
...
global function RateSpawnpoints_FFA
...
struct
{
	...
	entity functionref( entity ) spawnPointOverride
	bool loggedMissingSp
} file
...
void function Spawn_SetSpawnPointOverride( entity functionref( entity ) func )
{
	file.spawnPointOverride = func
}

// .sp is native on spawnpoint classes only. CBaseEntity throws class-mismatch.
bool function Spawnpoint_HasSp( entity spawnpoint )
{
	...
	try
	{
		var unused = spawnpoint.sp.enabled
		return true
	}
	catch ( err )
	{
		if ( !file.loggedMissingSp )
		{
			file.loggedMissingSp = true
			printt( "[SPAWN] no spawnpoint script data on " + spawnpoint.GetClassName() + "; skipping .sp" )
		}
		return false
	}
}

void function Spawn_MarkUsed( entity player, entity spawnpoint )
{
	if ( !Spawnpoint_HasSp( spawnpoint ) )
		return
...
entity function Spawn_TryOverrideSpawnPoint( entity player )
{
	if ( file.spawnPointOverride == null )
		return null
	entity spawnpoint = file.spawnPointOverride( player )
	if ( !IsValid( spawnpoint ) )
		return null
	Spawn_MarkUsed( player, spawnpoint )
	return spawnpoint
}
...
void function DebugSpawnsVisibleToTurret()
{
	for ( ;; )
	{
		foreach ( spawnpoint in svSpawnGlobals.allNormalSpawnpoints )
		{
			...
			{
				DebugDrawLineRGB( spawnpoint.GetOrigin(), turret.GetOrigin(), 255, 255, 255, true, 0.2 )
			}
		}
		WaitFrame()
	}
...
entity function FindSpawnPoint( entity player )
{
	...
	entity override = Spawn_TryOverrideSpawnPoint( player )
	if ( IsValid( override ) )
		return override
...
// S3 has no SpawnPoints_GetPilotAllianceStart native (S21 does).
// Map multi-squad alliances onto classic 2-side start tags (IMC / Militia).
array< entity > function GetPilotAllianceStartSpawns_Script( int alliance )
{
	array< entity > spawnpoints

	array< int > teamsInAlliance = AllianceProximity_GetTeamsInAlliance( alliance )
	foreach ( t in teamsInAlliance )
	{
		array< entity > teamStarts = SpawnPoints_GetPilotStart( t )
		foreach ( sp in teamStarts )
		{
	...
	// Most FreeDM / Control maps only tag start sides as TEAM_IMC / TEAM_MILITIA.
	if ( spawnpoints.len() == 0 )
	{
		int sideTeam = ( alliance == ALLIANCE_A ) ? TEAM_IMC : TEAM_MILITIA
		spawnpoints = SpawnPoints_GetPilotStart( sideTeam )
	}
...
entity function FindStartSpawnPoint( entity player )
{
	...
	if ( AllianceProximity_IsUsingAlliances() )
	{
		...
		spawnpoints = GetPilotAllianceStartSpawns_Script( alliance )
	}
...
void function RateSpawnpoints_FFA( int checkClass, array<entity> spawnPoints, int teamId, entity player )
{
	array<entity> others
	foreach ( entity other in GetPlayerArray_Alive() )
	{
		if ( !IsValid( other ) || other == player )
			continue
		if ( other.GetTeam() == TEAM_SPECTATOR )
			continue
		others.append( other )
	}
	...
	vector actionOrigin = GetMedianOriginOfEntities( others )
	...
	foreach ( entity spawnPoint in spawnPoints )
	{
		...
		{
			float dist = Distance2D( spawnOrigin, other.GetOrigin() )
			if ( dist < nearest )
				nearest = dist
		}

		float toAction = Distance2D( spawnOrigin, actionOrigin )
		vector toActionVec = actionOrigin - spawnOrigin
		toActionVec.z = 0.0
		float facing = 0.0
		if ( LengthSqr( toActionVec ) > 1.0 )
			facing = DotProduct( spawnPoint.GetForwardVector(), Normalize( toActionVec ) )

		// Maximin vs living players, then stay near the pack.
		float rating = nearest - ( toAction * 0.125 ) + ( facing * 32.0 )
		spawnPoint.SetRating( rating )
	}
}
...
bool function IsSpawnPointForTeam( entity spawnpoint, int team )
{
	...
	// FreeDM multi-squad: start sides are usually only TEAM_IMC / TEAM_MILITIA.
	// Treat those as ALLIANCE_A / ALLIANCE_B so allied squads share a side.
	if ( AllianceProximity_IsUsingAlliances() )
	{
		int playerAlliance = AllianceProximity_GetAllianceFromTeam( team )
		if ( spawnpointTeam == TEAM_IMC || spawnpointTeam == TEAM_MILITIA )
		{
			int sideAlliance = ( spawnpointTeam == TEAM_IMC ) ? ALLIANCE_A : ALLIANCE_B
			return sideAlliance == playerAlliance
		}
		return AllianceProximity_GetAllianceFromTeam( spawnpointTeam ) == playerAlliance
	}
...
void function DebugSpawnZones()
{
	for ( ;; )
	{
		...
		{
			DebugDrawLineRGB( zoneA.origin, spawnpoint.GetOrigin(), 0, 0, 255, true, 0.5 )
		}
		foreach ( spawnpoint in zoneB.spawnPoints )
		{
			DebugDrawLineRGB( zoneB.origin, spawnpoint.GetOrigin(), 255, 0, 0, true, 0.5 )
		}

		foreach ( player in GetPlayerArray() )
		{
			if ( player.GetTeam() == TEAM_MILITIA )
				DebugDrawLineRGB( player.GetOrigin(), player.GetOrigin() + <0,0,82>, 0, 0, 255, true, 0.1 )
			else if ( player.GetTeam() == TEAM_IMC )
				DebugDrawLineRGB( player.GetOrigin(), player.GetOrigin() + <0,0,82>, 255, 0, 0, true, 0.1 )
			else
				DebugDrawLineRGB( player.GetOrigin(), player.GetOrigin() + <0,0,82>, 128, 128, 128, true, 0.1 )
		}
...
void function DrawSpawnPointRating( entity spawnPoint, table ratingData )
{
	...
	while ( true )
	{
		...
		DebugDrawBoxRGB( spawnPoint.GetOrigin(), <-14, -14, enemyRating * -ratingScale>, <14,14,0>, 255, 128, 0, 1, 1.1 )

		var friendlyRating = ratingData.tfr + ratingData.pfr + ratingData.nfr
		DebugDrawBoxRGB( spawnPoint.GetOrigin(), <-14,-14,0>, <14, 14, friendlyRating * ratingScale>, 0, 128, 255, 1, 1.1 )
