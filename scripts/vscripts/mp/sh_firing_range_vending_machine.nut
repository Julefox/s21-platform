...
void function VendingMachine_LevelInit()
{
		...
		// Both callbacks take the machine the client is acting on, so the
		// signature has to declare it -- and typing it means a client can only
		// ever name a real vending machine here.
...
void function VendingMachineDeployThread( vector origin, vector angles, entity spawnTarget, int realm = -1 )
{
	...
	entity vendingMachine
	{
		...
		vendingMachine.SetImpactEffectColorID( COLORID_FX_LOOT_TIER1 )
		vendingMachine.SetIgnorePredictedTriggerTypes( TT_JUMP_PAD | TT_GRAVITY_LIFT | TT_BLACKHOLE )
		...
		if ( IsValid( spawnTarget ) )
		{
			...
			{
				entity mover = CreateScriptMover( VENDING_MACHINE_MOVER_SCRIPTNAME, origin, angles )
...
void function ClientCallback_OpenVendingMachine( entity player, entity grabber )
{
	...
	{
		FR_Nessie_OnUseVendingMachine( player )
	}
