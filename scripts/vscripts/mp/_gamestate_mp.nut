...
{
	// FlagInit lives in GameState_Init; belt-and-suspenders if init order skipped it.
	if ( !FlagExists( "GamePlaying" ) )
...
void function GameState_Init_MP()
{
	...
	{
		ServerCommand( "mp_enablematchending 0" )
		printt( "GameState: match_ending_enabled 0 -> mp_enablematchending 0" )
	}
}
...
bool function DoneWaitingForPlayers()
{
	...
	bool forceStartPastTimeout = GetCurrentPlaylistVarBool( "waiting_for_players_timeout_force", true )
	bool pastTimeout = ( Time() >= file.waitForPlayersEndTime )
	int pctDesired = GetCurrentPlaylistVarInt( "waiting_for_players_percentage_desired", 70 )
	int playersDesired = maxint( minPlayers, int( expectedPlayers * pctDesired * 0.01 ) )
	...
	else if ( Time() < file.waitForPlayersStartTime )
	{
	...
	else if ( ( !pastTimeout || !forceStartPastTimeout ) && connectedPlayersCount < playersDesired )
	{
		SvDemo_ConsistencyCheckString( "DoneWaitingForPlayers() F" )
		return false
	}
	else if ( countdownSeconds <= 0.0 )
	{
