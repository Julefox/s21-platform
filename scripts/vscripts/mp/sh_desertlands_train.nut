	...
	for ( int idx = 0; idx < file.true_trainCarCount; idx++ )
	{
		...
		foreach ( entity linkEnt in data.mover.GetLinkEntArray() )
		{
			...
			if ( scriptName == "train_particle_main_spotlight" )
			{
				...
				data.spotlightFX = StartParticleEffectOnEntityWithPos_ReturnEntity( data.brush, GetParticleSystemIndex( FX_TRAIN_MAIN_SPOTLIGHT ), FX_PATTACH_ABSORIGIN_FOLLOW_NOROTATE, ATTACHMENTID_INVALID, data.offsetSpotlight, <0, 0, 0> )
...
void function TrainInitiateEmergencyStop()
{
	...
	thread ResumeTrainAfterEmergencyStop()
		...
		{
			DebugDrawSphereRGB( file.train.GetOrigin(), 64.0, 128, 128, 255, false, 60.0 )
		}
	#endif
}
...
void function TrainSpotlightBlink()
{
	...
	data.spotlightFX.Destroy()
	data.spotlightFX = StartParticleEffectOnEntityWithPos_ReturnEntity( data.brush, GetParticleSystemIndex( FX_TRAIN_SPOTLIGHT_BLINK ), FX_PATTACH_ABSORIGIN_FOLLOW_NOROTATE, ATTACHMENTID_INVALID, data.offsetSpotlight, <0, 0, 0> )
		...
		while( true )
		{
			...
			else
			{
				...
				currentNode.RegenerateSmoothPoints()
					...
					if ( file.devShowTrainPath )
					{
						DebugDrawSphereRGB( currentNode.GetOrigin(), 128.0, 255, 255, 0, false, 180.0 )
						DebugDrawSphereRGB( nextNode.GetOrigin(), 128.0, 255, 0, 255, false, 180.0 )
					}
				#endif
			}
...
void function DrawTrainPath( array<entity> nodes )
{
	...
	{
		//if ( idx + 1 < nodes.len() )
			DebugDrawLineRGB( nodes[idx].GetOrigin(), nodes[ idx + 1 ].GetOrigin(), 0, 0, 255, true, 5 )
	}
}
#endif // SERVER
