...
array< entity > function RandomizeNodeLocations( array< entity > nodes, float exclusionDistance, int goal, bool destroyUnusedEntities, string propType, vector optionalDebugColor = <0,0,0>)
{
	...
	// TODO(s3): PropTool_GetRandomPropCombination missing on S3
		...
		if ( filterType == ePropPlacementType.NO_TYPE_SPECIFIED || spawnParams.propType == filterType )
		{
			...
			if( !usingOldLogic )
			{
				for( int i = 0; i < spawnParams.nodes.len(); i++ )
				{
					...
					{
						DebugDrawSphereRGB( spawnParams.nodes[i].GetOrigin(), 1024, int(debugColor.x), int(debugColor.y), int(debugColor.z), true, 100 )
						DebugDrawSphereRGB( linkedEnt.GetOrigin(), 1024, int(debugColor.x), int(debugColor.y), int(debugColor.z), true, 100 )
						DebugDrawLineRGB( spawnParams.nodes[i].GetOrigin(), linkedEnt.GetOrigin(), int(debugColor.x), int(debugColor.y), int(debugColor.z), true, 100 )
			...
			else
			{
				for ( int i = 0; i < spawnParams.nodes.len() - 1; i++ )
				{
					for ( int j = i + 1; j < spawnParams.nodes.len(); j++ )
					{
						...
						{
							DebugDrawSphereRGB( spawnParams.nodes[i].GetOrigin(), 1024, int(debugColor.x), int(debugColor.y), int(debugColor.z), true, 100 )
							DebugDrawSphereRGB( spawnParams.nodes[j].GetOrigin(), 1024, int(debugColor.x), int(debugColor.y), int(debugColor.z), true, 100 )
							DebugDrawLineRGB( spawnParams.nodes[i].GetOrigin(), spawnParams.nodes[j].GetOrigin(), int(debugColor.x), int(debugColor.y), int(debugColor.z), true, 100 )
...
void function MapAnalysis_ShowPropNodes( string filterType = ePropPlacementType.NO_TYPE_SPECIFIED )
{
	...
	foreach( SpawnParameters spawnParams in file.cachedSpawnParameters )
	{
		if( filterType == ePropPlacementType.NO_TYPE_SPECIFIED || spawnParams.propType == filterType )
		{
			...
			{
				DebugDrawSphereRGB( node.GetOrigin(), 1024, int(debugColor.x), int(debugColor.y), int(debugColor.z), true, 100 )
...
{
	// TODO(s3): PropTool_GetIndexForCurrentPropCombination / PropTool_GetPropCombinationByIndex missing on S3
	...
	if ( IsValid( currentCombo ) && currentCombo.len() > 0 )
	{
		...
		{
			DebugDrawSphereRGB( vec, 1024, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 100 )
		}
	}
}
...
void function PropTool_ValidateSolution( array< entity > nodes, string propType, bool verbose = false )
{
	...
	// TODO(s3): PropTool_GetPropCombinationCount / PropTool_GetPropCombinationSize / PropTool_GetPropCombinationByIndex missing on S3
