#if SERVER
global function CryptoUndergroundScreens_Init

global function OnDroneAttemptHackBunkerScreen
#endif
...
#if SERVER
const string SCREEN_DISCOVER_TRIGGER_SCRIPTNAME = "screen_room_discover_trigger"
#endif
...
struct BunkerScreenData
{
	...
	#if SERVER
		array<entity> playersThatHaveHacked
	#else
...
struct
{
	...
	#if SERVER
		array<entity> cryptoPlayersThatHaveDiscoveredRoom
	#endif
...
#if SERVER
void function CryptoUndergroundScreens_Init()
{
	RegisterCSVDialogue( SCREEN_CSV_DIALOGUE )
	file.customQueueIdx = RequestCustomDialogueQueueIndex()

	AddCallback_EntitiesDidLoad( EntitiesDidLoad )
	AddCallback_OnClientDisconnected( BunkerLoreScreenRemoveHackedPlayer )
...
{
	Remote_CallFunction_NonReplay( player, "SCB_SetCryptoLoreScreenDialogueIdx", file.customQueueIdx )
}
#endif
...
bool function IsBunkerLoreScreenHacked( entity ent, entity player )
{
	...
	#if SERVER
		if ( data.playersThatHaveHacked.contains( player ) )
			return true
	#else
...
#if SERVER
void function BunkerLoreScreenRemoveHackedPlayer( entity player )
{
	foreach ( BunkerScreenData data in file.allScreenDatas )
	{
		if ( data.playersThatHaveHacked.contains( player ) )
			data.playersThatHaveHacked.fastremovebyvalue( player )
	}
}
#endif
...
void function SetupBunkerLoreScreens()
{
	...
	#if SERVER
		foreach ( entity trigger in GetEntArrayByScriptName( SCREEN_DISCOVER_TRIGGER_SCRIPTNAME ) )
			trigger.SetEnterCallback( OnEnterDiscoverTrigger )
	#endif
}
#endif // SERVER || CLIENT
...
#if SERVER
void function OnEnterDiscoverTrigger( entity trigger, entity player )
{
	if ( !player.IsPlayer() )
		return

	// if you aren't crypto, or you are crypto and have entered before, then cancel
	if ( !IsPlayerCrypto( player ) || file.cryptoPlayersThatHaveDiscoveredRoom.contains( player ) )
		return
	...
	PlayBattleChatterLineToSpeakerAndTeam( player, "bc_CynMU2Lore_Discover" )
	file.cryptoPlayersThatHaveDiscoveredRoom.append( player )
}
#endif


#if SERVER
bool function IsPlayerCrypto( entity player )
{
	ItemFlavor character = LoadoutSlot_GetItemFlavor( ToEHI( player ), Loadout_Character() )
	string characterRef  = ItemFlavor_GetCharacterRef( character ).tolower()

	if ( characterRef != "character_crypto" )
		return false
...
#if SERVER
void function OnDroneAttemptHackBunkerScreen( entity screenModel, entity player, entity drone )
{
	DroneHackExtendedUse_ServerClient( screenModel, player, drone )
	Remote_CallFunction_NonReplay( player, "ServerCallback_OnBunkerLoreScreenAttemptHack", screenModel, drone )
}
#endif
...
void function DroneHackExtendedUse_ServerClient( entity screenModel, entity player, entity drone )
{
	...
	#if SERVER
		settings.exclusiveUse = false // doesn't work with drone
...
{
	EndSignal( drone, "OnDestroy" ) // drone destroyed, or fully recalled

	#if SERVER
		EndSignal( player, "OnDeath" )
		EndSignal( player, "ExitCameraView" ) //leave drone view (tap to leave or Crypto Damaged) Server side
	#else
		entity localPlayer = GetLocalClientPlayer()
		EndSignal( localPlayer, "OnDeath" )
		EndSignal( localPlayer, "CameraViewEnd" ) //leave drone view (tap to leave or Crypto Damaged) Client side
	#endif

	// get screen message info
	...
	OnThreadEnd(
		function() : ( data, player )
		{
			...
			{
				#if SERVER
					if ( HasPlayerHackedScreen( player, data ) )
						data.playersThatHaveHacked.fastremovebyvalue( player )
				#else
				...
				#if SERVER
					StopAllQueuedAndCurrentDialogueForPlayer( player, eDialogueFlags.USE_CUSTOM_QUEUE, file.customQueueIdx )
				#else
	...
	#if SERVER
		if ( !HasPlayerHackedScreen( player, data ) )
			data.playersThatHaveHacked.append( player )
	#else
	...
	#if SERVER
		if ( messageIdx == 3 )
			PlayBattleChatterLineToPlayerThatBlocksLowerPriorityLines( "bc_CynMU2Lore_SignOff", player, player )
	#else
		foreach ( BunkerScreenData bunkerData in file.allScreenDatas )
		{
			foreach ( entity model in bunkerData.models )
