...
{
	Remote_RegisterServerFunction( "ClientCallback_SetSelectedHealthPickupType", "int", WHEEL_HEAL_AUTO, eHealthPickupType.COMBO_FULL )
...
array<CommsMenuOptionData> function BuildMenuOptions( int chatPage )
{
	...
	switch ( chatPage )
	{
		...
		case eChatPage.PING_MAIN_2:
		{
			...
			int quipsInWheel
			array<ItemFlavor> wheelItems = GetRandomEmoteWheelItems( character )
			int emoteCount = wheelItems.len()
			if ( emoteCount > MAX_QUIPS_EQUIPPED )
				emoteCount = MAX_QUIPS_EQUIPPED

			for ( int i = 0; i < emoteCount; i++ )
			{
				ItemFlavor quip = wheelItems[i]
...
void function PerformQuipAtSlot( int index )
{
	...
	ItemFlavor quip = allEmotes[index]
	if ( !CharacterQuip_IsTheEmpty( quip ) )
	{
...
void function HandleQuipPick( ItemFlavor quip, int choice )
{
	...
	Remote_ServerCallFunction( "ClientCallback_BroadcastQuip", ItemFlavor_GetGUID( quip ) )
...
void function ClientCallback_SetSelectedHealthPickupType( entity player, int healthPickupType )
{
	switch( healthPickupType )
	{
		...
		default:
			Warning( "SetSelectedHealthPickupType: unsupported type " + healthPickupType )
			return
	}
