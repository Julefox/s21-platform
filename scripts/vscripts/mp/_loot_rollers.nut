...
void function SetLootRollerLootFX( LootRollerData rollerData )
{
	...
	// The roller entity has no lootTier/hasVaultKey keyfields here, so the kv
	// writes never reached the networked values the client reads.
	lootRollerModel.SetTier( rollerData.lootTier )
	lootRollerModel.SetHasVaultKey( rollerData.hasVaultKey )
}
