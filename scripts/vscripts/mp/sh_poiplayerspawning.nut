...
void function DEVSphere( bool debugParm, vector center, float radius, vector color, bool bShowThruGeo, float showTime, int segments = 4 )
{
	...
	if ( doDebug )
	{
		// Dual-arity: CLIENT native wants color vector; use Color helper not bare RGB.
		DebugDrawSphereColor( center, radius, color, bShowThruGeo, showTime, segments )
	}
}
...
void function DEVCube( bool debugParm, vector center, float size, vector color, bool bShowThruGeo, float showTime )
{
	...
	if ( doDebug )
	{
		// Dual-arity: SERVER rgb ints / CLIENT color vec.
		DebugDrawCubeColor( center, size, color, bShowThruGeo, showTime )
	}
}
...
void function DEVLine( bool debugParm, vector start, vector end, vector color, bool showThruGeo, float showTime )
{
	...
	if ( doDebug )
	{
		DebugDrawLineRGB( start, end, int(color.x), int(color.y), int(color.z), showThruGeo, showTime )
	}
}
...
void function DEVCylinder( bool debugParm, vector center, vector angles, float radius, float height, vector color, bool bShowThruGeo, float showTime )
{
	...
	if ( doDebug )
	{
		DebugDrawCylinderRGB( center, angles, radius, height, int( color.x ), int( color.y ), int( color.z ), bShowThruGeo, showTime )
	}
}
...
void function Assign_Spawnpoints_FromLootLocs_ToZone( int zoneID, vector center, float radius, int numPoints, bool debug = false )
{
	...
	file.spawnPoints_ByZoneID[ zoneID ].extend( usablePoints )
		...
		if ( debug )
		{
			...
			{
				DebugDrawLineRGB( pt.origin, center, int(COLOR_ORANGE.x), int(COLOR_ORANGE.y), int(COLOR_ORANGE.z), true, POISPAWNING_DEBUGDRAWTIME )
...
void function OnCuratedSpawnpoint_Spawned( entity ent )
{
	...
	foreach( savedSpawn in file.curatedSpawn_Ents )
	{
		...
		if ( Distance2DSqr( savedSpawnLoc, entLoc ) <= tooCloseDistanceSqr )
		{
			...
			Warning( warningStr )
			#if DEVELOPER
				DebugDrawSphereRGB( savedSpawnLoc, 64, int(COLOR_DARK_RED.x), int(COLOR_DARK_RED.y), int(COLOR_DARK_RED.z), true, 120 )
				DebugDrawSphereRGB( entLoc, 64, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 120 )
			#endif
		}
	}
...
void function Register_CuratedSpawns( bool debug = false )
{
	...
	foreach( placedSpawn in file.curatedSpawn_Ents )
	{
		...
		else
		{
				...
				DebugDrawSphereRGB( placedSpawnLoc, 128, int(COLOR_PINK.x), int(COLOR_PINK.y), int(COLOR_PINK.z), true, 120 )
			#endif // DEV
		}
	}
	#if DEVELOPER
...
void function DelayedLockForwardInput( entity player, float delay = 0.25 )
{
	...
	while( !player.Player_IsFreefalling() )
	{
		WaitFrame()
	}
...
void function CustomDropship_PutPlayerInDropship( entity player, entity ship, int positionIndex, Point destination, bool useCameraDown1PAnims = false, bool debug = false )
{
	...
	int seatCount = file.dropshipAnimDataList.len()
	if ( seatCount < 1 )
		return
	if ( positionIndex < 0 || positionIndex >= seatCount )
	{
		Warning( FUNC_NAME() + "(): dropship seat " + positionIndex + " exceeds " + seatCount + " jump seats" )
		positionIndex = positionIndex % seatCount
		if ( positionIndex < 0 )
			positionIndex += seatCount
	}
	...
	if ( PLV_CustomDropship_Skip1P() )
	{
		...
		switch ( positionIndex )
		{
			...
			if ( debug )
			{
				...
				DebugDrawLineRGB( posLoc0, posLoc0 + AnglesToForward( destination.angles ) * spacingXY * 2, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, debugDrawDuration )
				DebugDrawLineRGB( posLoc0, posLoc0 + vRight * spacingXY * 2, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, debugDrawDuration )
				DebugDrawSphereRGB( posLoc0, 96, int(COLOR_WHITE.x), int(COLOR_WHITE.y), int(COLOR_WHITE.z), true, debugDrawDuration )
				DebugDrawSphereRGB( posLoc, 64, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), true, debugDrawDuration )
...
void function CustomDropship_View_Thread( entity player, entity ship, Point destination, float shot1Duration, float shot2Duration, bool debugShot1 = true, bool debugShot2 = false )
{
	...
	shotDuration = shot1Duration
		...
		if ( debugShot1 )
		{
			...
			DebugDrawLineRGB( viewOrigin, shipLoc_Adjusted, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, debugDuration )
			DebugDrawSphereRGB( shipLoc_Adjusted, 128, int(COLOR_PURPLE.x), int(COLOR_PURPLE.y), int(COLOR_PURPLE.z), true, debugDuration )
			DebugDrawSphereRGB( ship.GetOrigin(), 128, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, debugDuration )
			DebugDrawSphereRGB( viewOrigin, 128, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, debugDuration )
		}
	#endif
