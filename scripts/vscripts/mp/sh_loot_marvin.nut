...
void function SeeMarvinSpawnLocations()
{
	...
	foreach ( entity marvin, MarvinData data in file.spawnedMarvinData )
	{
		if ( IsAlive( marvin ) )
		{
			...
			{
				DebugDrawSphereRGB( marvin.GetOrigin(), 256, 0, 0, 255, true, 45.0 )
			}
			else if ( data.isStoryMarvin )
			{
				DebugDrawSphereRGB( marvin.GetOrigin(), 256, 0, 255, 0, true, 45.0 )
				storyMarvinExists = true
			}
			else
			{
				DebugDrawSphereRGB( marvin.GetOrigin(), 256, 255, 255, 0, true, 45.0 )
			}
		}
	}
