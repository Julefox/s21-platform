...
global function SetNestedGladiatorCardRawTracker
...
global function TriggerUpdateOfNestedGladiatorCardsForPlayer
#endif
...
global struct NestedGladiatorCardHandle
{
	...
	string[GLADIATOR_CARDS_NUM_TRACKERS]            overrideTrackerLabelList
	string[GLADIATOR_CARDS_NUM_TRACKERS]            overrideTrackerSuffixList
	bool                                            rawTrackersAuthored = false
...
// Writes a tracker slot directly, with no tracker ItemFlavor and no loadout slot behind it.
// An empty label hands the slot back to the flavor-driven path.
void function SetNestedGladiatorCardRawTracker( NestedGladiatorCardHandle handle, int trackerIndex, string label, int value, string valueSuffix = "" )
{
	handle.overrideTrackerLabelList[trackerIndex] = label
	handle.overrideTrackerSuffixList[trackerIndex] = valueSuffix
	handle.overrideTrackerDataIntegerList[trackerIndex] = value
	handle.rawTrackersAuthored = ( label != "" )
	TriggerNestedGladiatorCardUpdate( handle )
}
...
void function MenuGladCardThread( bool isForLocalPlayer, string playerUID )
{
	...
	ChangeNestedGladiatorCardOwner( fileLevel.currentMenuGladCardHandle[ playerUID ], ToEHI( player ), Time() )
	...
	if ( handle.cardRui != null )
	{
		...
		if ( characterOrNull != null )
		{
			...
			if ( handle.isFrontFace )
			{
				...
				if ( skinOrNull == null )
					skinOrNull = CharacterClass_GetDefaultSkin( character )
				...
				if ( handle.presentation != eGladCardPresentation.FRONT_FRAME_ONLY )
				{
					...
					{
						ItemFlavor loadedStance = LoadoutSlot_GetItemFlavor( handle.currentOwnerEHI, stanceSlot )
						if ( GetGlobalSettingsAsset( ItemFlavor_GetAsset( loadedStance ), "parentItemFlavor" ) != ""
							&& GladiatorCardStance_GetCharacterFlavor( loadedStance ) == character )
							stanceOrNull = loadedStance
					}
					if ( stanceOrNull == null )
						stanceOrNull = stanceSlot.defaultItemFlavor
				}
...
void function DoGladiatorCardCharacterCapture( CharacterCaptureState ccs )
{
	...
	// StreamModelsForAnim is S21 CLIENT (in r5apex); dropping it leaves an infinite WaitFrame.
	// Cap wait so a missing seq cannot kill the whole gladcard presentation.
	float streamDeadline = Time() + 10.0
	while ( streamSequence != "" && !ccs.model.StreamModelsForAnim( streamSequence ) )
	{
		if ( Time() >= streamDeadline )
		{
			printt( "[GLADCARD] StreamModelsForAnim timeout seq='" + streamSequence + "' key=" + ccs.key )
			break
		}
		WaitFrame()
	}
	...
	else
	{
		...
		if ( EHIHasValidScriptStruct( ccs.playerEHI ) && LoadoutSlot_IsReady( ccs.playerEHI, Loadout_Character() ) )
		{
...
#if CLIENT
// 1v1 has no GRX tracker flavors; stamp career kills/deaths/k/d from FS_1v1_Career* netvars.
// This reads the OWNER, so a card that shows someone else through overrides has to
// keep the trackers its caller stamped -- otherwise it displays the viewer's career.
void function FS_1v1_FillGladCardCareerTrackers( NestedGladiatorCardHandle handle )
{
	if ( !Flowstate_IsGame1v1Type() )
		return
	if ( handle.rawTrackersAuthored )
		return
	if ( handle.currentOwnerEHI == EHI_null )
		return

	entity owner = FromEHI( handle.currentOwnerEHI )
	if ( !IsValid( owner ) )
		return

	int kills = owner.GetPlayerNetInt( "FS_1v1_CareerKills" )
	if ( kills < 0 )
		return
	int deaths = owner.GetPlayerNetInt( "FS_1v1_CareerDeaths" )
	if ( deaths < 0 )
		deaths = 0

	handle.overrideTrackerLabelList[0] = "LIFETIME KILLS"
	handle.overrideTrackerSuffixList[0] = ""
	handle.overrideTrackerDataIntegerList[0] = kills

	handle.overrideTrackerLabelList[1] = "LIFETIME DEATHS"
	handle.overrideTrackerSuffixList[1] = ""
	handle.overrideTrackerDataIntegerList[1] = deaths
	...
	handle.overrideTrackerLabelList[2] = "LIFETIME K/D"
	handle.overrideTrackerDataIntegerList[2] = kdWhole
	handle.overrideTrackerSuffixList[2] = format( ".%02d", kdFrac )
}
#endif
...
{
	FS_1v1_FillGladCardCareerTrackers( handle )

	for ( int index = 0; index < GLADIATOR_CARDS_NUM_TRACKERS; index++ )
	{
		if ( handle.overrideTrackerLabelList[index] != "" )
		{
			RuiSetString( handle.cardRui, "statTracker" + index + "Label", handle.overrideTrackerLabelList[index] )
			RuiSetInt( handle.cardRui, "statTracker" + index + "Value", handle.overrideTrackerDataIntegerList[index] )
			RuiSetString( handle.cardRui, "statTracker" + index + "ValueSuffix", handle.overrideTrackerSuffixList[index] )
			continue
		}
