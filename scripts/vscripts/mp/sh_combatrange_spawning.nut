...
struct
{
	...
	// Rate-limit IsTotalDummiePopulationMaxed spam (AimTrainer can keep the pool full).
	float lastPopMaxedLogTime = -999.0
...
void function DynSpawns_AutoStart( entity player )
{
	...
	// Do NOT gate on IsCombatRangeMap. S3 FR maps often lack
	// info_spawnpoint_combatrange_target (log: "using spectre spawn points")
	// and may still have resident FR dummies. Manual toggle uses the same
	// UCB path without this check -- AutoStart must match.

	entity rangeLeader = RangeMaster_Get_ByPlayer( player )

	// Solo / first-ready: RangeMaster_Get can still be null if ready list
	// was never filled. Treat self as leader so default-ON dummies spawn.
	if( !IsValid( rangeLeader ) )
		rangeLeader = player
	...
	if( player == rangeLeader )
	{
		...
		// PVar bit defaults to 0 until written. If never set earlier,
		// fall back to the stock default (true) so AutoStart matches the UI.
		if( !enabled )
			enabled = frBoolSettings_Defaults[ eFRSettingType.DYNAMICDUMMIESON ]
...
{
	return DummieData_Count( realm ) >= file.maxDynamicDummies
}
...
bool function IsTotalDummiePopulationMaxed( int realm, bool forceOutput = false )
{
	...
	bool isMaxed = ( maxPopulated_AllDummies || maxPopulated_RealmDummies || maxPopulated_RealmDynDummies )

	// Spam guard: DynDummie_SpawnManager polls ~2 Hz; only log when forced or every 5s while maxed.
	const float POP_MAXED_LOG_INTERVAL = 5.0
	bool shouldLog = forceOutput
	if ( !shouldLog && isMaxed )
	{
		if ( ( Time() - file.lastPopMaxedLogTime ) >= POP_MAXED_LOG_INTERVAL )
		{
			shouldLog = true
			file.lastPopMaxedLogTime = Time()
		}
	}

	if( shouldLog && ( isMaxed || forceOutput ) )
	{
...
entity ornull function SpawnDummieAtSpawnPoint( int realm, entity spawnPt, int dummieSpawningType, bool debugDraws  )
{
	...
	vector spawnPtOrigin = spawnPt.GetOrigin()
	vector destination = NavMesh_GetClosestPoint( spawnPtOrigin )
	if ( !IsLegalWorldOrigin( destination ) )
		destination = spawnPtOrigin
	...
	if( spawnPt.HasKey( "exact_spawn" ))
	{
		...
		if( radiusSpawn )
		{
			...
			if ( !IsLegalWorldOrigin( randomPoint ) )
				...
				if( file.dev_spawnPrintsOn )
				{
					...
					DebugDrawCircleRGB( destination, <0, 0, 0>, spawnRadius, 255, 255, 255, true, circleTime )
					DebugDrawSphereRGB( destination, debugRadius, 255, 0, 0, true, circleTime )
					DebugDrawSphereRGB( randomPoint, debugRadius, 0, 0, 255, true, circleTime )
				}
			#endif
...
// AimTrainer freeroam dummies share FIRING_RANGE_DUMMIE_SCRIPT_NAME ("npc_dummie")
// but own npc.ai.aimTrainerOwner. They must NOT count against combat-range soft caps
// (maxDynamicDummies+3 == 7) or FR and AimTrainer thrash / log-spam / starve each other.
bool function IsCombatRangeCountedDummy( entity dummie )
{
	if ( !IsValid( dummie ) )
		return false

	// Owned by AimTrainer budget registry -- separate pool (AIMTRAINER_DUMMY_BUDGET).
	if ( IsValid( AimTrainer_GetDummyOwner( dummie ) ) )
		return false
...
table< string, array< entity > > function Dummies_GetAll( int realm )
{
	...
	array< entity > allDummiesRaw = GetEntArrayByScriptName( FIRING_RANGE_DUMMIE_SCRIPT_NAME )
	allDummiesRaw.extend( GetEntArrayByScriptName( FIRING_RANGE_COMBAT_DUMMIE_SCRIPT_NAME ) )

	array< entity > allDummies = []
	foreach( dummie in allDummiesRaw )
	{
		if ( !IsCombatRangeCountedDummy( dummie ) )
			continue
		allDummies.append( dummie )
	}
...
void function DummieData_Remove( entity dummie, bool doDestroy = true )
{
	...
	if( !IsValid( dummie ) && !IsInvalidButMemberVarsStillValid( dummie ) )
		return
...
void function DEV_CombatRange_Show_SpawnPoints( float showTime = 10 )
{
	...
	foreach( spawnPoint in spawnPoints )
	{
		...
		{
			DebugDrawSphereRGB( spawnPoint.GetOrigin(), 50, 255, 0, 0, true, showTime )
		}
		else
		{
...
void function DEV_CountDummies( entity playerParm )
{
	...
	foreach( dummie in file.dummiesSpawnedInRealm[ realm ] )
	{
		DebugDrawSphereRGB( dummie.GetOrigin(), 48, 0, 255, 0, true, 20 )
	}
	...
	foreach( dummie in allDummiesByName )
	{
		if( dummie.GetRealms()[ 0 ] == realm )
		{
			...
			{
				DebugDrawSphereRGB( dummie.GetOrigin(), 64, 255, 0, 0, true, 20 )
			}
		}
	}
...
#if DEVELOPER // DEVELOPER for Client || Server
