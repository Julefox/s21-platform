...
void function ControlPanelRefresh( entity panel )
{
	...
	// Survey beacons (and other non-prop_control_panel panels) get
	// ServerCallback_ControlPanelRefresh after use, but never hit
	// CreateCallback_Panel -> ControlPanelInit, so panel.s has no updateCallbacks.
	if ( !( "updateCallbacks" in panel.s ) )
		return
