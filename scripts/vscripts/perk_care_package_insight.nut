...
void function Perks_CarePackageInsight_ClientToServer_MinimapIconPinged( entity player, entity minimapEnt )
{
	if ( Perks_CarePackageInsight_IsClientDataValid( player, minimapEnt, false ) )
	{
		...
		int pingType = minimapEnt.GetAreContentsTaken() ?  ePingType.CAREPACKAGE_INSIGHT_LOOTED : ePingType.CAREPACKAGE_INSIGHT
...
void function Perks_CarePackageInsight_ClientToServer_RevealPackage( entity player, entity revealEnt )
{
	...
	if( !alreadyRevealed )
	{
		...
		LootData data               = SURVIVAL_Loot_GetLootDataByIndex( revealedEnt.GetLootIndex() )
		vector carePackageLookatPos = carePackage.GetOrigin() + <0, 0, CAREPACKAGE_WAYPOINT_UP_OFFSET>
...
{
	int team = player.GetTeam()
	// no per-team transmit filtering here, so a revealed package reads as visible to everyone
	if( true )
	{
		int lootIndex = carePackage.GetLootIndex()
...
void function Perks_CarePackageInsight_CreateCarePackageInsightPing( entity player, entity revealEnt, vector pingOrigin, int userTicketId )
{
	...
	int pingType = revealEnt.GetAreContentsTaken() ? ePingType.CAREPACKAGE_INSIGHT_LOOTED : ePingType.CAREPACKAGE_INSIGHT
...
// Dev spawner: stands up a care package and its hidden/revealed markers without
// waiting for a real airdrop. From the dedicated server console:
//   script DEV_SpawnCarePackageInsight( GetPlayerArray()[0] )
//   script DEV_SpawnCarePackageInsight( GetPlayerArray()[0], "mp_weapon_energy_ar" )
void function DEV_SpawnCarePackageInsight( entity player, string lootRef = "mp_weapon_energy_ar", float forwardDist = 2000.0 )
{
	if ( !IsValid( player ) || !player.IsPlayer() )
	{
		printt( "DEV_SpawnCarePackageInsight: needs a valid player" )
		return
	}

	if ( !SURVIVAL_Loot_IsRefValid( lootRef ) )
	{
		printt( "DEV_SpawnCarePackageInsight: unknown loot ref", lootRef )
		return
	}

	LootData lootData = SURVIVAL_Loot_GetLootDataByRef( lootRef )

	// The perk's client half only starts once the passive flips, and the passive
	// is networked, so this is what gets both realms to activate it.
	if ( !player.HasPassive( ePassives.PAS_UPGRADE_CAREPACKAGE_INSIGHT ) )
		GivePassive( player, ePassives.PAS_UPGRADE_CAREPACKAGE_INSIGHT )
	if ( !Perks_DoesPlayerHavePerk( player, ePerkIndex.CARE_PACKAGE_INSIGHT ) )
		Perks_AddPerk( player, ePerkIndex.CARE_PACKAGE_INSIGHT )

	vector wantPos = player.GetOrigin() + AnglesToForward( <0, player.EyeAngles().y, 0> ) * forwardDist
	vector origin  = GetGroundPosition( wantPos + <0, 0, 512>, wantPos - <0, 0, 8192> ).endPos

	entity carePackage = CreatePropDynamic( SURVIVAL_LOOT_POD_MODEL, origin, <0, 0, 0>, SOLID_VPHYSICS )
	SetTargetName( carePackage, CARE_PACKAGE_TARGETNAME )
	carePackage.RemoveFromAllRealms()
	carePackage.AddToOtherEntitysRealms( player )

	entity hiddenMarker = CreateEntity( "prop_care_package_insight" )
	hiddenMarker.RemoveFromAllRealms()
	hiddenMarker.AddToOtherEntitysRealms( player )
	hiddenMarker.SetValueForModelKey( $"mdl/dev/empty_model.rmdl" )
	hiddenMarker.SetOrigin( origin )
	hiddenMarker.MakeInvisible()
	hiddenMarker.SetScriptName( HIDDEN_CARE_PACKAGE_ENT_NAME )
	DispatchSpawn( hiddenMarker )

	entity revealedMarker = CreateEntity( "prop_care_package_insight" )
	revealedMarker.RemoveFromAllRealms()
	revealedMarker.AddToOtherEntitysRealms( player )
	revealedMarker.SetValueForModelKey( $"mdl/dev/empty_model.rmdl" )
	revealedMarker.SetOrigin( origin )
	revealedMarker.MakeInvisible()
	revealedMarker.SetScriptName( REVEALED_CARE_PACKAGE_ENT_NAME )
	revealedMarker.SetLootIndex( lootData.index )
	DispatchSpawn( revealedMarker )

	hiddenMarker.LinkToEnt( carePackage )
	revealedMarker.LinkToEnt( carePackage )

	CarePackageData droppod
	droppod.carePackage    = carePackage
	droppod.highestLootItem = lootData.ref
	droppod.revealedEnt    = revealedMarker
	droppod.revealEnt      = hiddenMarker
	file.carePackageArray[carePackage] <- droppod

	Perks_AddMinimapEntityForPerk( ePerkIndex.CARE_PACKAGE_INSIGHT, carePackage )

	printt( "DEV_SpawnCarePackageInsight: at", origin, "ref", lootData.ref,
		"index", lootData.index, "tier", lootData.tier,
		"readback", revealedMarker.GetLootIndex(), "contentsTaken", revealedMarker.GetAreContentsTaken() )
}


// Flips every spawned marker to looted, which is the other half of the wire
// (the map icon should grey out).
void function DEV_CarePackageInsightMarkLooted()
{
	int n = 0
	foreach ( ent, carePackageData in file.carePackageArray )
	{
		if ( !IsValid( carePackageData.revealedEnt ) )
			continue
