...
{
	EndSignal( soundMover, "OnDestroy" )

	soundMover.e.isDisabled = true

	OnThreadEnd( void function() : ( soundMover ) {
		if( IsValid( soundMover ) )
			soundMover.Destroy()
	} )

	// S3 WaitSignalTimeout returns null on timeout (not a table).
	table ornull result = WaitSignalTimeout( soundMover, file.soundMoverDestroyIfNotStartedTime, START_SOUND_SIGNAL )

	if ( result == null )
		return

	table resultTable = expect table( result )
	if ( !( "signal" in resultTable ) || resultTable.signal != START_SOUND_SIGNAL )
		return

	soundMover.e.isDisabled = false

	EmitSoundOnEntity( soundMover, sound )

	wait file.soundMoverDuration
}
...
void function CreateWallThread( entity weapon, vector pos, vector dir, int numPillars, float delay, float initalStep = FERRO_WALL_STEP_INIT )
{
	...
	for ( int i = 0; i < numPillars; i++ )
	{
		...
		if( forwardTrace.fraction < 1.0 )
		{
			...
			DebugDrawLineRGB( traceStart, forwardTrace.endPos, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 25.0 )
		#endif
		...
		#if DEVELOPER && FERRO_WALL_DEBUG_DRAW
			DebugDrawLineRGB( forwardTrace.endPos, downTrace.endPos, int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), true, 25.0 )
		#endif
...
void function CreateFerroWallPillar( asset model, asset baseFX, vector pos, vector angles, entity attacker, entity newParent, int teamId, int modelSize, SoundMoverData soundMoverData, entity infoTarget = null, int variationIndex = 0, bool inWater = false )
{
	...
	// S3 has no TRACE_COLLISION_GROUP_PERMEABLE; BLOCK_WEAPONS is the walk-through group (COLGROUP-XLAT 19->21).
	pillar.kv.CollisionGroup = TRACE_COLLISION_GROUP_BLOCK_WEAPONS
	pillar.SetPassThroughThickness( 0 )
	pillar.SetPassThroughDirection( 0 )
	// S3 has no PTF_BLOCKS_PING; CONTENTS_BLOCK_PING already covers ping.
	pillar.SetPassThroughFlags( PTF_ADDS_MODS | PTF_NO_DMG_ON_PASS_THROUGH )
	...
	if( FERRO_WALL_FX_VERSION == 2 )
	{
		...
		DebugDrawBoxRGB( pillar.GetOrigin(), mins, maxs, int( COLOR_GREEN.x ), int( COLOR_GREEN.y ), int( COLOR_GREEN.z ), 1, 25.0 )
...
#if ( DEVELOPER && FERRO_WALL_DEBUG_DRAW )
			DebugDrawLineColor( traceStart, forwardTrace.endPos, COLOR_GREEN, true, 25.0 )
#endif
...
#if ( DEVELOPER && FERRO_WALL_DEBUG_DRAW )
			DebugDrawLineColor( forwardTrace.endPos, downTrace.endPos, COLOR_BLUE, true, 25.0 )
#endif
