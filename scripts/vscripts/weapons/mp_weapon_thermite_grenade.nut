...
#else
void function OnProjectileCollision_weapon_thermite_gun( entity projectile, vector pos, vector normal, entity hitEnt, int hitbox, bool isCritical, bool isPassthrough )
#endif
{
	entity player = projectile.GetOwner()
	if ( hitEnt == player )
...
array<SegmentData> function CreateSpreadPattern( entity owner, entity inflictor, vector pos, vector dir, int stepCount, BurnDamageSettings burnSettings )
{
	...
	for ( int i = 0; i < stepCount; i++ )
	{
		...
		{
			#if DEVELOPER && DEBUG_THERMITE_GRENADE_TRACES
				DebugDrawLineRGB( forwardTrace.endPos, forwardTrace.endPos + <0,0,-225>, 255, 0, 0, true, 25.0 )
			#endif
