...
void function PhaseTunnel_OnProjectileHitVortexTrigger( entity weapon, entity vortexSphere, entity attacker, entity projectile, vector contactPos )
{
	...
	/*if ( PHASE_TUNNEL_DEBUG_DRAW_PROJECTILE_TELEPORT )
	{
		DebugDrawLineRGB( contactPos, contactPos + (velNorm * 64), int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 30.0 ) //Grenade Entry Vel
		DebugDrawLineRGB( startPortalData.portalFX.GetOrigin(), startPortalData.portalFX.GetOrigin() + (startPortalData.portalFX.GetUpVector() * 64), int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 30.0 ) //Grenade Entry Vel

		DebugDrawLineRGB( endPortalData.portalFX.GetOrigin(), impactPoint, int(COLOR_CYAN.x), int(COLOR_CYAN.y), int(COLOR_CYAN.z), true, 30.0 ) //Grenade Entry Vel
		DebugDrawLineRGB( endFXOrigin, endFXOrigin + exitPoint, int(COLOR_MAGENTA.x), int(COLOR_MAGENTA.y), int(COLOR_MAGENTA.z), true, 30.0 ) //Grenade Entry Vel

		DebugDrawLineRGB( contactPos, endFXOrigin + exitPoint, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), true, 30.0 ) //Grenade Entry Vel

		DebugDrawLineRGB( endFXOrigin, endFXOrigin + (endPortalData.portalFX.GetUpVector() * 64), int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 30.0 ) //Grenade Entry Vel
		DebugDrawLineRGB( endFXOrigin + exitPoint, (endFXOrigin + exitPoint) + (velNorm * 64), int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 30.0 ) //Grenade Entry Vel
		DebugDrawLineRGB( endFXOrigin + exitPoint, (endFXOrigin + exitPoint) + (newDir * 128), int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), true, 30.0 ) //Grenade Entry Vel
	}*/
}
...
void function PhaseTunnel_MoveEntAlongPath( entity player, PhaseTunnelPathData pathData, PhaseTunnelTravelState travelState )
{
	...
	for ( int i = 0; i < frameSteps.len(); i++ )
	{
		...
		if ( PHASE_TUNNEL_DEBUG_DRAW_PLAYER_TRAVEL )
		{
			...
			DebugDrawLineRGB( prevPosition, player.GetOrigin(), int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), false, DRAW_TIME ) //Prev Player Position to current Pos
	...
	if ( PHASE_TUNNEL_DEBUG_DRAW_PLAYER_TRAVEL )
	{
		...
		DebugDrawSphereRGB( player.GetOrigin(), 10, int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), false, DRAW_TIME )
		DebugDrawSphereRGB( pathNodeDataArray[stopPathIndex].origin, 6, int(COLOR_LIGHT_BLUE.x), int(COLOR_LIGHT_BLUE.y), int(COLOR_LIGHT_BLUE.z), false, DRAW_TIME )
		DebugDrawLineRGB( player.GetOrigin(), pathNodeDataArray[stopPathIndex].origin, int(COLOR_LIGHT_BLUE.x), int(COLOR_LIGHT_BLUE.y), int(COLOR_LIGHT_BLUE.z), false, DRAW_TIME )
	}
...
vector function GetNextAngleToLookAt( int currentIndex, int step, array< PhaseTunnelPathNodeData > pathNodeDataArray )
{
	...
	{
		DebugDrawSphereRGB( startPosition, 2, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 10.0 )
		DebugDrawLineRGB( startPosition, nextPosition, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 10.0 )
	}
...
bool function PhaseTunnel_ShouldPhaseEnt( entity target )
{
	...
	// Free-function form: these are stand-ins here, not entity methods.
	if ( Player_IsSkywardFollowing( target ) )
	{
		ValkUlt_AllyCancel( target )
	}

	if ( Player_IsSkywardLaunching( target ) )
		return false
...
bool function IsValidWorldExitPos( TraceResults results )
{
	...
	// IsPlayerVehicle is not a SERVER-VM entity method here.
