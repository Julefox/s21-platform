...
void function OnWeaponDeactivate_weapon_horizon_heirloom_primary( entity weapon )
{
		...
		if ( IsValid( weapon ) )
		{
			weapon.SetScriptFloat0( 0.0 )
			printt( "Resetting Weapon Sprint Spam" )
		}
	#endif
...
{
	if ( !ent.IsWeaponX() && ent.GetModelName().find( HORIZON_MODEL_IDENTIFIER ) == -1 ) // R5DEV-567984 - in rare cases when swapping weapons, we'll get a view model & weapon mismatch
		return
...
{
	StopPermanentVFX( weapon ) // Safety Check to ensure we don't double play vfx
