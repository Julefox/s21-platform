...
void function SpawnMobileBeacon_Sequence( vector origin, vector angles, vector surfaceNormal, entity owner )
{
	...
	entity beaconMover = CreateScriptMover( MOBILE_RESPAWN_BEACON_MOVER_SCRIPTNAME, respawnBeacon.GetOrigin(), respawnBeacon.GetAngles() )
	...
	CreateAirdropBadPlace( respawnBeacon, origin, MOBILE_RESPAWN_BEACON_BAD_AIRSPACE_RADIUS )  // Prevent placement of beacons too close together while it's coming down
		...
		if ( MOBILE_RESPAWN_BEACON_DEBUG_DRAW )
		{
			 DebugDrawCylinderRGB( origin, <270.0, 0.0, 0.0>, MOBILE_RESPAWN_BEACON_BAD_AIRSPACE_RADIUS, 2.0, int( COLOR_RED.x ), int( COLOR_RED.y ), int( COLOR_RED.z ), true, TotalTimeToLand )
...
void function SpawnMobileBeacon_SetupPushAway_Thread( entity beaconPod, vector landingPos )
{
	...
	pushAwayTrigger.SetCylinderRadius( 100 )
	...
	// Re-enable this to see the cylinder of prop destruction
		...
		if ( MOBILE_RESPAWN_BEACON_DEBUG_DRAW )
		{
			DebugDrawCylinderRGB( pushAwayTrigger.GetOrigin() - <0.0, 0.0, pushAwayTrigger.GetBelowHeight()>, <270.0, 0.0, 0.0>, pushAwayTrigger.GetRadius(), pushAwayTrigger.GetAboveHeight() + pushAwayTrigger.GetBelowHeight(), int( COLOR_WHITE.x ), int( COLOR_WHITE.y ), int( COLOR_WHITE.z ), true, 3.0 )
		}
	#endif
...
void function RespawnUserTeam_Mobile( entity ent, entity playerUser, ExtendedUseSettings settings )
{
	...
	if ( isOnSlope )
	{
		...
		beaconMover = CreateScriptMover( MOBILE_RESPAWN_BEACON_MOVER_SCRIPTNAME, beaconPlaceholder.GetOrigin(), ent.GetAngles() )
	...
	if ( IsValid( beaconPlaceholder ) )
	{
		...
		beaconPlaceholder.Dissolve( ENTITY_DISSOLVE_NONE, <0,0,0>, 200 )
	}
