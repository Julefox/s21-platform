...
var function OnWeaponPrimaryAttack_weapon_trophy_defense_system( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	if ( !weapon.ObjectPlacementHasValidSpot() )
	{
		...
		thread Trophy_Deploy( ownerPlayer, placementInfo, vehicleAttachmentIndex )
	#endif
...
TrophyPlacementInfo function _GetPlacementInfo( entity player, entity proxy, vector eyePos, vector viewVec, bool doUpTrace = true )
{
	...
	{
		DebugDrawBoxRGB( fwdResults.endPos, TROPHY_BOUND_MINS, TROPHY_BOUND_MAXS, int( COLOR_GREEN.x ), int( COLOR_GREEN.y ), int( COLOR_GREEN.z ), 1, 1.0 ) //Forward Hull Cast Bounding Box
		DebugDrawBoxRGB( downResults.endPos, TROPHY_BOUND_MINS, TROPHY_BOUND_MAXS, int( COLOR_BLUE.x ), int( COLOR_BLUE.y ), int( COLOR_BLUE.z ), 1, 1.0 ) //Downward Hull Cast Bounding Box
		DebugDrawLineRGB( eyePos + viewVec * min( TROPHY_PLACEMENT_RANGE_MIN, maxRange ), fwdResults.endPos, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 1.0 ) //Forward Hull Cast
		DebugDrawLineRGB( fwdResults.endPos, eyePos + viewVec * maxRange, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 1.0 ) //Forward Hull Cast Blocked
		DebugDrawLineRGB( fwdResults.endPos, downResults.endPos, int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), true, 1.0 ) //Downward Hull Cast
		DebugDrawBoxRGB( upResults.endPos, TROPHY_BOUND_MINS, TROPHY_BOUND_MAXS, int( COLOR_CYAN.x ), int( COLOR_CYAN.y ), int( COLOR_CYAN.z ), 1, 1.0 ) //"Upward" Hull Cast Bounding Box
		DebugDrawLineRGB( upStart, upResults.endPos, int(COLOR_CYAN.x), int(COLOR_CYAN.y), int(COLOR_CYAN.z), true, 1.0 ) //"Upward" Hull Cast
		DebugDrawLineRGB( eyePos, roofTraceEnd, int(COLOR_MAGENTA.x), int(COLOR_MAGENTA.y), int(COLOR_MAGENTA.z), true, 1.0 ) //Roof Check
		DebugDrawLineRGB( player.GetOrigin(), player.GetOrigin() + (AnglesToForward( angles ) * TROPHY_PLACEMENT_RANGE_MAX), int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 1.0 ) //Max Placement Dist
		DebugDrawLineRGB( eyePos + <0, 0, 8>, eyePos + <0, 0, 8> + (viewVec * TROPHY_PLACEMENT_RANGE_MAX), int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 1.0 ) //Max Placement Dist
	}
	...
	if ( success && useResults.fraction < 1.0 )
	{
		...
		if ( TROPHY_DEBUG_DRAW_PLACEMENT )
		{
			DebugDrawLineRGB( proxy.GetOrigin(), proxy.GetOrigin() + (right * 64), int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 1.0 ) //Ground Right Vector
			DebugDrawLineRGB( proxy.GetOrigin(), proxy.GetOrigin() + (forward * 64), int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), true, 1.0 ) //Ground Forward Vector
		}
		...
		foreach ( vector testOffset in groundTestOffsets )
		{
			...
			if ( TROPHY_DEBUG_DRAW_PLACEMENT )
				DebugDrawLineRGB( testPos + (proxy.GetUpVector() * TROPHY_PLACEMENT_MAX_GROUND_DIST), traceResult.endPos, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 1.0 ) //Ground Hull Cast
...
void function Trophy_Deploy( entity owner, TrophyPlacementInfo placementInfo, int vehicleAttachmentIndex )
{
	...
	trophy.SetBoundingBox( TROPHY_BOUND_MINS, TROPHY_BOUND_MAXS )
	trophy.AllowMantle()
	...
	trophy.EnableAttackableByAI( AI_PRIORITY_NO_THREAT, 0, AI_AP_FLAG_SMART_AI_ONLY )
	...
	if ( placementInfo.parentTo != null )
	{
		...
		{
			// TODO(s3): HoverVehicle_SetAbilityAttachmentEntity missing on S3 / compile — restore when bound
...
bool function Trophy_HasLOSToPlayer( entity trophy, entity player )
{
	...
	foreach ( testVector in testEndVectors )
	{
		...
		{
			DebugDrawLineRGB( traceResults.endPos, traceEnd, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 1.0 )
			DebugDrawLineRGB( traceStart, traceResults.endPos, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 1.0 )
		}
...
void function Trophy_CheckForGeoIntersection( entity trophy )
{
	...
	while ( true )
	{
		...
		{
			DebugDrawBoxRGB( results.endPos, TROPHY_INTERSECTION_BOUND_MINS, TROPHY_INTERSECTION_BOUND_MAXS, int( COLOR_GREEN.x ), int( COLOR_GREEN.y ), int( COLOR_GREEN.z ), 1, 1.0 ) //Forward Hull Cast Bounding Box
		}
...
bool function Trophy_ShouldTargetProjectile( entity trophy, entity projectile, vector contactPos )
{
	...
	if ( TROPHY_DEBUG_DRAW )
	{
		DebugDrawLineRGB( projectileOrigin, projectileOrigin + (velDirSaved * 128), int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 20.0 )
		DebugDrawLineRGB( projectileOrigin, projectileOrigin + (trophyToProj * 128), int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 20.0 )
	}
	...
	if ( distSqr <= file.trophy_interceptProjectileRangeSqr && newOldDot > 0.0 )
	{
		...
		{
			if ( TROPHY_DEBUG_DRAW )
				DebugDrawLineRGB( projectileOrigin, traceResults.endPos, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 20.0 )
