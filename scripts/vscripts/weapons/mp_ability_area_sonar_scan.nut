...
var function OnWeaponPrimaryAttackAnimEvent_ability_area_sonar_scan( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	EmitSoundOnPS5Controller( "/host/SonarScan_ButtonPress_1p_2ch_v1_01.wav", "/host/wpn_light_vib_single.wav" )
		...
		thread PlayBattleChatterLineDelayedToSpeakerAndTeam( weaponOwner, weapon.GetWeaponSettingString( eWeaponVar.battle_chatter_event ), 0.4 )
	#endif //SERVER
...
{
	#if DEVELOPER
	if ( AREA_SONAR_PERF_TESTING )
		PerfStart( PerfIndexServer.AreaSonarScan  )
	#endif
...
// S21: owner.Anim_PlayAttackGesture — S3 has no entity method (cosmetic)
...
#if DEVELOPER
		if ( AREA_SONAR_PERF_TESTING )
			PerfStart( PerfIndexServer.AreaSonarTargetList )
		#endif
...
#if DEVELOPER
		if ( AREA_SONAR_PERF_TESTING )
			PerfEnd( PerfIndexServer.AreaSonarTargetList )
#endif
	}
	...
	else
	{
		#if DEVELOPER
		if ( AREA_SONAR_PERF_TESTING )
			PerfStart( PerfIndexServer.AreaSonarScanTriggers )
		#endif
		...
		#if DEVELOPER
			if ( AREA_SONAR_PERF_TESTING )
				PerfEnd( PerfIndexServer.AreaSonarScanTriggers )
		#endif
	...
	if ( USE_FASTER_SCAN_LOGIC )
	{
		#if DEVELOPER
		if ( AREA_SONAR_PERF_TESTING )
			PerfStart( PerfIndexServer.AreaSonarScanPropsList )
		#endif
		...
		#if DEVELOPER
		if ( AREA_SONAR_PERF_TESTING )
			PerfEnd( PerfIndexServer.AreaSonarScanPropsList )
		#endif
	}
	else
	{
		#if DEVELOPER
		if ( AREA_SONAR_PERF_TESTING )
			PerfStart( PerfIndexServer.AreaSonarScanPropsTrigger )
		#endif
		...
		#if DEVELOPER
		if ( AREA_SONAR_PERF_TESTING )
			PerfEnd( PerfIndexServer.AreaSonarScanPropsTrigger )
		#endif
	}
	...
	if ( AREA_SONAR_PERF_TESTING )
	{
		PerfEnd( PerfIndexServer.AreaSonarScan )
		PerfDump()
	}
	#endif
...
void function AreaSonarScan_CheckandScanValidTarget( entity owner, entity ent )
{
	if ( !IsEnemyTeam( owner.GetTeam(), ent.GetTeam() )
		...
		&& !TrackingVision_IsEntWhiteRaven( owner, ent )
	...
	if ( angle > matchAngle )
		...
		if( TrackingVision_IsEntWhiteRaven( owner, ent ) )
		{
			if( !ent.e.isBusy ) //!TrackingVision_HasActiveWhiteRaven( owner ) )
			{
				bool shouldRefundTac = true
				if ( owner in file.areaScanTargets )
				{
					foreach ( target in file.areaScanTargets[owner][file.areaScanIndex[owner]] )
					{
						if( IsValid( target ) && target.IsPlayer() )
						{
							//We have a player in the scan - no full refund.
							shouldRefundTac = false
							break
						}
					}
				}

				TrackingVision_TriggerBirdFromScan( owner, ent, shouldRefundTac )
			}
		}
	...
	if ( FerroWall_BlockScan( owner.EyePosition(), ent.GetCenter() ) )
		return

	// NPC pass + sonar-prop registry can both yield the same ent (e.g. FR dummies).
	if ( owner in file.areaScanTargets )
	{
		int scanIdx = file.areaScanIndex[owner]
		if ( scanIdx in file.areaScanTargets[owner] && file.areaScanTargets[owner][scanIdx].contains( ent ) )
			return
	}
...
void function AreaSonarScan_OnSonarTriggerEnter( entity trigger, entity ent )
{
	...
	if ( FerroWall_BlockScan( trigger.GetOrigin(), ent.GetCenter() ) )
		return
