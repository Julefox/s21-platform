...
void function JetDrive_Thread( entity player, entity companion )
{
	...
	while ( player.IsJetDriveActive() )
	{
		...
		{
			DebugDrawSphereColor( player.GetOrigin(), 20, <0, 200, 255>, false, 30 )
			DebugDrawLineColor( previousPos, player.GetOrigin(), <0, 100, 255>, false, 30 )
		}
...
void function DEV_DebugDrawPathPoints( array<vector> pathPoints, float time = 5 )
{
	...
	{
		DebugDrawSphereColor( point, 10, COLOR_YELLOW, false, time )
		if ( prevPathPoint != ZERO_VECTOR )
		{
			DebugDrawLineColor( point, prevPathPoint, <100,255,0>, false, time )
		}
		prevPathPoint = point
	}
}
