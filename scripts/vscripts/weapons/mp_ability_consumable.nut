...
struct
{
	...
	table< entity, float > playerToLastShieldChatterTime
		...
		float lastConsumableTryTime
		string lastConsumableTryMod
	#endif // CLIENT
...
void function Consumable_Init()
{
	...
	Remote_RegisterServerFunction( "ClientCallback_SetSelectedConsumableTypeNetInt", "int", 0, eConsumableType.COMBO_FULL )
...
void function AddModAndFireWeapon_Thread( entity player, string modName )
{
	...
	if ( file.lastConsumableTryMod == modName && Time() - file.lastConsumableTryTime < 0.35 )
		return
	file.lastConsumableTryMod = modName
	file.lastConsumableTryTime = Time()
...
void function SetSelectedConsumableTypeNetInt( entity player, int consumableType )
{
	switch( consumableType )
	{
		...
		default:
			Warning( "SetSelectedConsumableTypeNetInt: unsupported type " + consumableType )
			return
	}
}
...
{
#if CLIENT
	// Duel rest/spectate: no heals in waiting room (legacy playlist gate).
	if ( FS_HasCap( FS_CAP_REST_ROOM ) && IsValid( player ) )
	{
		int st = player.GetPlayerNetInt( "FS_1v1_PlayerState" )
		if ( st == e1v1State.RESTING || st == e1v1State.SPECTATING || st == e1v1State.WAITING )
