...
// BreachTrace is engine-native on S21 client; S3 dedi registers a bridge native.
...
void function MpWeaponRiotDrill_Init()
{
	...
	// Both names are handed to SetScriptName below, and _weapon_utility compares
	// against RIOT_DRILL_SCRIPT_NAME for the sticky-projectile path.
...
void function RiotDrill_CreateBreachingCharge_System( RiotDrillPlacementInfo placementInfo, entity owner, entity hitEnt )
{
	...
	fbs.riotDrillDrillMover = CreateScriptMover( RIOT_DRILL_MOVER_SCRIPTNAME, placementInfo.startOrigin, spikeEndAngles )
	if ( hitEnt.IsMoverOrChildOfMover() || hitEnt.GetScriptName() == WRECKING_BALL_BALL_SCRIPT_NAME )
	{
...
array<entity> function RiotDrill_CreateDangerZoneFX( RiotDrillSystem fbs )
{
	...
	if( DEBUG_INFO )
		DebugDrawCylinderRGB( origin, VectorToAngles( Normalize( origin - fbs.riotDrillStart.GetOrigin() ) ), RiotDrill_GetRadius( ownerPlayer ), RiotDrill_GetLength( ownerPlayer ), 0, 200, 0, true, RiotDrill_GetDuration( ownerPlayer ) )
	...
	while ( player.IsUsingOffhandWeapon( eActiveInventorySlot.altHand ) )
	{
		...
		else
		{
				...
				|| placementInfo.placementResult == eBreachPlacementResult.FAILED_SAFETY_CATCH )
			{
				...
				if ( lastDrillErrorState != 1 )
				{
					lastDrillErrorState = 1
			...
			else
			{
				...
				if ( lastDrillErrorState != 0 )
				{
					lastDrillErrorState = 0
					Remote_ServerCallFunction( "ClientCallback_DrillError_Off" )
				}
			}
