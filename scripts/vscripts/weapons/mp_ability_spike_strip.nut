...
#if SERVER
void function OnProjectileCollision_ability_spike_strip( entity projectile, vector pos, vector normal, entity hitEnt, int hitBox, bool isCritical )
#else
...
void function SpikeTotemThread( entity totem, entity owner  )
{
	...
	useEntity.NotSolid()
	useEntity.SetBoundingBox( SPIKE_STRIP_USE_BOUNDING_MINS, SPIKE_STRIP_USE_BOUNDING_MAXS )
...
void function GetAdjustedTestOffsets( entity mainSpike, array< vector > testOffsets )
{
	...
	foreach( offset in SPIKE_STRIP_TEST_OFFSETS )
	{
		...
		testOffsets.append( testTrace.endPos )
			...
			DebugDrawSphereRGB( testTrace.endPos, 5.0, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 5.0 )
		#endif
	}
}
...
array<SpikeData> function CreateSpikeLineData2( entity owner, entity mainSpike, entity projectileHitEnt, vector pos, vector dir, vector angles, int stepCount, float spacing, bool ignoreFirst, array< vector > testOffsets )
{
	...
	for ( int i = 0; i < stepCount; i++ )
	{
		...
		{
			#if DEVELOPER && SPIKE_STRIP_DEBUG
				DebugDrawSphereRGB( downTrace.endPos, 5.0, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 5.0 )
			#endif
			continue
		}
		...
		{
			#if DEVELOPER && SPIKE_STRIP_DEBUG
				DebugDrawSphereRGB( downTrace.endPos, 5.0, int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), true, 5.0 )
			#endif
			continue
		}

		#if DEVELOPER && SPIKE_STRIP_DEBUG
			DebugDrawSphereRGB( downTrace.endPos, 5.0, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), true, 5.0 )
		#endif
	...
	for ( int i = 0; i < stepCount; i++ )
	{
		...
		if ( !firstTrace )
		{
			...
			if ( useUpwardTraces )
			{
				...
				#if DEVELOPER && SPIKE_STRIP_DEBUG
					DebugDrawLineRGB( lastDownPos, upwardTrace.endPos, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 25.0 )
				#endif
		...
		#if DEVELOPER && SPIKE_STRIP_DEBUG
			DebugDrawLineRGB( traceStart, traceEndUnder, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 25.0 )
		#endif
		...
		if ( forwardTrace.fraction > 0.5 )
		{
			...
			#if DEVELOPER && SPIKE_STRIP_DEBUG
				DebugDrawLineRGB( downTraceStartPos, downTrace.endPos, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 25.0 )
			#endif
		...
		#if DEVELOPER && SPIKE_STRIP_DEBUG
			DebugDrawLineRGB( traceStart, traceEndOver, int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), true, 25.0 )
		#endif
		...
		else
		{
			...
			#if DEVELOPER && SPIKE_STRIP_DEBUG
				DebugDrawLineRGB( upwardImpactTrace.endPos, downTrace.endPos, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 25.0 )
			#endif
...
entity function CreateSpike( entity owner, entity newParent, vector pos, vector angles, int team, float duration, float health, asset model = SPIKE_STRIP_SPIKE_DORMANT, entity mainSpike = null  )
{
	...
	// S3 has no TRACE_COLLISION_GROUP_PERMEABLE; BLOCK_WEAPONS is walk-through (COLGROUP-XLAT 19->21).
	spike.kv.CollisionGroup = TRACE_COLLISION_GROUP_BLOCK_WEAPONS
	spike.SetPassThroughThickness( 0 )
	spike.SetPassThroughDirection( 0 )
	// S3 has no PTF_BLOCKS_PING.
	spike.SetPassThroughFlags( PTF_ADDS_MODS )
	...
	while( true )
	{
		...
		foreach( spike in linkedSpikes )
		{
			if( IsValid( spike ) )
			{
				...
				DebugDrawCylinderRGB( cylinderBottom, RotateAnglesAboutAxis( spike.GetAngles(), spike.GetRightVector(), 90 ), file.pieceRadius, file.pieceHeight, int( COLOR_YELLOW.x ), int( COLOR_YELLOW.y ), int( COLOR_YELLOW.z ), true, 0.1 )
			}
		}

		vector cylinderBottom  = totem.GetOrigin()
		DebugDrawCylinderRGB( cylinderBottom, RotateAnglesAboutAxis( totem.GetAngles(), totem.GetRightVector(), 90 ), file.pieceRadius, file.pieceHeight, int( COLOR_YELLOW.x ), int( COLOR_YELLOW.y ), int( COLOR_YELLOW.z ), true, 0.1 )
		#endif
