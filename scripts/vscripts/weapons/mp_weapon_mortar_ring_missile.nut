...
// Calc*Origin/Angles_Entity: engine-native on S21 client; S3 dedi SDK natives
// (EntityToWorldTransform + mathlib) in server.dll.
...
void function BeginBombletFire( entity owner, entity projectile, entity projectile2, vector dir, vector dirToCenter )
{
	...
	foreach ( segment in segmentsArray )
	{
			...
			DebugDrawSphereRGB( segment.startPos + <0, 0, 10>, 10, 255, 0, 0, true, 25.0 )
			//DebugDrawArrow( segment.startPos, segment.endPos, 25, <0, 0, 255>, true, 25.0)
			DebugDrawSphereRGB( segment.endPos, 10, 0, 255, 0, true, 25.0 )
		#endif
	}
...
vector function FindValidEndPosForFireSegment( vector startPos, vector dir, float maxFlattnedDistance, entity moveParent, array< entity > ignoreArray )
{
	...
	while( !endTraces )
	{
		...
		#if DEVELOPER && MORTAR_RING_MISSILE_DEBUG
			DebugDrawLineRGB( traceStart, forwardTrace.endPos, 0, 255, 0, true, 25.0 )
		#endif
		...
		#if DEVELOPER && MORTAR_RING_MISSILE_DEBUG
			DebugDrawLineRGB( forwardTrace.endPos, downTrace.endPos, 0, 0, 255, true, 25.0 )
		#endif
...
void function MortarRingFireSegmentTriggerThread( entity effect, entity controlPoint, entity moveParent, entity owner, float radius, bool preburn )
{
	...
	while ( true )
	{
			...
			DebugDrawSphereRGB( effect.GetOrigin(), 10, 0, 255, 0, true, 0.1 )
			DebugDrawSphereRGB( controlPoint.GetOrigin() + <0, 0, 10>, 10, 255, 0, 0, true, 0.1 )
			//DebugDrawArrow( controlPoint.GetOrigin(), effect.GetOrigin(), 25, <0, 0, 255>, true, 0.1)
			DebugDrawLineRGB( effect.GetOrigin(), controlPoint.GetOrigin(), 0, 255, 0, true, 0.1 )
		#endif
		WaitFrame()
	}
}
...
void function MortarRingAirburst( entity player, entity projectile, int numBombs, float launchAngle, float launchSpeed, float radiusModMin = 1.0, float radiusModMax = 1.0 )
{
	...
	#if DEVELOPER && MORTAR_RING_MISSILE_DEBUG
		DebugDrawSphereRGB( projectileOrigin, 10, 0, 0, 255, true, 10.0 )
	#endif
...
void function LaunchBomblet( entity player, entity weapon, vector origin, vector target, float launchSpeed, array<entity> bombArray )
{
	...
	if( !as.valid )
	{
		DebugDrawSphereRGB( target, 10, 255, 0, 0, true, 10.0 )
	}
	else
	{
		DebugDrawSphereRGB( target, 10, 0, 0, 255, true, 10.0 )
	}
#endif
