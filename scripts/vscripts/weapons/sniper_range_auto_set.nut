	...
	while( true )
	{
		...
		else
		{
				...
				if ( SNIPER_RANGE_DEBUG_DRAW )
				{
					...
					DebugDrawLineRGB( player.EyePosition(), traceEndPos, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), false, 0.3 )
					DebugDrawSphereRGB( traceEndPos, 3, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), false, 0.3 )

					entity aaTarget = GetAimAssistCurrentTarget()
					if ( IsValid(aaTarget) )
					{
						DebugDrawSphereRGB( aaTarget.GetOrigin(), 3, int(COLOR_MAGENTA.x), int(COLOR_MAGENTA.y), int(COLOR_MAGENTA.z), false, 0.3 )
					}
