...
void function CreateSilenceField( entity player, vector origin, entity mover, vector normal )
{
	...
	if ( SILENCE_DEBUG )
	{
		DebugDrawSphereRGB( origin, SILENCE_AREA_RADIUS, 255, 0, 0, true, SILENCE_AREA_DURATION )
	}
	...
	while( true )
	{
		for ( int i=0; i<offsetVectors.len(); i++ )
		{
			...
			#if DEVELOPER
			if ( SILENCE_DEBUG )
				DebugDrawLineRGB( center, trace.endPos, 255, 0, 0, true, 0.1 )
			#endif
					...
					else
					{
						...
						//{
							if ( !IsValid( smokeFXs[i][0] ) )
							{
								...
								{
									fx = StartParticleEffectOnEntityWithPos_ReturnEntity( mover, index, FX_PATTACH_ABSORIGIN_FOLLOW_NOROTATE, ATTACHMENTID_INVALID, offsetVector + FX_SILENCE_SMOKE_OFFSET, <0,0,0> )
							...
							else
							{
								...
								if ( !IsValid( mover ) )
								{
									...
									if ( SILENCE_DEBUG )
									{
										DebugDrawSphereRGB( org, 64, 0, 255, 255, true, 0.1 )
									}
								}
							}
						//}
