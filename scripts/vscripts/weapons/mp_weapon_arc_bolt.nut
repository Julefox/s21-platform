...
void function Bolt_Thread( entity bolt, entity owner, float lifetime, float activationDelay, bool boltIsPlanted )
{
	...
	else
	{
		...
		entity plantedFx = StartParticleEffectOnEntity_ReturnEntity( bolt, GetParticleSystemIndex( ARC_BOLT_TETHER_RADIUS_FX ), FX_PATTACH_ABSORIGIN_FOLLOW, ATTACHMENTID_INVALID )
...
vector ornull function GetArcConnectPoint( entity bolt, vector dir, vector maxAngleDeviations )
{
	...
	if ( DEBUG_CONNECT_POINT )
	{
		...
		{
			DebugDrawSphereRGB( arcTrace.endPos, 10, 255, 0, 0, false, 5.0 )
		}
		else
		{
			DebugDrawSphereRGB( arcTrace.endPos, 10, 255, 255, 0, false, 5.0 )
		}
	}
...
void function CreateTether_Thread( entity attacker, entity victim, entity inflictor )
{
	...
	{
		entity radiusFxEnt = StartParticleEffectOnEntity_ReturnEntity( tetherEnt, GetParticleSystemIndex( ARC_BOLT_TETHER_RADIUS_FX ), FX_PATTACH_ABSORIGIN_FOLLOW, ATTACHMENTID_INVALID )
