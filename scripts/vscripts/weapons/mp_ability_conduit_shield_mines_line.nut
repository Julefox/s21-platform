...
void function DeployShieldMineLine( entity player, entity projectile )
{
	...
	StartParticleEffectInWorldForRealms( airdeployFXID, projectile.GetOrigin(), ZERO_VECTOR, projectile )
...
array<vector> function GenerateMineLocations( entity weapon, vector airBurstLocation, vector directionRight, bool DEBUG_DRAW, int projectileID = -1, bool simulateCollisions = true )//entity projectile )
{
	...
	for ( int i=startIndex; i<=endIndex; ++i )
	{
		...
		if ( DEBUG_DRAW )
		{
			DebugDrawSphereRGB( airBurstLocation, 6, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), false, DRAW_TIME )
			DebugDrawSphereRGB( idealMinePos, 6, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), false, DRAW_TIME )
			DebugDrawSphereRGB( minePos, 5, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), false, DRAW_TIME )
...
#if SERVER
void function OnProjectileCollision_ability_shield_mines_line( entity projectile, vector pos, vector normal, entity hitEnt, int hitBox, bool isCritical )
#else
...
void function ShieldMineLifetime_Thread( entity player, vector pos, float radius, entity parentTo = null )
{
	...
	while( Time() <= endTime )
	{
		...
		{
			DebugDrawSphereRGB( damagePos, 3, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.1 )
			DebugDrawSphereRGB( damagePos, radius, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), false, 0.1 )
		}
