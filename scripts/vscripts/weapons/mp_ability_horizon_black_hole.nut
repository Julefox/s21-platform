...
var function OnWeaponPrimaryAttack_weapon_black_hole( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	// S21: ThrowDeployable + BLACKHOLE_ProjectileLanded (not PlantSuperSticky / FireWeaponGrenade dual).
	...
	if ( deployable )
	{
		...
		PlayerUsedOffhand( player, weapon, true, deployable )
			...
			StartParticleEffectOnEntityWithPos_ReturnEntity( deployable, thrusterFXID, FX_PATTACH_POINT_FOLLOW, deployable.LookupAttachment( "thruster1" ), <0, 0, 0>, <90, 0, 0> )
			StartParticleEffectOnEntityWithPos_ReturnEntity( deployable, thrusterFXID, FX_PATTACH_POINT_FOLLOW, deployable.LookupAttachment( "thruster2" ), <0, 0, 0>, <90, 0, 0> )
			StartParticleEffectOnEntityWithPos_ReturnEntity( deployable, thrusterFXID, FX_PATTACH_POINT_FOLLOW, deployable.LookupAttachment( "thruster3" ), <0, 0, 0>, <90, 0, 0> )
		#endif
	}
...
entity function CreateNewtProp( entity owner, vector origin, vector angles, entity parentTo )
{
	...
	newtProp.SetIgnorePredictedTriggerTypes( TT_JUMP_PAD | TT_GRAVITY_LIFT | TT_BLACKHOLE ) // So the hole never drags its own prop
...
void function BLACKHOLE_PullTriggerThread( entity newtProp, entity pullSphereFX )
{
	...
	float blackholePullOuterSpeed = GetCurrentPlaylistVarFloat( "blackhole_pull_outer_speed", BLACKHOLE_TUNING_CODE_PULL_OUTER_SPEED )
	float blackholePullInnerSpeed = GetCurrentPlaylistVarFloat( "blackhole_pull_inner_speed", BLACKHOLE_TUNING_CODE_PULL_INNER_SPEED )
	float blackholeMoveOuterSpeed = GetCurrentPlaylistVarFloat( "blackhole_move_outer_speed", BLACKHOLE_TUNING_CODE_MOVE_OUTER_SPEED )
	float blackholeMoveInnerSpeed = GetCurrentPlaylistVarFloat( "blackhole_move_inner_speed", BLACKHOLE_TUNING_CODE_MOVE_INNER_SPEED )

	// The pull is applied by the engine for this trigger type, so the script only
	// authors the parameters and the presentation.
	entity trigger = CreateEntity( "trigger_cylinder_heavy" )
	trigger.SetTriggerType( TT_BLACKHOLE )
	trigger.SetBlackholeIsStrongPulling( true )
	trigger.SetBlackholeParams( 200, blackholePullOuterSpeed, blackholePullInnerSpeed, blackholeMoveOuterSpeed, blackholeMoveInnerSpeed, BLACKHOLE_TUNING_INNER_RADIUS )
	...
	// Strong pull is the opening yank only; the rest of the duration holds.
	trigger.SetBlackholeIsStrongPulling( false )
...
void function BLACKHOLE_InTriggerThread( entity trigger, entity ent )
{
	...
	if ( ent.IsPlayer() )
	{
		...
		// The pull on a player is the engine's, applied every command on both the
		// server and the client's own prediction. This loop only keeps the field
		// status effect alive for as long as the player is inside.
	...
	OnThreadEnd(
		function() : ( ent, player3PBlackholeFXArray, newtProp )
		{
			if ( IsValid( ent ) )
			{
				StatusEffect_StopAllOfType( ent, eStatusEffect.in_black_hole_field )
...
void function BLACKHOLE_ProjectileLanded( entity projectile, DeployableCollisionParams collisionParams )
{
		...
		vector endOrigin     = origin - collisionParams.normal * 32
		...
		thread BLACKHOLE_MainLifetimeThread( projOwner, origin, surfaceAngles, parentTo )
	#endif
}
