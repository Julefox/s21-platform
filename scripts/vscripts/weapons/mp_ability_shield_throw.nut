...
void function TrackProjetilePath( entity player, entity projectile )
{
	...
	if ( IsValid( player ) )
	{
		...
		if ( DEBUG_WALL_CHECK )
		{
			DebugDrawSphereRGB( player.EyePosition(), 8.0, int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), true, 3.0 )
		}
		#endif
	}
	...
	while ( true )
	{
		...
		if ( DEBUG_WALL_CHECK )
		{
			DebugDrawSphereRGB( projectile.GetOrigin(), 8.0, int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), true, 3.0 )
		}
		#endif
