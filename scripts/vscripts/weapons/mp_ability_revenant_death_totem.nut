...
void function MpAbilityRevenantDeathTotem_Init()
{
	...
	PrecacheScriptString( DEATH_TOTEM_TARGETNAME )
...
var function OnWeaponPrimaryAttack_ability_revenant_death_totem( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	thread DeathTotem_DisableWallClimbWhileDeployingTotem( ownerPlayer, weapon )
		...
		LockWeaponsAndMelee( ownerPlayer, DEATH_TOTEM_WEAPON_NAME )
		...
		thread DeathTotem_DeployTotem( ownerPlayer, origin, angles, parentTo, vehicleAttachmentIndex ) // Place your object in some utility function as normal using object placement's final positional data
...
void function DeathTotem_DisableWallClimbWhileDeployingTotem( entity ownerPlayer, entity weapon )
{
	...
	OnThreadEnd(
		function() : ( ownerPlayer, wallHangID, wallClimbID, doubleJumpID )
		{
			...
			if ( serverOrPredicted )
			{
				if ( StatusEffect_HasSeverity( ownerPlayer, eStatusEffect.disable_wall_run ) )
					StatusEffect_Stop( ownerPlayer, wallClimbID )

				if ( StatusEffect_HasSeverity( ownerPlayer, eStatusEffect.disable_double_jump ) )
...
#if SERVER
void function DeathTotem_DeployTotem( entity owner, vector origin, vector angles, entity parentTo, int vehicleAttachmentIndex )
{
	if ( !IsValid( owner ) )
		return
	...
	totemProxy.SetIgnorePredictedTriggerTypes( TT_JUMP_PAD | TT_GRAVITY_LIFT | TT_BLACKHOLE  ) // So it ignores jump pads placed underneath later
	...
	if ( parentTo != null )
	{
		mover = CreateScriptMover( DEATH_TOTEM_MOVER_SCRIPTNAME, origin, angles )
		mover.SetParent( parentTo )
		totemProxy.SetParent( mover )
		                     
		if( EntIsHoverVehicle( parentTo ) )
		{
			//parentTo.HoverVehicle_SetAbilityAttachmentEntity( vehicleAttachmentIndex, mover )
		}
	...
	AddWreckingBallEMPDamageDevice( totemProxy )
...
void function Totem_CheckForGeoIntersection( entity totemProxy )
{
	...
	while ( true )
	{
		...
		if ( results.startSolid )
		{
			if ( DEATH_TOTEM_DEBUG )
			{
				DebugDrawBoxRGB( startPos, <-8,-8,0>, <8,8,32>, 255, 100, 0, 1, 3.0 )
		...
		{
			if ( DEATH_TOTEM_DEBUG )
				DebugDrawBoxRGB( results.endPos, <-8,-8,0>, <8,8,32>, int( COLOR_GREEN.x ), int( COLOR_GREEN.y ), int( COLOR_GREEN.z ), 1, 0.3 )
		}
...
void function DeathTotem_RecallPlayer( entity player )
{
	...
	foreach( ent in player.GetChildren() )
	{
	...
	// if you're waiting to take off with Valk's ult, decouple you from the ult before trying to move you
	...
	ArmoredLeap_HandleInterruptedMidLeap( player )
	...
	if ( DEATH_TOTEM_DEBUG )
	{
		DebugDrawSphereRGB( playerStartLoc, 10, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), false, 5.0 )
		DebugDrawSphereRGB( totemOrigin, 5, 0, 255, 125, false, 5.0 )
		DebugDrawSphereRGB( recallOrigin, 10, int(COLOR_CYAN.x), int(COLOR_CYAN.y), int(COLOR_CYAN.z), false, 5.0 )
	}
	...
	if ( DEATH_TOTEM_DEBUG )
	{
		...
		DebugDrawSphereRGB( player.GetOrigin(), 20, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), false, 5.0 )
		DebugDrawLineRGB( playerStartLoc, player.GetOrigin(), 0, 255, 125, false, 5.0 )
...
{
	entity mover = CreateScriptMover( DEATH_TOTEM_MOVER_SCRIPTNAME + "__shadow_recallMover", startPosition )
...
void function DeathTotem_SetupDecoy( entity player, entity decoy )
{
	...
	// Movement is already disabled by the final CreatePlayerDecoy argument above.
...
void function DeathTotem_MarkLocation( entity player, entity totemProxy )
{
	...
	int statusEffectID = StatusEffect_AddTimed( player, eStatusEffect.death_totem_visual_effect, 1.0, file.deathTotemBuffDuration, 0.0 )
		...
		if ( DEATH_TOTEM_DEBUG )
		{
			DebugDrawSphereRGB( data.origin, 10, 255, 100, 0, false, DEATH_TOTEM_EFFECT_DURATION_DEFAULT )
...
DeathTotemPlacementInfo function CalculateDeathTotemPosition( entity weaponOwner, entity totemProxy )
{
	...
	//DebugDrawSphere( traceResults.endPos, 16.0, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 15.0, 2 )
	...
	if ( isUpwardSlope )
	{
		...
		//DebugDrawSphere( traceResults.endPos, 16.0, int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), true, 15.0, 2 )
	}
	...
	{
		//DebugDrawSphere( traceResultsDown.endPos, 16.0, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 15.0, 2 )
