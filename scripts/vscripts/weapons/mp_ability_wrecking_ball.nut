...
// CafeMod maggie_mitosis / big_balls
// Every ball wall-edge spawn (recursive). Live ball cap plus a magnet cap --
// each rolling ball drops a puck every 0.8s that lives 60s.
const int MAGGIE_MITOSIS_MAX_BALLS = 16
const int MAGGIE_MAX_MAGNETS = 48
const float MAGGIE_MITOSIS_COOLDOWN = 0.20
const float MAGGIE_MITOSIS_SCATTER_DEG = 28.0
const float BIG_BALLS_MODEL_SCALE = 5.0
...
struct
{
	...
	// CafeMod maggie_mitosis
	table<entity, bool> ballAllowMitosis
	table<entity, bool> ballWasTouchingWall
	table<entity, float> ballLastMitosisTime
...
void function MpAbilityWreckingBall_Init()
{
	...
	#if CLIENT
		PrecacheScriptString( WRECKING_BALL_ANIMCHILD_SCRIPT_NAME )
		PrecacheScriptString( WRECKING_BALL_BALL_SCRIPT_NAME )
		PrecacheScriptString( WRECKING_BALL_SPEEDTRIGGER_SCRIPT_NAME )
		PrecacheScriptString( WRECKING_BALL_MAGNET_SCRIPT_NAME )
	#endif
...
entity function WreckingBall_Launch( entity weapon, vector attackPos, vector throwVelocity, bool isPredicted, bool isLagCompensated )
{
	...
	if ( ball == null )
		...
		// CafeMod big_balls: scale in-flight grenade so size matches deployed ball
		if ( CafeMod_IsEnabled( "big_balls" ) )
			ball.SetModelScale( BIG_BALLS_MODEL_SCALE )
	#endif
...
entity function WreckingBall_CreateAnimatedChildModelForPhysicsObject( entity deployable, entity player )
{
	...
	// Visible flight model (projectile is Hidden); match big_balls deploy scale
	if ( CafeMod_IsEnabled( "big_balls" ) )
		animatedChild.SetModelScale( BIG_BALLS_MODEL_SCALE )
...
entity function WreckingBall_DestroyProjectileAndSpawnBall( entity projectile, vector dir )
{
	...
	{
		printt( "[CafeMod] wrecking ball CreateEntity failed" )
		return null
	}
	if ( !IsValid( ball ) )
		return null
	...
	#if SERVER
	if ( CafeMod_IsEnabled( "big_balls" ) )
		modelScale = BIG_BALLS_MODEL_SCALE
	#endif
	ball.SetModelScale( modelScale )
	ball.kv.massScale = modelScale
	...
	{
		AddToUltimateRealm( owner, ball )
		FiringRange_AddToRemoveOnCharacterChange( ball, owner )
	}

	ball.e.spawnTime = projectile.e.spawnTime
	#if SERVER
	file.ballAllowMitosis[ ball ] <- true
	file.ballWasTouchingWall[ ball ] <- false
	file.ballLastMitosisTime[ ball ] <- 0.0
	#endif

	// reparenting he animating child of the projectile to the new physics prop
	bool foundAnimChld = false
	foreach ( child in projectile.GetChildren() )
	{
		...
		if ( modelScale != 1.0 )
			child.SetModelScale( modelScale )

		FiringRange_AddToRemoveOnCharacterChange( child, owner )
	}
...
void function WreckingBall_GetTheBallRolling( entity ball, vector dir )
{
	...
	if ( !( ball in file.ballWasTouchingWall ) )
		file.ballWasTouchingWall[ball] <- false
	vector angles 				= VectorToAngles( dir )
	...
	radiusFxEnt = StartParticleEffectOnEntity_ReturnEntity( ball, GetParticleSystemIndex( WRECKING_BALL_RADIUS_FX ), FX_PATTACH_ABSORIGIN_FOLLOW, ATTACHMENTID_INVALID )
	...
	OnThreadEnd(
		function() : ( ball, owner, radiusFxEnt )
		{
			...
			if ( ball in file.ballAllowMitosis )
				delete file.ballAllowMitosis[ball]

			if ( ball in file.ballWasTouchingWall )
				delete file.ballWasTouchingWall[ball]

			if ( ball in file.ballLastMitosisTime )
				delete file.ballLastMitosisTime[ball]
	...
	while ( IsValid( ball ) && ( Time() < endRollTime ) )
	{
		...
		bool hittingWall = ( wallTrace.fraction < 1.0 )
		bool wasWall = ( ball in file.ballWasTouchingWall ) && file.ballWasTouchingWall[ ball ]
		if ( hittingWall )
		{
			vector normal = wallTrace.surfaceNormal
			dir = dir - 2.0 * DotProduct( dir, normal ) * normal
			dir = Normalize( <dir.x, dir.y, 0> )
			// Rising edge only = one spawn per real wall contact (not every stuck frame)
			if ( !wasWall )
				WreckingBall_TryMitosisOnBounce( ball, wallTrace.endPos, dir )
		}
		file.ballWasTouchingWall[ ball ] <- hittingWall
		...
		{
			if ( CafeMod_CanSpawnEntities( 2 ) )
			{
				entity fullburn = null
				try
				{
					fullburn = StartParticleEffectInWorld_ReturnEntity( file.fireFxId, ball.GetOrigin(), <0,0,0> )
				}
				catch ( e )
				{
					fullburn = null
		...
		foreach ( device in destroyDevices )
		{
			...
			if ( DEBUG_EMP_DAMAGE_DESTRUCTION )
			{
				...
				DebugDrawLineRGB( device.GetOrigin(), deviceProjectedPosition, int(debugLineColor.x), int(debugLineColor.y), int(debugLineColor.z), true, 0.1 )
		...
		if ( ( Time() >= endImpactDelayTime ) )
		{
			...
			if ( CafeMod_CanSpawnEntities( 8 ) && WreckingBall_CountActiveMagnets() < MAGGIE_MAX_MAGNETS )
...
void function WreckingBall_CreateFinalExplosion( entity ball, bool isFirstHit )
{
	...
	if( !isFirstHit )
	{
		if ( CafeMod_CanSpawnEntities( 16 ) && WreckingBall_CountActiveMagnets() < MAGGIE_MAX_MAGNETS )
		{
...
void function WreckingBall_CreateSpeedTrigger( entity owner, vector position, vector angles, entity hostEnt )
{
	...
	{
		printt( "[CafeMod] wrecking ball trigger_cylinder create failed" )
		return
	}
	if ( !IsValid( trigger ) )
		return
...
void function WreckingBall_SpawnRepulsorPropFromEntity( entity ent, vector throwDir, bool debug = false )
{
	...
	if ( !CafeMod_CanSpawnEntities( 8 ) )
		return
	if ( WreckingBall_CountActiveMagnets() >= MAGGIE_MAX_MAGNETS )
		return

	entity repulsorProp = null
	try
	{
		repulsorProp = CreatePropPhysics( WRECKING_BALL_PIECE_MODEL, ent.GetCenter() + throwOffset, <0,0,0>, SF_PHYSPROP_DEBRIS )
	}
	catch ( e )
	{
		printt( "[CafeMod] wrecking ball magnet CreatePropPhysics failed" )
		return
	}
	...
	*/
	...
	if ( debug )
	{
		...
		DebugDrawSphereRGB( ent.GetCenter(), 15.0, 255, 0, 0, false, 10.0 )
		DebugDrawSphereRGB( startPos, 5.0, 0, 150, 255, false, 10.0 )
...
void function WreckingBall_RepulsorProp_AirThink( entity repulsorProp )
{
	...
	if ( CafeMod_CanSpawnEntities( 2 ) )
	{
		try
		{
			flyFX = StartParticleEffectOnEntity_ReturnEntity( repulsorProp, GetParticleSystemIndex( WRECKING_BALL_PIECE_FLYING_FX ),
...
void function WreckingBall_RepulsorProp_GroundThink( entity ball, entity repulsorProp )
{
	...
	if ( IsValid( ball ) )
	{
		...
		if ( file.isBallOnFire[ ball ] && CafeMod_CanSpawnEntities( 2 ) ) // upgrade_maggie_thermite_explosion
		{
			try
			{
	...
	entity groundFx = null
	if ( CafeMod_CanSpawnEntities( 2 ) )
	{
		try
		{
			groundFx = StartParticleEffectOnEntity_ReturnEntity( repulsorProp, GetParticleSystemIndex( WRECKING_BALL_PIECE_LOOP_FX ),
				FX_PATTACH_POINT_FOLLOW, repulsorProp.LookupAttachment( "ORIGIN" ) )
		}
		catch ( e )
		{
...
#if SERVER
int function WreckingBall_CountActiveMagnets()
{
	return CafeMod_CountByScriptName( WRECKING_BALL_MAGNET_SCRIPT_NAME )
}

int function WreckingBall_CountActiveBalls()
{
	array<entity> balls = GetEntArrayByScriptName( WRECKING_BALL_BALL_SCRIPT_NAME )
	int count = 0
...
// Every roller wall-edge spawns a child (recursive). Hard live cap only.
void function WreckingBall_TryMitosisOnBounce( entity parentBall, vector pos, vector bounceDir = <0, 0, 0> )
{
	if ( !CafeMod_IsEnabled( "maggie_mitosis" ) )
		return

	if ( !IsValid( parentBall ) )
		return

	if ( parentBall.GetScriptName() != WRECKING_BALL_BALL_SCRIPT_NAME )
		return

	float now = Time()
	float last = 0.0
	if ( parentBall in file.ballLastMitosisTime )
		last = file.ballLastMitosisTime[ parentBall ]
	if ( ( now - last ) < MAGGIE_MITOSIS_COOLDOWN )
		return

	int live = WreckingBall_CountActiveBalls()
	if ( live >= MAGGIE_MITOSIS_MAX_BALLS )
		return

	if ( !CafeMod_CanSpawnEntities( 8 ) )
		return

	entity owner = parentBall.GetOwner()
	if ( !IsValid( owner ) )
		return

	file.ballLastMitosisTime[ parentBall ] = now

	vector flat = Normalize( < bounceDir.x, bounceDir.y, 0.0 > )
	if ( Length( flat ) < 0.01 )
	{
		vector vel = parentBall.GetVelocity()
		flat = Normalize( < vel.x, vel.y, 0.0 > )
	}
	if ( Length( flat ) < 0.01 )
		flat = <1, 0, 0>

	float angle = RandomFloatRange( -MAGGIE_MITOSIS_SCATTER_DEG, MAGGIE_MITOSIS_SCATTER_DEG )
	vector dir = VectorRotateAxis( flat, <0, 0, 1>, angle )
	dir = Normalize( < dir.x, dir.y, 0.0 > )
	...
	{
		printt( "[CafeMod] maggie_mitosis CreateEntity failed" )
		return
	}
	if ( !IsValid( ball ) )
		return
	ball.SetOwner( owner )
	ball.SetModel( WRECKING_BALL_PROJ_WRECKING_BALL )
	ball.SetOrigin( pos + <0, 0, 28> + ( dir * 56 ) )
	ball.SetAngles( VectorToAngles( dir ) )
	ball.SetModelScale( modelScale )
	ball.kv.massScale = modelScale
	ball.kv.inertiaScale = 1.0
	ball.kv.gravity = 1
	...
	AddToUltimateRealm( owner, ball )
	FiringRange_AddToRemoveOnCharacterChange( ball, owner )
	ball.e.spawnTime = Time()

	// Recursive: kids also spawn on their wall edges (until live cap)
	file.ballAllowMitosis[ ball ] <- true
	file.ballWasTouchingWall[ ball ] <- false
	file.ballLastMitosisTime[ ball ] <- 0.0

	printt( format( "[CafeMod] maggie_mitosis spawn live=%d/%d", live + 1, MAGGIE_MITOSIS_MAX_BALLS ) )
	thread WreckingBall_GetTheBallRolling( ball, dir )
}
...
void function CodeCallback_WreckingBallImpact( entity ball, int impactType, vector pos, entity hitEnt )
{
	...
	// Keep impact path if engine ever calls it; wall mitoses primarily from roll-loop TraceHull
	WreckingBall_TryMitosisOnBounce( ball, pos )
}
