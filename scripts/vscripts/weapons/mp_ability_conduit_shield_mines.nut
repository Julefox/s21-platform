...
void function OnWeaponActivate_ability_shield_mines( entity weapon )
{
	...
	#if SERVER
	entity bombardmentWeapon = VerifyBombardmentWeapon( weaponOwner, SHIELD_MINE_BOMBARDMENT_WEAPON )
	#endif
...
#if SERVER
void function OnProjectileCollision_ability_shield_mines( entity projectile, vector pos, vector normal, entity hitEnt, int hitBox, bool isCritical )
#else
...
void function WeaponArcPreviewThread_Client( entity owner, entity weapon )
{
	...
	while( true )
	{
		...
		if ( SHIELD_MINES_AIRBURST_DEBUG == eDebugStage.INITIAL_TARGET_AIR_POS )
		{
			...
			DebugDrawLineColor( impactPos, finalTargetPos, COLOR_RED, false, 0.1 )
		...
		if ( SHIELD_MINES_POSE_PARAM_DEBUG )
		{
			...
			DebugDrawLineColor( start, start + (fireUp * 20), COLOR_YELLOW, false, 0.1 )
			DebugDrawLineColor( start, start + (FlattenNormalizeVec( owner.GetViewForward() ) * 20), COLOR_RED, false, 0.1 )
			DebugDrawLineColor( start, start + (UP_VECTOR * 20), COLOR_RED, false, 0.1 )
...
vector function GetFinalImpactPos( entity weapon, entity player, bool DEBUG_DRAW = false )
{
	...
	if ( DEBUG_DRAW )
	{
		DebugDrawLineColor( impactPos, trImpact.endPos, trImpact.fraction < 1.0 ? COLOR_RED : COLOR_GREEN, false, 0.1 )
