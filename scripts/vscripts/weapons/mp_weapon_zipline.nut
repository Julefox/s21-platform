...
void function CreateGunZipline( entity weapon, vector startPos, vector endPos, vector startBasePos, vector endBasePos, entity startModel, entity endModel )
{
	...
	{
		owner.Zipline_SetBeginStation( null, 0 ) // S3 dedi native clears begin-station only on index 0; -1/ATTACHMENTID_INVALID is rejected ("Invalid attachment index given")
	}
							...
							if ( IsValid( weapon.w.ziplineStartPos ) )
							{
								...
								if ( spotsOrNull )
								{
									...
									if ( DEBUG_ZIPLINE_AUDIO_LINE )
										DebugDrawLineColor( spots.beginZiplineOrigin, spots.endZiplineOrigin, COLOR_MAGENTA, true, 10.0 )
									#endif
								}
							}
						}
