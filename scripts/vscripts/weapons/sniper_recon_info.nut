	...
	foreach ( target in potentialTargets )
	{
		...
		if ( distanceToTargetSqr > MAX_RANGE_SQR )
		{
			if ( SNIPER_RECON_DEBUG )
			{
				...
				DebugDrawSphereRGB( target.GetWorldSpaceCenter(), 3, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), true, 0.1 )
			}
			continue
		}
		...
		if ( !targetInCone )
		{
			if ( SNIPER_RECON_DEBUG )
			{
				DebugDrawSphereRGB( target.GetWorldSpaceCenter(), 3, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.1 )
				//DebugDrawText( target.GetWorldSpaceCenter(), "center", true, 0.1 )
				DebugDrawSphereRGB( target.EyePosition(), 3, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.1 )
				//DebugDrawText( target.EyePosition(), "eye", true, 0.1 )
				DebugDrawSphereRGB( target.GetOrigin(), 3, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.1 )
		...
		if ( targetInCone )
		{
			if ( SNIPER_RECON_DEBUG )
			{
				...
				DebugDrawSphereRGB( posToDraw, 3, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), true, 0.1 )
	...
	if ( arrayToPopulate.len() > 0 )
	{
		...
		foreach( targetData in arrayToPopulate )
		{
			...
			if ( trace.fraction >= 1.0  )
			{
				...
				if ( SNIPER_RECON_DEBUG )
				{
					DebugDrawSphereRGB( trace.endPos, 5, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 0.1 )
				}
			}
			else if ( SNIPER_RECON_DEBUG )
			{
				DebugDrawSphereRGB( targetData.target.GetWorldSpaceCenter(), 5, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.1 )
				DebugDrawSphereRGB( trace.endPos, 3, int(COLOR_PINK.x), int(COLOR_PINK.y), int(COLOR_PINK.z), true, 0.1 )
			}
			...
			if ( bestTarget == null ) //If I cant see then check the origin
			{
				...
				{
					DebugDrawSphereRGB( targetData.target.GetOrigin(), 5, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.1 )
					DebugDrawSphereRGB( trace.endPos, 3, int(COLOR_PINK.x), int(COLOR_PINK.y), int(COLOR_PINK.z), true, 0.1 )
				}
			}
			...
			if ( bestTarget == null && !skipEyeTest ) //If I cant see then check the eyes
			{
				...
				{
					DebugDrawSphereRGB( targetData.target.EyePosition(), 5, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.1 )
					DebugDrawSphereRGB( trace.endPos, 3, int(COLOR_PINK.x), int(COLOR_PINK.y), int(COLOR_PINK.z), true, 0.1 )
				}
			}
...
void function CL_SniperRecon_UI_Thread( entity owner )
{
	...
	)
	{
		...
		if ( SNIPER_RECON_DEBUG )
		{
			if ( IsValid(bestScopeTarget) )
			{
				DebugDrawSphereRGB( bestScopeTarget.GetWorldSpaceCenter(), 10, 0, 100, 255, true, 0.1 )


				entity aaTarget = GetAimAssistCurrentTarget()
				if ( IsValid(aaTarget) && aaTarget != bestScopeTarget )
				{
					DebugDrawSphereRGB( aaTarget.GetWorldSpaceCenter(), 15, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.1 )
				}
			}
		}
