...
void function OnWeaponActivate_weapon_tesla_trap( entity weapon )
{
	...
	Assert( ownerPlayer.IsPlayer() )
		...
		// Client re-tracks focal on deactivate; server must re-lock too or multi-place starts a new chain.
		// Shared placement proxy -- do not Destroy (TeslaTrap_CreateTrapPlacementProxy reuses it).
...
// ObjectPlacement may return any hit entity as parent. Stock placement only
// parents poles to these movers -- other parents break CanLink after the first
// pole (second placement must share the same parent or both unparented).
entity function TeslaTrap_SanitizePlacementParent( entity parentTo )
{
	if ( !IsValid( parentTo ) )
		return null

	// GetNetworkedClassName is string ornull on this engine.
	if ( parentTo.GetNetworkedClassName() != "func_brush" && parentTo.GetNetworkedClassName() != "script_mover" && parentTo.GetNetworkedClassName() != "func_brush_lightweight" )
		return null
...
TeslaTrapPlacementInfo function TeslaTrap_GetObjectPlacementInfo( entity player, entity weapon )
{
	...
	placementInfo.parentTo = TeslaTrap_SanitizePlacementParent( weapon.GetObjectPlacementParent() )
...
TeslaTrapPlacementInfo function TeslaTrap_GetPlacementInfoFromTraceResults_New( entity player, entity proxy, entity weapon )
{
	...
	entity placementParent = TeslaTrap_SanitizePlacementParent( weapon.GetObjectPlacementParent() )
...
TeslaTrapPlacementInfo function TeslaTrap_GetPlacementInfo( entity player, entity proxy, bool ignorePlacedTraps = false, int maxFallbacks = 3 )
{
	...
	{
		DebugDrawBoxRGB( fwdResults.endPos, TESLA_TRAP_BOUND_MINS, TESLA_TRAP_BOUND_MAXS, int( COLOR_GREEN.x ), int( COLOR_GREEN.y ), int( COLOR_GREEN.z ), 1, 0.1 ) //Forward Hull Cast Bounding Box
		DebugDrawBoxRGB( downResults.endPos, TESLA_TRAP_BOUND_MINS, TESLA_TRAP_BOUND_MAXS, int( COLOR_BLUE.x ), int( COLOR_BLUE.y ), int( COLOR_BLUE.z ), 1, 0.1 ) //Downward Hull Cast Bounding Box
		DebugDrawLineRGB( eyePos + viewVec * min( TESLA_TRAP_PLACEMENT_RANGE_MIN, maxRange ), fwdResults.endPos, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 0.1 ) //Forward Hull Cast
		DebugDrawLineRGB( fwdResults.endPos, eyePos + viewVec * maxRange, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.1 ) //Forward Hull Cast Blocked
		DebugDrawLineRGB( fwdResults.endPos, downResults.endPos, int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), true, 0.1 ) //Downward Hull Cast
		DebugDrawBoxRGB( upResults.endPos, TESLA_TRAP_BOUND_MINS, TESLA_TRAP_BOUND_MAXS, int( COLOR_CYAN.x ), int( COLOR_CYAN.y ), int( COLOR_CYAN.z ), 1, 0.1 ) //"Upward"" Hull Cast Bounding Box
		DebugDrawLineRGB( upStart, upResults.endPos, int(COLOR_CYAN.x), int(COLOR_CYAN.y), int(COLOR_CYAN.z), true, 0.1 ) //"Upward" Hull Cast
		DebugDrawLineRGB( roofTraceStart, roofTraceEnd, int(COLOR_MAGENTA.x), int(COLOR_MAGENTA.y), int(COLOR_MAGENTA.z), true, 0.1 ) //Roof Check
		DebugDrawLineRGB( player.GetOrigin(), player.GetOrigin() + (AnglesToForward( angles ) * file.balance_teslaTrapRange), int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 0.1 ) //Max Placement Dist
		DebugDrawLineRGB( eyePos + <0, 0, 8>, eyePos + <0, 0, 8> + (viewVec * file.balance_teslaTrapRange), int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 0.1 ) //Max Placement Dist

		DebugDrawLineRGB( eyePos + <0, 0, 4>, viewTraceResults.endPos + <0, 0, 4>, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 0.1 ) //Max Placement Dist Adjusted
	}
	...
	while ( !placementInfo.success && attempts < maxFallbacks )
	{
		...
		if ( TESLA_TRAP_DEBUG_DRAW_PRE_PLACEMENT )
		{
			DebugDrawBoxRGB( downFallbackResults.endPos, TESLA_TRAP_BOUND_MINS, TESLA_TRAP_BOUND_MAXS, int( COLOR_RED.x ), int( COLOR_RED.y ), int( COLOR_RED.z ), 1, 0.1 ) //Downward Fallback Hull Cast Bounding Box
		}
...
TeslaTrapPlacementInfo function TeslaTrap_GetPlacementInfoFromTraceResults( entity player, entity proxy, TraceResults hullTraceResults, TraceResults ornull upTraceResults, TraceResults viewTraceResults, array<entity> ignoreEnts, vector idealPos )
{
	...
	if ( success && hullTraceResults.fraction < 1.0 )
	{
		...
		foreach ( int i, vector testOffset in groundTestOffsets )
		{
			...
			{
				DebugDrawLineRGB( testPos, traceResult.endPos, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.05 )
			}
...
bool function TelsaTrap_AttemptSnapToNeighbor( entity player, vector origin, TeslaTrapPlacementInfo placementInfo )
{
	...
	foreach ( entity trap in traps )
	{
		...
		{
			DebugDrawSphereRGB( focalTrap.GetOrigin(), 15, int(COLOR_PINK.x), int(COLOR_PINK.y), int(COLOR_PINK.z), false, 0.1 )
			DebugDrawText( trap.GetOrigin()+<0,0,8>, VM_NAME()+": link trap ", false, 0.1 )
			DebugDrawSphereRGB( trap.GetOrigin(), 15, int(COLOR_PURPLE.x), int(COLOR_PURPLE.y), int(COLOR_PURPLE.z), false, 0.1 )
		}
		if ( distSqr <= closestDist )
		{
			closestTrap = trap
...
void function TeslaTrap_SnapClientOnlyModel( entity player, entity weapon, entity model )
{
	...
	{
		TeslaTrap_ClientSyncFocalFromNetvar( player )
		...
		if ( hasFocalTrap )
		{
			...
			if ( TESLA_TRAP_DEBUG_DRAW_CLIENT_TRAP_LINKING )
				DebugDrawLineRGB( model.GetOrigin(), focalTrap.GetOrigin(), int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 0.1 )
...
void function TeslaTrap_PlacementProxy( entity weapon, entity player, asset model )
{
	...
	while ( IsValid( weapon ) )
	{
		...
		if ( hasFocalTrap )
		{
			...
			if ( TESLA_TRAP_DEBUG_DRAW_CLIENT_TRAP_LINKING )
			{
				DebugDrawLineRGB( proxy.GetOrigin(), focalTrap.GetOrigin(), int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 0.1 )
			}
...
void function TeslaTrap_LinkTraps( int mainTrapID, int linkTrapID )
{
	...
	{
		DebugDrawLineRGB( mainData.trap.GetOrigin(), otherData.trap.GetOrigin(), int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 30.0 )
	}
...
bool function TeslaTrap_IsFlushWithGround( TeslaTrapData mainTrapData, TeslaTrapData otherTrapData )
{
	...
	for ( int i = 1; i < (groundCheckDensity + 1); i++ )
	{
		...
		{
			DebugDrawLineRGB( results.endPos, endOffset, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 1.0 )
			DebugDrawLineRGB( startOffset, results.endPos, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 1.0 )
		}
...
entity function TeslaTrap_CreateLinkTriggerCylinder( TeslaTrapData mainTrapData, TeslaTrapData otherTrapData, int fxCount, array<entity> damageFXs, array<entity> controlPoints )
{
	...
	if ( TESLA_TRAP_DEBUG_DRAW )
	{
		DebugDrawLineRGB( mainOrigin, mainOrigin - (offsetDir * TESLA_TRAP_LINK_TRIGGER_RADIUS), int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), true, 20.0 )
	}
	...
	if ( TESLA_TRAP_DEBUG_DRAW )
	{
		 DebugDrawCylinderRGB( trigOrigin, AnglesCompose( trigger.GetAngles(), < 90, 0, 0 > ), trigger.GetRadius(), trigger.GetAboveHeight(), int( COLOR_RED.x ), int( COLOR_RED.y ), int( COLOR_RED.z ), true, 60.0 )
		 DebugDrawCylinderRGB( trigOrigin, AnglesCompose( trigger.GetAngles(), < 90, 0, 0 > ), trigger.GetRadius(), -trigger.GetBelowHeight(), int( COLOR_RED.x ), int( COLOR_RED.y ), int( COLOR_RED.z ), true, 60.0 )
	}
...
void function TeslaTrap_CheckForRealTimeGeoObstructions( TeslaTrapData mainTrapData, TeslaTrapData otherTrapData, entity trigger )
{
	...
	while ( true )
	{
		...
		for ( int i = 1; i <= count; i++ )
		{
			...
			if ( TESLA_TRAP_DEBUG_DRAW )
			{
				DebugDrawLineRGB( results.endPos, endOffset, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 20.0 )
				DebugDrawLineRGB( startOffset, results.endPos, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 20.0 )
			}
			...
			if ( results.fraction < 1.0 )
			{
				...
				if ( IsValid( hitEnt ) )
				{
					...
					else if ( (hitEnt.GetClassName() == "prop_physics") && (trigger.GetObstructedEndTime() < Time()) && (hitEnt.GetScriptName() == LOOT_ROLLER_SCRIPTNAME) )
						hitEnt.TakeDamage( hitEnt.GetMaxHealth(), mainTrapData.trap.GetOwner(), mainTrapData.trap, { origin = hitEnt.GetOrigin(), scriptType = DF_EXPLOSION } )
...
void function TeslaTrap_CheckForPostIntersection( TeslaTrapData trapData )
{
	...
	while ( true )
	{
		...
		if ( heightPose > 0 )
		{
			...
			{
				DebugDrawLineRGB( results.endPos, endPos, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 20.0 )
				DebugDrawLineRGB( startPos, results.endPos, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 20.0 )
			}
...
#if CLIENT
// Server writes focal via SetPlayerNetEnt; stock expects RegisterNetVarEntityChangeCallback
// to update the local table. While the placer is out, TrackFocalTrap is stopped, so if the
// entity change callback does not fire we still need the server value for link UI.
void function TeslaTrap_ClientSyncFocalFromNetvar( entity player )
{
	entity netFocal = player.GetPlayerNetEnt( "focalTrap" )
	if ( IsValid( netFocal ) && netFocal.GetScriptName() == "tesla_trap" )
	{
		if ( TeslaTrap_GetFocalTrapForPlayer( player ) != netFocal )
			TeslaTrap_SetFocalTrapForPlayer( player, netFocal )
	}
	else if ( TeslaTrap_PlayerHasFocalTrap( player ) )
	{
		TeslaTrap_ClearFocalTrapForPlayer( player )
	}
}
...
vector function TeslaTrap_GetPointOnRectangularPlane( TeslaTrapData mainTrapData, TeslaTrapData otherTrapData, vector testPoint )
{
	...
	if ( TESLA_TRAP_DEBUG_DRAW )
	{
		DebugDrawLineRGB( triAPointA, triAPointB, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 20.0 )
		DebugDrawLineRGB( triAPointB, triAPointC, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 20.0 )
		DebugDrawLineRGB( triAPointC, triAPointA, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 20.0 )

		DebugDrawLineRGB( triBPointA, triBPointB, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 20.0 )
		DebugDrawLineRGB( triBPointB, triBPointC, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 20.0 )
		DebugDrawLineRGB( triBPointC, triBPointA, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 20.0 )
	}
...
void function CodeCallback_TeslaTrapCrossed( entity trigger, entity start, entity end, entity crossingEnt )
{
		...
		// HoverVehicle_StunBegin is not registered on the S3 dedi entity VM.
		// Client half still stuns for local vehicles when the callback fires there.
		...
		if ( !TrippedEntIsFriendly( crossingEnt, start ) )
		{
			// HoverVehicle_StunBegin not bound on S3 client entity methods either in all paths.
			// Leave no-op until the vehicle stun native is registered.
...
int function TeslaTrap_GetLinkLOSBeamCount( vector mainOrigin, vector mainUp, vector otherOrigin, vector otherUp, entity mainTrap, entity otherTrap )
{
	...
	for ( int i = 1; i <= TESLA_TRAP_LINK_FX_COUNT; i++ )
	{
		...
		if ( TESLA_TRAP_DEBUG_DRAW_LINKS )
		{
			DebugDrawLineRGB( results.endPos, endOffset, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.1 )
			DebugDrawLineRGB( startOffset, results.endPos, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 0.1 )
		}
