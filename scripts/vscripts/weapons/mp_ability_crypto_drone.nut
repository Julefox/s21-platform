...
global function ClientCallback_AttemptDroneRecall
...
#else
const asset CRYPTO_DRONE_SIGHTBEAM_FX = $"P_BT_scan_SML"
#endif
...
#if SERVER
void function ClientCallback_AttemptDroneRecall( entity player )
{
	if ( !IsAlive( player ) )
		return
	...
	if ( StatusEffect_HasSeverity( player, eStatusEffect.crypto_camera_is_emp ) )
		return
...
bool function Crypto_TryDestroyDroneProjectile( entity cameraVehicle, bool useFacingOverride = false, vector facingOverride = < 0, 0, 0 > )
{
	...
	if ( IsValid( projectileParent ) )
	{
		...
		//R5DEV-354790 - Need to catch if the drone has been thrown underwater. PutPlayerVehicleInSafeSpot will move around, but not up above the water if we're already under water from the drone grenade being thrown under there. Need High Detail trace to hit water.
		TraceResults tr = TraceHullHighDetail( cameraVehicle.GetOrigin() + <0,0,20>, cameraVehicle.GetOrigin() - <0,0,20>, CRYPTO_DRONE_HULL_TRACE_MIN, CRYPTO_DRONE_HULL_TRACE_MAX, [ cameraVehicle ], TRACE_MASK_NPCSOLID | CONTENTS_WATER | CONTENTS_ABILITYCLIP, TRACE_COLLISION_GROUP_PLAYER )
		...
		if ( AutoReloadWhileInCryptoDroneCameraView() )
		{
			...
			if ( IsValid( currentlyReloadingWeapon ) )
			{
				if ( Time() > reloadEndTime )
				{
					...
					if ( GetInfiniteAmmo( currentlyReloadingWeapon ) )
					{
...
void function Camera_OnBeginView_Think( entity player, int statusEffect, bool actuallyChanged )
{
	...
	ChangeHUDVisibilityWhenInCryptoDrone( true )
		...
		file.fakePlayerMarkerRui = Minimap_AddIconAtPosition( player.GetOrigin(), player.GetAngles(), $"rui/hud/minimap/compass_icon_player", 1.0, GetPlayerInfoColor( player ) / 255.0 )
		Minimap_StartTrackVehicleData( activeCamera )
...
void function NeurolinkThink( entity camera, bool attachFx = true )
{
	...
	while( true )
	{
		...
		//S21 implementation uses VehicleGetPlayersInViewArray and VehicleGetNpcsInViewArray code functs that we don't have in s3.
