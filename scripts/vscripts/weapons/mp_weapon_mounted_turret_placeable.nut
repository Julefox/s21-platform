...
void function MpWeaponMountedTurretPlaceable_Init()
{
	...
	// The turret ent is CreateScriptMoverModel, whose native classname is
	// script_mover_lightweight; the typed_entity check matches native classnames.
	Remote_RegisterServerFunction( "ClientCallback_TryPickupMountedTurret", "typed_entity", "script_mover_lightweight" )
...
{
	if ( player.IsPlayer() && IsValid( player.GetTurret() ) )
...
void function MountedTurretPlaceable_UseGun( entity player, entity turretProxy )
{
	...
	entity driver = turretProxy.GetDriver()
...
{
	if ( IsValid_ThisFrame( player ) && player.IsPlayer() && IsValid( player.GetTurret() ) )
	{
		MountedTurretPlaceable_ClearDriver_ForOtherReason( player.GetTurret() )
	}
}
...
void function ClientCallback_TryPickupMountedTurret( entity player, entity device )
{
	...
	thread (void function() : ( player, device ) {
		...
		if ( IsValid( player) && GetPlayerVoice( player ) == RAMPART_VOICE )
...
MountedTurretPlaceablePlacementInfo function MountedTurretPlaceable_GetPlacementInfo( entity player, entity turretModel )
{
	...
	{
		DebugDrawLineRGB( fwdStart, fwdResults.endPos, 255, 0, 0, true, 0.05 )
		DebugDrawLineRGB( fwdStart, fwdResults.endPos, 255, 0, 0, true, 0.05 )
		DebugDrawSphereRGB( fwdResults.endPos, 16, 255, 0, 0, true, 0.05 )
		DebugDrawLineRGB( fwdResults.endPos, fwdResults.endPos - MOUNTED_TURRET_PLACEABLE_PLACEMENT_TRACE_OFFSET, 255, 0, 0, true, 0.05 )
	}
	...
	if ( success && downResults.fraction < 1.0 )
	{
		...
		foreach ( vector testOffset in groundTestOffsets )
		{
			...
			if ( MOUNTED_TURRET_PLACEABLE_DEBUG_DRAW_PLACEMENT )
			{
				DebugDrawLineRGB( testPos, traceResult.endPos, 255, 0, 0, true, 0.05 )
			}
