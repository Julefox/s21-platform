...
// Per-player recall state lives on the harvester class. The Nexus root spawns
// from the native dynamic-prop factory here, so the client builds a
// C_DynamicProp and any use-state call throws at runtime.
//
// This cannot be detected from script: GetNetworkedClassName() reports whatever
// classname CreateEntity was given ("prop_material_harvester"), not the class
// the client actually built, so a classname guard passes and still throws. Flip
// this const when the harvester class is genuinely networked.
global const bool TRANSPORT_PORTAL_HAS_USE_STATE = false
...
bool function IsPlacementPositionValid( vector position, entity player, entity projectile )
{
	...
	if ( tr.fraction == 1.0 )
	{
		Warning( "TransportPortal: placement rejected, no floor at " + position )
		return false
	}

	array<int> realms = projectile.GetRealms()
	foreach ( entity trigger in GetTriggersByClassesInRealms_HullSize(
		file.disallowedTriggerTypes,
		position, position,
		realms, TRACE_MASK_PLAYERSOLID,
		player.GetPlayerMins(), player.GetPlayerMaxs() ) )
	{
		Warning( "TransportPortal: placement rejected by " + trigger )
		return false
	}
	...
	foreach ( entity trigger in GetTriggersByClassesInRealms(
		...
		realms, TRACE_MASK_PLAYERSOLID ) )
	{
		int damageSourceId = (trigger.HasKey( "damageSourceId" ) ? int(trigger.kv.damageSourceId) : eDamageSourceId.damagedef_unknown)
		if ( damageSourceId in fallTriggerDamageSourceIdSet )
		{
			Warning( "TransportPortal: placement rejected by fall trigger_hurt " + trigger )
			return false
		}
	}
...
void function TransportPortal_Deploy( entity owner, vector origin, vector angles, entity parentTo )
{
	...
	if ( !IsValid( rootEnt ) )
	{
		Warning( "TransportPortal: CreateEntity(prop_material_harvester) returned null" )
		return
	}
	...
	Warning( "TransportPortal: deployed root=" + rootEnt + " at " + origin )
	...
	receiver.AddUsableValue( USABLE_BY_TEAMMATES | USABLE_CUSTOM_HINTS | USABLE_EXTENDED_USE | USABLE_FROM_EXTENDED_RANGE | USABLE_NO_LOS_REQUIREMENT )
...
void function OnBleedoutStarted( entity victim, float endTime )
{
	...
	foreach ( entity receiver in file.teamToReceiversMap[team] )
	{
		...
		if ( TransportPortal_GetRecallUsed( rootEnt, victim.GetPlayerIndex() ) )
			continue
	...
	while ( true )
	{
		...
		for ( int index = 0 ; index < playerSquad.len() ; index++ )
		{
			...
			if ( index < playerSquad.len() && index > 0)
			{
				...
				RuiSetBool( iconRui, "teammateHasUsed" + index, TransportPortal_GetRecallUsed( rootEnt, squadMate.GetPlayerIndex() ) )

				if (TransportPortal_GetRecallUsed( rootEnt, squadMate.GetPlayerIndex() ))
					squadUses++
			}
		}
...
void function GeneratePathFromPlayerToRootEnt( entity rootEnt, entity player, entity teamMatePortalVFX, entity allyPortalRootEnt )
{
	...
	vector endPos = rootEnt.GetOrigin()
	// Standing maxs is not a server-side accessor here; the current hull only
	// differs while crouched, and this just sets how far below the ceiling the
	// path is hung.
	vector playerMaxs = player.GetPlayerMaxs()

	// No mesh pathfinding is available, so the waypoints are a subdivision of the
	// direct line rather than a route around geometry. Travel phases straight
	// through geo regardless (see TransportPortal_PathNodeCheck), so the route
	// only shapes how the flight reads -- the raise and prune passes below are
	// what actually place it, and those need nothing but world traces.
	const float WAYPOINT_SPACING = 150.0
	int steps = int( Distance( startPos, endPos ) / WAYPOINT_SPACING )
	steps = maxint( steps, 3 )

	array<vector> waypoints
	for ( int i = 0; i <= steps; i++ )
	{
		waypoints.append( startPos + ( endPos - startPos ) * ( float( i ) / float( steps ) ) )
	}

	array<vector> pathSamples = [ startPos ]
	TraceResults traceResults
	vector pathPointToAdd

	vector zOffsetForTraceStart = <0,0,20>
	vector zOffsetForTraceEnd = <0,0,15 * METERS_TO_INCHES>
	vector zOffsetTraceFail = <0,0,20>

	bool pointWasRaised
	bool previousWasRaised = false
	bool twoPointsAgoWasRaised = false

	// Lift each waypoint into whatever headroom it has, so the flight rises over
	// cover instead of dragging along the floor.
	for ( int index = 0; index < waypoints.len() - 2; index++ )
	{
		if ( index == (waypoints.len() - 3) )
		{
			zOffsetForTraceEnd /= 2
		}

		traceResults = TraceLine( waypoints[index] + zOffsetForTraceStart, waypoints[index] + zOffsetForTraceEnd, null, TRACE_MASK_NPCWORLDSTATIC, TRACE_COLLISION_GROUP_NONE )

		if ( !traceResults.startSolid && !traceResults.allSolid && ( traceResults.fraction >= 0.99 ) )
		{
			pathPointToAdd = traceResults.endPos - <0,0,playerMaxs.z>
			pointWasRaised = true
		}
		else
		{
			pointWasRaised = false
			pathPointToAdd = ( waypoints[index] + zOffsetTraceFail )

			if ( !twoPointsAgoWasRaised && previousWasRaised )
			{
				//unraise the middle point if the other two aren't raised
				pathSamples[pathSamples.len() - 1] = waypoints[index - 1] + zOffsetTraceFail
			}
		}

		pathSamples.append( pathPointToAdd )

		twoPointsAgoWasRaised = previousWasRaised
		previousWasRaised = pointWasRaised
	}

	if ( !twoPointsAgoWasRaised && previousWasRaised && waypoints.len() > 3 )
	{
		pathSamples[pathSamples.len() - 1] = waypoints[waypoints.len() - 3] + zOffsetTraceFail
	}

	pathSamples.append( endPos )

	// Drop any point a straight line can already skip.
	for ( int index = 1; index < pathSamples.len() - 1; index++ )
	{
		traceResults = TraceLine( pathSamples[index - 1], pathSamples[index + 1], null, TRACE_MASK_NPCWORLDSTATIC, TRACE_COLLISION_GROUP_NONE )

		if ( traceResults.fraction > 0.99 )
		{
			pathSamples.remove( index )
			--index
		}
	}

	// The smoother needs at least 3 points to interpolate between.
	if ( pathSamples.len() == 2 )
	{
		vector midpoint = ( pathSamples[0] + pathSamples[1] ) / 2
		pathSamples.insert( 1, midpoint )
	}

	array<vector> pathToTake = CreateOptimizedSmoothPath( pathSamples )

	if ( pathToTake.len() <= 1 )
	{
		pathToTake = [ startPos, endPos ]
	}

	PhaseTunnelPortalData phaseTunnelData = ConvertPathToTunnelPortalData( rootEnt, teamMatePortalVFX, pathToTake )
	PhaseTunnel_CleanAndFinalizePath( phaseTunnelData.pathData, tuning.portalSpeed, tuning.portalMinTravelTime, tuning.portalMaxTravelTime, true )

	Assert( rootEnt in file.rootEntToDataMap, "rootEnt isn't in the map??" )
	VoidVisionSetExitEnt( phaseTunnelData.pathData, file.rootEntToDataMap[rootEnt].translocator )

	file.allyPortalToDataMap[allyPortalRootEnt].tunnelData = phaseTunnelData
}
...
void function ManageWarningFX_Thread( float travelTime, entity rootEnt, entity player )
{
	...
	EmitSoundOnEntityToTeam( translocator, TRANSPORT_PORTAL_ARRIVAL_WARNING_SOUND_FRIENDLY_3P, rootEntTeam )
	EmitSoundOnEntityToEnemies( translocator, TRANSPORT_PORTAL_ARRIVAL_WARNING_SOUND_ENEMY_3P, rootEntTeam )
...
void function InRangeForMinimapRui_Thread( entity receiver, var rui, string argName )
{
	...
	while ( true )
	{
		...
		{
			if ( ( rootEnt.GetNetworkedClassName() == "prop_material_harvester" ) && !TransportPortal_GetRecallUsed( rootEnt, player.GetPlayerIndex() ) )
			{
