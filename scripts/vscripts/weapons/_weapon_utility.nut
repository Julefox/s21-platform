...
void function WeaponUtility_Init()
{
	...
	RegisterSignal( "EnergyAmmoRegenAbort" )
...
array<vector> function GetProjectileShotgunBlastVectors( vector pos, vector forward, vector right, float outerSpread, float innerSpead, int numSegments )
{
		...
		DebugDrawLineRGB( pos, pos + forward * 250, 255, 0, 0, true, 3.0 )
	...
	randVecs.append( forward )
		...
		for ( int i = 0 ; i < numRadialSegments ; i++ )
		{
			...
			DebugDrawLineRGB( o1, o2, 255, 255, 0, true, 3.0 )
			DebugDrawLineRGB( i1, i2, 255, 255, 0, true, 3.0 )
			DebugDrawLineRGB( i1, o1, 255, 255, 0, true, 3.0 )
		}

		foreach ( vector vec in randVecs )
		{
			DebugDrawSphereColor( pos + vec * 250, 1.0, <255, 0, 0>, true, 3.0, 3 )
		}
	#endif
...
array<vector> function GetProjectileBlastPatternVectors( WeaponPrimaryAttackParams attackParams, array<vector> blastPattern, float patternZeroDistance )
{
	...
	foreach ( offsetVec in blastPattern )
	{
		...
		#if DEBUG_PROJECTILE_BLAST
			DebugDrawLineRGB( startPos, offsetPos, 255, 0, 0, true, 3.0 )
		#endif
	}
...
bool function PlantStickyEntity( entity ent, DeployableCollisionParams cp, vector angleOffset = ZERO_VECTOR, bool ignoreHullTrace = false, bool moveOnNoHitTrace = true )
{
	...
	else
	{
		...
		{
			DebugDrawSphereRGB( cp.pos, 5, 255, 255, 0, false, 60 )
		...
		if( moveOnNoHitTrace || trace.fraction < 1.0 )
		{
			...
			if ( DEBUG_DRAW_PLANT_STICKY )
			{
				DebugDrawSphereRGB( plantPosition, 3, 255, 0, 0, false, 60 )
			}
			#endif
		}
		else
		{
			...
			if ( DEBUG_DRAW_PLANT_STICKY )
			{
				DebugDrawSphereRGB( plantPosition, 3, 0, 0, 255, false, 60 )
			}
			#endif
		}
...
function MissileRandomDrift( timeElapsed, timeStep, windiness, intensity )
{
	...
	DebugDrawLineRGB( self.GetOrigin, self.GetOrigin + right * 100, 255, 255, 255, true, 0 )
	DebugDrawLineRGB( self.GetOrigin, self.GetOrigin + up * 100, 255, 128, 255, true, 0 )
...
function MissileControlledDrift( timeElapsed, timeStep, windiness, intensity, pathTimeMin, pathTimeMax, pathRadiusMin, pathRadiusMax )
{
 ...
 DebugDrawLineRGB( org, org + vel * 0.1, 255, 255, 255, true, 0 )
 DebugDrawLineRGB( org, org + backToPathVel * intensity * lerp * 0.1, 128, 255, 128, true, 0 )
...
void function DebugDrawMissilePath( entity missile )
{
	...
	while ( true )
	{
		...
		if ( !IsValid( missile ) )
			return
		DebugDrawLineRGB( lastPos, missile.GetOrigin(), 0, 255, 0, true, 20.0 )
...
{
	if ( !IsBitFlagSet( DamageInfo_GetCustomDamageType( damageInfo ), DF_STUN_AI ) )
		return false
...
void function NpcEmpRebootPrototype( entity npc, var damageInfo, asset humanFx, asset titanFx )
{
	...
	DebugDrawLineRGB( origin, groundPos, 255, 0, 0, true, 15 )
...
void function EMP_FX( asset effect, entity ent, string tag, float duration, int visFlagOverride = -1 )
{
	...
	if ( !isPlayer )
	{
		...
		// The server has no hover-vehicle stun, so shocked vehicles keep moving.
...
{
	// UpdateLaserSightColor now exists (dedi native, added ) --
	// this catches up the player's lobby-persisted laserSightColor/
	// laserSightColorCustomized userinfo ConVars into the networked DT_Player
	// fields the moment they connect, matching S21 own design intent:
	// menu_laser_sight_options_color.nutui's SaveCurrentColorToPlayerConfig
	// deliberately skips the live RunClientScript round-trip while IsLobby
	// is true, relying on this connect hook to sync the color once the match
	// actually starts.
...
{
	if ( !IsWeaponKeyFieldDefined( baseWeapon, "is_akimbo_weapon" ) )
		return false

	var akimboKey = GetWeaponInfoFileKeyField_Global( baseWeapon, "is_akimbo_weapon" )
	if ( akimboKey == null )
		return false

	if ( typeof akimboKey == "int" )
		return expect int( akimboKey ) != 0

	return expect bool( akimboKey )
}
...
void function EnergyAmmoRegen_Start( entity weapon )
{
	...
	if ( "energyRegenRunning" in weapon.s && weapon.s.energyRegenRunning )
		return
	weapon.s.energyRegenRunning <- true
	thread EnergyAmmoRegen_Think( weapon )
#endif
}
...
{
	weapon.EndSignal( "OnDestroy" )
	weapon.EndSignal( "EnergyAmmoRegenAbort" )

	OnThreadEnd(
		function() : ( weapon )
		{
			if ( IsValid( weapon ) && ( "energyRegenRunning" in weapon.s ) )
				weapon.s.energyRegenRunning = false
		}
	)
...
vector function CalcProjectileTrajectory( vector startPos, vector targetPos, float desiredTravelTime, bool debugDraw )
{
	...
	if ( debugDraw )
	{
		...
		DebugDrawSphereRGB( startPos, 5, 255, 255, 0, false, DRAW_TIME )
		DebugDrawSphereRGB( targetPos, 5, 255, 255, 0, false, DRAW_TIME )
...
CrosshairTargetData function GetCrosshairTargetDataAngles( entity player, float minDistance, float maxDistance, float airBurstHeight, bool capAtMaxRange = false )
{
	...
	if ( DEBUG_AIRBUST_TARGET )
	{
		DebugDrawLineRGB( data.crosshairStart + <0, 0, 1>, crosshairEnd, int(COLOR_ORANGE.x), int(COLOR_ORANGE.y), int(COLOR_ORANGE.z), false, 0.1 )
		DebugDrawSphereRGB( crosshairEnd, 5, int(COLOR_ORANGE.x), int(COLOR_ORANGE.y), int(COLOR_ORANGE.z), true, 0.1 )
		DebugDrawSphereRGB( initialCameraTrace.endPos, 10, 0, 255, 0, false, 0.1 )
	}
	...
	if ( initialCameraTrace.fraction < 1.0 ) //flattenedDistanceToTarget < (desiredDistanceToTarget - 2*METERS_TO_INCHES ) )
	{
		...
		if ( canTraceAbove )
		{
			...
			if ( DEBUG_AIRBUST_TARGET )
			{
				...
				DebugDrawLineRGB( traceAbove.endPos, traceDown.endPos, 0, 255, 0, false, 0.1 )
				DebugDrawSphereRGB( traceDown.endPos, 10, 0, 255, 0, false, 0.1 )
			}
		}
	...
	else
	{
		...
		if ( DEBUG_AIRBUST_TARGET )
		{
			DebugDrawLineRGB( initialCameraTrace.endPos, traceDown.endPos, 0, 255, 0, false, 0.1 )
			DebugDrawSphereRGB( traceDown.endPos, 10, 0, 255, 0, false, 0.1 )
		}
	...
	if( traceToAirburst.fraction < 1.0 )
	{
		...
		if ( DEBUG_AIRBUST_TARGET )
		{
			DebugDrawLineRGB( player.GetWorldSpaceCenter(), data.airburstTarget, int(COLOR_PURPLE.x), int(COLOR_PURPLE.y), int(COLOR_PURPLE.z), false, 0.1 )
			printt( "GetCrosshairTargetDataAngles - heightMult needed: " + heightMult )
		}
	}
	...
	if ( DEBUG_AIRBUST_TARGET )
	{
		DebugDrawSphereRGB( data.groundTarget, 5, int(COLOR_PURPLE.x), int(COLOR_PURPLE.y), int(COLOR_PURPLE.z), false, 0.1 )
		DebugDrawText( data.groundTarget, "ground", false, 0.1 )
		DebugDrawSphereRGB( data.airburstTarget, 15, int(COLOR_PURPLE.x), int(COLOR_PURPLE.y), int(COLOR_PURPLE.z), false, 0.1 )
		DebugDrawText( data.airburstTarget, "air", false, 0.1 )
		DebugDrawLineRGB( data.groundTarget, data.airburstTarget, int(COLOR_PURPLE.x), int(COLOR_PURPLE.y), int(COLOR_PURPLE.z), false, 0.1 )
	}
...
void function OnWeaponDeactivate_Smart_Reload ( entity weapon )
{
	...
	// Recorder dummies (CAI_BaseNPC) also own weapons; skip player-only path.
	if ( !player.IsPlayer() )
		return
...

{
...

	{
		...
		DebugDrawLineRGB( traceStart, traceEndUnder, 0, 255, 0, true, 25.0 )
...

		{
			...
			DebugDrawLineRGB( forwardTrace.endPos, forwardTrace.endPos + <0,0,-1000>, 255, 0, 0, true, 25.0 )
		...
		DebugDrawLineRGB( traceStart, traceEndOver, 0, 0, 255, true, 25.0 )
...

{
...

	{
		...
		else if ( IsBitFlagSet( damageType, DF_EXPLOSION ) && inflictor.IsPlayer() && IsValid( inflictor.GetActiveWeapon( eActiveInventorySlot.mainHand ) ) )
