...
var function OnWeaponPrimaryAttack_ability_crypto_drone_emp( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	if ( !StatusEffect_HasSeverity( weaponOwner, eStatusEffect.crypto_has_camera ) ) //This should handle a case of the drone getting killed while the weapon is raising and hasn't fired yet.
	{
			return 0
...
void function DroneEMP( entity owner )
{
	...
	bool empHitsAlterTac  = EMP_ShouldAffectAlterTac()
	array<entity> targets = CryptoDrone_GetNearbyTargetsForEMPRange( camera )
	foreach ( entity target in targets )
	{
		if ( target.IsPhaseShifted() )
		{
			if ( !empHitsAlterTac || target.GetPhaseShiftType() != PHASETYPE_DOOR )
				continue
		}

		if ( StatusEffect_HasSeverity( target, eStatusEffect.immune_to_abilities ) )
			continue
