...
void function MpWeaponGrenadeBangalore_Init()
{
	...
	RegisterSignal( "stop_smokescreen_screen_fx" )
		...
		AddCallback_OnPassiveChanged( ePassives.PAS_TAC_UPGRADE_ONE, HealInSmoke_OnPassiveChanged ) // upgrade_bangalore_tac_smoke_heal
		RegisterSignal( "ResetSmokeHealing" )
	#endif //SERVER
...
#if SERVER
void function HealInSmoke_OnPassiveChanged( entity player, int passive, bool didHave, bool nowHas )
{
	if ( nowHas )
	{
		AddEntityCallback_OnDamaged( player, PassiveSmokeHealthRegen_OnPlayerDamaged )
		file.playerToNextAllowedHealTime[ player ] <- Time()
	}
	if ( didHave )
	{
		RemoveEntityCallback_OnDamaged( player, PassiveSmokeHealthRegen_OnPlayerDamaged )
...
#if SERVER
void function PassiveSmokeHealthRegen_OnPlayerDamaged( entity player, var damageInfo )
{
	if ( ( player.GetHealth() == 0 ) )
		return

	if( player in file.playerToNextAllowedHealTime )
		file.playerToNextAllowedHealTime[ player ] <- Time() + UPGRADE_BANGALORE_SMOKE_HP_REGEN_DELAY

	player.Signal( "ResetSmokeHealing" )
}
#endif
...
void function OnProjectileIgnite_weapon_grenade_bangalore( entity projectile )
{
	...
	DebugDrawLineRGB( origin, origin + ( projectileForwardOnSurf * 128.0 ), 0, 128, 128, true, 2 )
...
float function Bangalore_GetSmokeDuration( entity player )
{
	...
	if( PlayerHasPassive( player, ePassives.PAS_TAC_UPGRADE_TWO ) ) // upgrade_bangalore_tac_smoke_duration
		duration += 3
...
void function Bangalore_DetonateSmokeGrenade( entity smokeGrenade, entity owner, entity inflictorHelper )
{
	...
	// CafeMod bangalore_gas: smoke detonation becomes gas cloud.
	if ( CafeMod_IsEnabled( "bangalore_gas" ) )
	{
		if ( !CafeMod_CanSpawnEntities( 32 ) )
			return

		entity dummyCloudSource = CreateScriptMover( "", smokeGrenade.GetOrigin() )
		if ( !IsValid( dummyCloudSource ) )
			return
	...
	smokescreen.lifetime = Bangalore_GetSmokeDuration( owner )
...
void function BangaloreSmokeGrenadeTriggerTouchingThread( entity trigger, entity ent )
{
	...
	{
		if( PlayerHasPassive( ent, ePassives.PAS_ADRENALINE ) && PlayerHasPassive( ent, ePassives.PAS_TAC_UPGRADE_ONE ) ) // upgrade_bangalore_tac_smoke_heal
		{
			if( !( ent in file.playerToHealHandle ) )
			{
				if( ent in file.playerToNextAllowedHealTime )
				{
					if( Time() > file.playerToNextAllowedHealTime[ent] )
						WeaponGrenadeSmoke_StartHealing( ent )
				}
			}
		}
...
void function WeaponGrenadeSmoke_StartHealing( entity player )
{
	if( !( player in file.playerToHealHandle ) )
	{
		file.playerToHealHandle[player] <- EntityHealResource_Add( player, UPGRADE_BANGALORE_SMOKE_HP_REGEN_RATE_PER_SEC, UPGRADE_BANGALORE_SMOKE_HP_REGEN_RATE_PER_SEC, 0, "regen_default", player )
	}
	...
	player.Signal( "ResetSmokeHealing" )
	thread WeaponGrenadeSmoke_EndGasHealing( player )
}

void function WeaponGrenadeSmoke_EndGasHealing( entity player )
{
	player.EndSignal( "ResetSmokeHealing" )
