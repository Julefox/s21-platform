...
void function SonicBlastHitPlayer( vector startPos, entity weapon, entity weaponOwner, entity victim )
{
	...
	if ( FerroWall_BlockScan( startPos, victim.GetWorldSpaceCenter() ) )
		return
	...
	if ( ( ( victim.IsPlayer() || isDummy ) && !immuneToAbilities ) )
	{
		if ( file.sonicBlastDoesDamage )
		{
			...
			//Firing range, dummies/friendly fire team mates when killed will become invalidated, so early out. R5DEV-279094
...
float function GetSonicBlast_Scan_Duration_Base( entity player )
{
	...
	if( PlayerHasPassive( player, ePassives.PAS_TAC_UPGRADE_ONE ) ) // upgrade_seer_scan_duration_extension
	{
...
float function GetSonicBlast_Silence_Duration_Base( entity player )
{
	...
	if( PlayerHasPassive( player, ePassives.PAS_TAC_UPGRADE_TWO ) ) // upgrade_seer_silence_duration_extension
	{
...
{
	// Remove the extra 10m padding for the blast range on top of the pas range if the upgrade is selected
