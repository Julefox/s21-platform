...
#if SERVER
void function OnProjectileCollision_DebuffZone( entity projectile, vector pos, vector normal, entity hitEnt, int hitBox, bool isCritical )
#else
...
void function OverheatSFX_Thread( entity target )
{
	...
	while ( true )
	{
		float currentOverheatMeterValue = target.GetPlayerOverheatValue()
...
void function FollowUp_Start( entity player, float duration )
{
	...
	// the boost is looked up by effect rather than by handle, same as the other adrenaline passives.
	if ( file.playerStatusEffects[ player ].speedBoostID != SE_INVALID_HANDLE && StatusEffect_GetTimeRemaining( player, eStatusEffect.speed_boost ) > 0 )
	{
