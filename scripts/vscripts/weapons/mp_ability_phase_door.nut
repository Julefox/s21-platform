...
void function AlterTacTest( entity player )
{
	...
	float radius = 8000
	float height = 4000
	DebugDrawCylinderRGB( triggerPos, triggerAng, radius, height, int( COLOR_RED.x ), int( COLOR_RED.y ), int( COLOR_RED.z ), false, 15 )
...
entity function CreatePhaseDoorTriggers( entity owner, entity portalRootEnt, vector origin, vector surfaceNormal )
{
	...
	if ( PHASE_DOOR_DEBUG_DRAW )
	{
		DebugDrawCylinderRGB( triggerPos - <0, 0, PHASE_DOOR_TRIGGER_RADIUS>, <-90, 0, 0>, PHASE_DOOR_TRIGGER_RADIUS, PHASE_DOOR_TRIGGER_RADIUS * 2, int( COLOR_GOLDEN_ROD.x ), int( COLOR_GOLDEN_ROD.y ), int( COLOR_GOLDEN_ROD.z ), false, 15 )
	}
	#endif
...
void function TeleportPlayerFromPortalTrigger( entity trigger, entity player )
{
	...
	if ( exitPortalOrientation != ePhaseDoorOrientation.PHASE_DOOR_ORIENTATION_WALL )
	{
		...
		if ( PHASE_DOOR_DEBUG_DRAW )
		{
			...
			DebugDrawSphereRGB( endPos, 5.0, int(COLOR_DARK_GREEN.x), int(COLOR_DARK_GREEN.y), int(COLOR_DARK_GREEN.z), true, 5.0 )
		}
	#endif
	...
	if ( endTrigger in file.phaseDoorPairings )
	{
		...
		if ( sqrDistToNearPortal > sqrDistToFarPortal )
		{
			...
			// OPSPR_* auto-reporting has no backing implementation here.
		}
	}
	else
	{
...
void function TravelingThroughPortalExtension_Thread( entity player, entity extension )
{
	...
	#if PHASE_DOOR_PORTAL_EXTENSIONS_DEBUG_DRAW
		DebugDrawLineRGB( endPos, extensionBottom, int(COLOR_GOLDEN_ROD.x), int(COLOR_GOLDEN_ROD.y), int(COLOR_GOLDEN_ROD.z), false, 10 )
	#endif
	...
	#if PHASE_DOOR_PORTAL_EXTENSIONS_DEBUG_DRAW
		DebugDrawSphereRGB( closestPointOnLine, 5, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), false, 10 )
		DebugDrawSphereRGB( endPos, 5, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), false, 10 )
	#endif
...
bool function PhaseDoor_CheckInvalidEnt( entity ent )
{
	...
	if ( ent.GetScriptName() in file.invalidEntityScriptNames )
		return true

	entity parentEnt = ent.GetParent()
	if ( IsValid( parentEnt ) && (parentEnt.GetScriptName() in file.invalidEntityScriptNames) )
		return true
