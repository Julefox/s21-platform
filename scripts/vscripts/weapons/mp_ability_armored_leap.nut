...
void function ArmoredLeap_Master_Thread( entity player, vector endPoint, vector airPoint, entity hitEnt )
{
	...
	thread ArmoredLeap_CheckForUpdraft_Thread( player ) //todo: Travis -> Can we move this to code as part of the normal Interrupt logic?
		...
		thread ArmoredLeap_CreateWarningArea( player, endPoint, ARMORED_LEAP_WARNING_DURATION )
		thread ArmoredLeap_LaunchVFX_Thread( player )

		TraceResults groundTraceStart = TraceLine( player.GetOrigin(), player.GetOrigin() + <0, 0, -5000>, [ player ], TRACE_MASK_SOLID, TRACE_COLLISION_GROUP_PLAYER_MOVEMENT )
		TrackingVision_CreatePOI( eTrackingVisionNetworkedPOITypes.PLAYER_ABILITY_NEWCASTLE_ARMORED_LEAP_START, player, groundTraceStart.endPos, player.GetTeam(), player )

		if( player in file.allyTarget )
		{
			entity allyTarget = file.allyTarget[player]
			if( IsValid(allyTarget) && allyTarget.IsPlayer() )
				Remote_CallFunction_NonReplay( allyTarget, "ServerToClient_RescueTargetRui_Activate", allyTarget, player )
		}


		EmitSoundOnEntityOnlyToPlayer( player, player, ARMORED_LEAP_SOUND_ACTIVATE_1P )


		//State - START//
		if ( !GetArmoredLeapUseCode() )
		{
			mover = CreateScriptMover( ARMORED_LEAP_MOVER_SCRIPTNAME, player.GetOrigin(), player.GetAngles() )
		}

		#if DEVELOPER
			//thread Debug_TrackPlayerInLeap_Thread( player, mover, endPoint, airPoint )
		#endif

		// ==================== Set 3rd person settings ================//
		{
			float minPitch = 10 //-180
			float maxPitch = 90 //180

			vector oldFacing = player.Player_GetWorldViewAngles()
			player.SetTrackEntity( player )
			player.SetTrackEntityBlendInTimes( 0.25, 0.20, 0.0 ) //( 0.35, 0.1, 0.20 )
			player.SetTrackEntityBlendOutTime( 0.75 )
			...
			player.SetTrackEntityMinPitch( minPitch )
			player.SetTrackEntityMaxPitch( maxPitch )
			player.SetTrackEntityOffsetDistance( 90 ) //60
			player.SetTrackEntityOffsetHeight( 10 )
			player.SetTrackEntityOffsetRight( -25 )
			//player.SetTrackEntityOffsetDistance( 100 )
			//player.SetTrackEntityOffsetHeight( 5 )
			player.SetTrackEntityShouldViewAnglesFollowTrackedEntity( true )
			player.SnapEyeAngles( oldFacing )
		}
	...
	OnThreadEnd(
		function() : ( player, mover, startpoint, didUltDeployOnInterrupted )
		{
			if ( IsValid( player ) )
			{
					...
					if( player in file.allyTarget )
					{
						if( IsValid(file.allyTarget[player]) && file.allyTarget[player].IsPlayer() )
							Remote_CallFunction_NonReplay( file.allyTarget[player], "ServerToClient_RescueTargetRui_Deactivate", file.allyTarget[player] )
						delete file.allyTarget[player]
					}

					if( player in file.playerWeaponsHolstered )
					{
						if( file.playerWeaponsHolstered[player] )
						{
							DeployAndEnableWeapons( player )
							delete file.playerWeaponsHolstered[player]
						}
					}

					entity ultimateWeapon = player.GetOffhandWeapon( OFFHAND_ULTIMATE )
					if( IsValid( ultimateWeapon ) && ultimateWeapon.HasMod( "ability_in_effect_regen_paused" ) )
					{
						ultimateWeapon.RemoveMod( "ability_in_effect_regen_paused" )
					}
				#endif
				...
				{
					if ( player in didUltDeployOnInterrupted )
					{
						if ( !didUltDeployOnInterrupted[player] )
						{
							//only refund if we were interrupted and didn't deploy the ult.
							ArmoredLeap_OnAttemptFailed( player, startpoint )
						}
					}
				}

				//We only set the deployed Ult flag if we build the Castle.
				//If this flag is still false when we end this thread, it means the Leap was interrupted and the shield was not placed -> Refund Ult
				if( player in file.ultDeployed )
				{
					if( !file.ultDeployed[player] )
					{
						ArmoredLeap_OnAttemptFailed( player, startpoint )
					}
				}
				...
				if ( player in file.currentArmoredLeapPhase )
					delete file.currentArmoredLeapPhase[player]
				#elseif CLIENT
				file.currentArmoredLeapPhase = PLAYER_ARMORED_LEAP_PHASE_NONE
				#endif
	...
	#if SERVER
	//player.SetLocalAngles( VectorToAngles( FlattenVec(Normalize( endPoint - player.GetOrigin() )) ) )

	float lerpTime = 0.0

	if( inDashRange )
		lerpTime = ARMORED_LEAP_LAUNCH_DASH_CROUCH_TIME
	else if ( player.IsOnGround() )
		lerpTime = ARMORED_LEAP_LAUNCH_CROUCH_TIME
	else
		lerpTime = ARMORED_LEAP_AIR_HOVER_TIME

	player.SetTrackEntityOffsetDistanceOverTime( 0, ARMORED_LEAP_INITIAL_CAMERA_DIST, lerpTime, THIRD_PERSON_CAMERA_LERP_MODE_LOGARITHMIC )
	player.SetTrackEntityOffsetHeightOverTime( 0, ARMORED_LEAP_INITIAL_CAMERA_HEIGHT, lerpTime, THIRD_PERSON_CAMERA_LERP_MODE_LOGARITHMIC )
	player.SetTrackEntityOffsetRightOverTime( 0, ARMORED_LEAP_INITIAL_CAMERA_RIGHT, lerpTime, THIRD_PERSON_CAMERA_LERP_MODE_LOGARITHMIC )

	#if DEVELOPER
	if ( DEBUG_CAMERA_LERP )
	{
		printt("Setting starting height to: " + 0 + " ending height: " + ARMORED_LEAP_INITIAL_CAMERA_HEIGHT + " over: " + lerpTime )
		printt("Setting starting right to: " + 0 + " ending right: " + ARMORED_LEAP_INITIAL_CAMERA_RIGHT + " over: " + lerpTime )
	}
	#endif //DEVELOPER
	#endif //SERVER
	...
	if ( armoredLeapType == PLAYER_ARMORED_LEAP_TYPE_DASH ) // LAUNCH PREP - DASH
	{
		if ( GetArmoredLeapUseCode() )
		{
			...
			{
				#if SERVER
				bool ultDeployed = ArmoredLeap_Handle_Interrupted( player, endPoint )
				didUltDeployOnInterrupted[player] = ultDeployed
				#endif
	...
	else if ( armoredLeapType == PLAYER_ARMORED_LEAP_TYPE_JUMP )	 // MOVE TO AIR POSITION
	{
		...
		if ( GetArmoredLeapUseCode() )
		{
			...
			float armoredLeapDistMax = GetArmoredLeapDistance( player )
			float leapDist = Distance( player.GetOrigin(), airPoint )
			float jumpSpeed = GraphCapped( leapDist, ARMORED_LEAP_GROUND_DASH_RANGE, armoredLeapDistMax, file.airJumpSpeedMin, file.airJumpSpeedMax )
			float hoverSpeed = GraphCapped( leapDist, ARMORED_LEAP_GROUND_DASH_RANGE, armoredLeapDistMax, file.airHoverSpeedMax, file.airHoverSpeedMin )
	...
	else // AIR HOVER
	{
		...
		if ( !GetArmoredLeapUseCode() )
		{
			...
			if ( !GetArmoredLeapUseCode() )
				player.SetParent( mover, "REF", false ) //return to mover-control for Slam
		#endif
		isHoverSlam = true
	}
	...
	{
		endPoint = ArmoredLeap_GetUpdatedLKP( player, airPoint, endPoint ) //LKP
	...
	if( player in file.allyTarget )
	{
		if( IsValid( file.allyTarget[player] ) )
			StatsHook_NewcastleCastleWallAllyRescues( player )
	}
	//printt( "Allies Rescued:  " + GetStat_Int( player, ResolveStatEntry( CAREER_STATS.newcastle_allies_rescued ), eStatGetWhen.CURRENT ) )
	#endif
	...
	//DebugDrawAngles( player.GetOrigin(), VectorToAngles( player.GetViewVector() ) , 20 )

	#if SERVER
		////// --- end 3rd person ---
		thread ArmoredLeap_CameraRecoveryDelay( player, ARMORED_LEAP_CAM_RECOVERY_TIME )
		if ( !GetArmoredLeapUseCode() )
		{
			player.ClearParent()
		...
		if ( !CreateCastleWall( player ) )
		{
			thread ArmoredLeap_LeapComplete_Thread( player )
			return
		}

		//Seems to be an issue where the player state can be INTERRUPTED on end, but the script had not updated this flag
		//As a result, the player gets a refund even when they deployed a wall. This MIGHT get resolved if Travis' signal change fix goes in.
		didUltDeployOnInterrupted[player] <- true
	#endif
...
void function ArmoredLeap_HandleInterruptedMidLeap( entity player )
{
	...
	if ( leapActive )
	{
		...
		//if we're above sprint speed, then cap to some percentage of our velocity
		if ( currentSpeed > 300.0 )
		{
			player.SetVelocity( Normalize( player.GetVelocity() ) * ( currentSpeed * ARMORED_LEAP_INTERRUPTED_VELOCITY_SCALE ) )
		}
		#endif //SERVER
	}
}
...
void function ArmoredLeap_SetAllyTargetAndLKP( entity ent, ArmoredLeapTargetInfo info )
{
	...
	if( IsValid( targetAlly ) )
	{
		...
		if( groundTrace.fraction < 1 || inPhaseShift ) //We have ground beneath our target player
		{
			...
			//DebugDrawSphere( allyEndPos, 10.0, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), true, 0.1 )
...
vector function ArmoredLeap_GetUpdatedLKP( entity player, vector airPoint, vector targetPos )
{
	...
	//DebugDrawLine( player.GetOrigin(), targetPos, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 20 )
...
void function ArmoredLeap_AR_Placement_Thread( entity weapon )
{
	...
	while ( !isLeaping )
	{
		...
		if ( leapPhase != PLAYER_ARMORED_LEAP_PHASE_NONE && leapPhase != PLAYER_ARMORED_LEAP_PHASE_PREP )
		{
			...
			{
				DebugDrawLineRGB( player.GetOrigin(), info.airPos, 0, 200, 200, true, 0.1 )
				DebugDrawLineRGB( info.airPos, info.finalPos, 200, 200, 0, true, 0.1 )
			}
		#endif
...
ArmoredLeapTargetInfo function GetArmoredLeapTargetInfo( entity ent )
{
	...
	if( !ArmoredLeap_HasValidHullRoom( ent, ent.GetOrigin() ) )
	{
		...
		if( DEBUG_ARMORED_LEAP_TARGETING_DRAW )
		{
			...
			DebugDrawSphereRGB( initialTrace.endPos, 5.0, 0, 150, 150, false, 0.1 ) //AQUA (Initial Trace)
		}
	#endif
	...
	if( !foundValidEnd )
	{
		...
		if ( DO_LEDGE_TRACE && !foundValidEnd && (initialTrace.fraction < 1.0 || didLowerTraceHit) )
		{
			...
			TraceResults ledgeTrace = TraceHull( ledgeTraceStart, ledgeTraceStart + <0, 0, -200>, ent.GetPlayerMins(), ent.GetPlayerMaxs(), ignoreArray, TRACE_MASK_PLAYERSOLID, TRACE_COLLISION_GROUP_PLAYER_MOVEMENT )
				...
				if( DEBUG_ARMORED_LEAP_TARGETING_DRAW )
				{
					DebugDrawSphereRGB( ledgeTraceStart, 8.0, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), true, 0.1 ) //YELLOW (LedgeStart)
					DebugDrawSphereRGB( ledgeTrace.endPos, 15.0, 255, 175, 0, true, 0.1 ) //ORANGE (LedgeEnd)
					DebugDrawCircleRGB( ledgeTrace.endPos, VectorToAngles(ledgeTrace.surfaceNormal), 64, 255, 175, 175, true, 0.1, 3 )
				}
			#endif
		...
		if ( !foundValidEnd )
		{
			...
			if ( downTrace.fraction < 1.0 )
			{
				...
				if( DEBUG_ARMORED_LEAP_TARGETING_DRAW )
				{
					DebugDrawSphereRGB( downTrace.endPos, 5.0, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), false, 0.1 ) //GREEN (Downcast End)
				}
			#endif
		}
		...
		if ( !foundValidEnd )
		{
			...
			if ( lowAirTrace.fraction < 1.0 )
			{
				...
				if( dropAirTrace.fraction < 1 && ArmoredLeap_HasValidLeapPos( ent, null, dropAirTrace.endPos, hitEnt, info ))
				{
					...
					if( DEBUG_ARMORED_LEAP_TARGETING_DRAW )
					{
						DebugDrawSphereRGB( intersect, 8.0, 255, 0, 250, false, 0.1 ) //PINK (LedgeInset)
						DebugDrawSphereRGB( dropAirTrace.endPos, 15.0, 255, 0, 100, true, 0.1 ) //RED (LedgeDropEnd)
					}
				#endif
			}
				...
				if( DEBUG_ARMORED_LEAP_TARGETING_DRAW )
				{
					DebugDrawSphereRGB( lowAirTrace.endPos, 5.0, 100, 0, 50, true, 0.1 ) //PURPLE (LowAir)
				}
			#endif
	...
	if ( IsValid( player ) && info.success )
	{
		...
		for ( int i = 0; i < findPosIterations; i++ )
		{
			...
			else
			{
				...
				if ( DEBUG_BETTER_AIR_POS )
				{
					DebugDrawLineRGB( playerPos, traceTarget, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.1 )
		...
		if ( !foundGoodAirPos )
		{
			...
			for ( int i = walkBackIterations; i >= 0; i-- )
			{
				...
				if ( traceToWalkbackPos.fraction >= 0.99 )
				{
						...
						if ( DEBUG_BETTER_AIR_POS )
						{
							DebugDrawLineRGB( player.GetOrigin(), traceToWalkbackPos.endPos, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 0.1 )
					...
					if ( traceFinalPos.fraction >= 0.99 )
					{
							...
							if ( DEBUG_BETTER_AIR_POS )
							{
								DebugDrawLineRGB( walkBackIterationPos, traceFinalPos.endPos, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 0.1 )
								//DebugDrawText( iterationTrace.endPos, "fraction: " + iterationTrace.fraction, true, 0.1 )
							}
						#endif
					...
					else
					{
							...
							if ( DEBUG_BETTER_AIR_POS )
							{
								DebugDrawLineRGB( walkBackIterationPos, info.finalPos, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.1 )
				...
				else
				{
						...
						if ( DEBUG_BETTER_AIR_POS )
						{
							DebugDrawLineRGB( player.GetOrigin(), walkBackIterationPos, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.1 )
...
vector function ArmoredLeap_GetBestAllyLandingPos( entity ent, vector endPos, vector eyeDir, ArmoredLeapTargetInfo info )
{
	...
	if( !ArmoredLeap_HasValidLandingRoom( ent, endPos, info ) )
	{
		...
		foreach ( traceVector in ridgeTraceVectorArray )
		{
			...
			if ( fraction == 1 )
			{
				//DebugDrawSphere( ridgeTrace.endPos, 5, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.1 )
				continue
			}
			...
			{
				//DebugDrawSphere( ridgeTrace.endPos, 5, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 0.1 )
				return bestPos
			}

			//DebugDrawSphere( ridgeTrace.endPos, 5, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), true, 0.1 )
		}
	}
	...
	if( !isDash ) //We only need to check for Air Clearance if we're LEAPING
	{
		...
		if ( upAirTrace.fraction < 1.0 || !hasAirSpace )
		{
			...
			if( zDist < ARMORED_LEAP_MIN_AIR_HEIGHT || newAirLeapTrace.fraction < 1 || !hasAirSpace)
			{
				...
				//DebugDrawLine( newAirLeapPos, adjustedEndPos, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 0.1 )
				...
				if ( overheadAirLeapTrace.fraction < 1.0 || !hasAirSpace)
				{
					...
					{
						//DebugDrawSphere( newAirLeapPos, 5, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 0.1, 8 )
						info.failCase = eFailCase.BLOCKED_LANDING
						return false
					}
					...
					if( IsValid(targetAlly) )
					{
						...
						if( dashHighTrace.fraction < 1.0 || !hasAirSpace)
						{
							...
							if( superDashTrace.fraction < 1 || !hasAirSpace)
							{
								...
								{
									//DebugDrawSphere( superDashTrace.endPos, 25, int(COLOR_MAGENTA.x), int(COLOR_MAGENTA.y), int(COLOR_MAGENTA.z), true, 0.1, 8 )
									info.failCase = eFailCase.BLOCKED_LEAP
									return false
								}
							}
							//DebugDrawSphere( dashHighTrace.endPos, 25, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), true, 0.1, 8 )
						}
...
void function CastleWall_CreateMainWall_Thread( entity player, entity castle, vector dir, vector originPos )
{
	...
	if( IsValid(castle) ) // Extend the peice upwards
	{
		...
		//DebugDrawLine( castle.GetOrigin(), castle.GetOrigin() + (sideDir) * 45, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 20 )
...
void function CastleWall_CreateSnakeWall_Thread( entity player, entity castle, vector dir, vector originPos, vector sideDir, bool isLeft, string characterSkinName, int characterCamo )
{
	...
	while( snakeInfo.wallLength < CASTLE_SNAKE_MAX_WALL_LENGTH  && IsValid(mover) && Time() < endTime )
	{
		...
		if( DEBUG_SNAKE_DRAW )
		{
			DebugDrawSphereRGB( mover.GetOrigin(), 3, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), true, 0.2 )
			DebugDrawSphereRGB( destination, 5, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), true, 1 )
		}
		#endif
...
//todo: Fix this - We check ahead and down to see if we can drop off - BUT on success, we basically move through the ground to get there.
	...
	if ( downTrace.fraction == 1.0 ) //FOUND A CLIFF
	{
		TraceResults downCliffTrace = TraceLine( tracePos, tracePos + < 0.0, 0.0, -CASTLE_SNAKE_DROP_TEST_HEIGHT_MAX >, ignoreArray, TRACE_MASK_SOLID, TRACE_COLLISION_GROUP_PLAYER_MOVEMENT )
			...
			if( DEBUG_SNAKE_DRAW )
			{
				DebugDrawSphere( downCliffTrace.endPos, 6.0, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 15.0, 8 )
			}
		#endif
		...
		else //Actually a CLIFF, start BEND ANGLE test to see if we can follow a LEDGE
		{
			...
			for( int i = 0; i < CASTLE_SNAKE_NUM_ANGLE_CHECKS; i++ )
			{
				...
				if( ledgeBendTrace.fraction == 1 ) //We're good ahead. Check down
				{
					...
					if( DEBUG_SNAKE_DRAW )
					{
						DebugDrawSphereRGB( ledgeBendTrace.endPos, 6.0, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 15.0 )//Show where we're checking around the circle
					}
					#endif
	...
	if ( IsValid( trigger ) )
	{
		...
		if( DEBUG_DRAW_DAMAGE_BARRIERS )
		{
			if( segmentType == eSegmentType.HIGH )
			{
				...
				DebugDrawLineRGB( trigger.GetOrigin(), trigger.GetOrigin() + trigger.GetRightVector() * widthHigh, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 60 )
				DebugDrawLineRGB( trigger.GetOrigin(), trigger.GetOrigin() - trigger.GetRightVector() * widthHigh, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 60 )
				DebugDrawSphereRGB( trigger.GetOrigin() + trigger.GetRightVector() * widthHigh, 4.0, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 60 )
				DebugDrawSphereRGB( trigger.GetOrigin() - trigger.GetRightVector() * widthHigh, 4.0, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 60 )
			...
			if( segmentType == eSegmentType.LOW )
			{
				...
				DebugDrawLineRGB( trigger.GetOrigin(), trigger.GetOrigin() + trigger.GetRightVector() * widthLow, 200, 200, 0, true, 60 )
				DebugDrawLineRGB( trigger.GetOrigin(), trigger.GetOrigin() - trigger.GetRightVector() * widthLow, 200, 200, 0, true, 60 )
				DebugDrawSphereRGB( trigger.GetOrigin() + trigger.GetRightVector() * widthLow, 4.0, int(COLOR_YELLOW.x), int(COLOR_YELLOW.y), int(COLOR_YELLOW.z), true, 60 )
				DebugDrawSphereRGB( trigger.GetOrigin() - trigger.GetRightVector() * widthLow, 4.0, 255, 255, 100, true, 60 )
			}
		}
	#endif
...
void function CastleWall_InterceptProjectiles( entity player, entity castle, entity shieldAnchor )
{
	...
	{
		DebugDrawSphereRGB( vortexOrigin, INTERCEPT_RANGE_MAX + INTERCEPT_RANGE_MIN, 255, 255, 100, true, 10 )
	}
	#endif
...
void function CastleWall_OnProjectileHitVortexTrigger( entity weapon, entity vortexSphere, entity attacker, entity projectile, vector contactPos )
{
	...
	if ( DEBUG_DRAW_ANTI_GRENADE_DEBUG )
	{
		DebugDrawSphereRGB( closestWall.GetOrigin(), 30, 255, 255, 255, true, 5 )
	}
	#endif
...
bool function CastleWall_ShouldTargetProjectile( entity wallEnt, entity projectile, entity attacker, vector contactPos )
{
	...
	if ( DEBUG_DRAW_ANTI_GRENADE_DEBUG )
	{
		...
		DebugDrawLineRGB( rDirA, rDirA + <0,0,100>, int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), true, 20.0 )
		DebugDrawLineRGB( rDirB, rDirB + <0,0,100>, int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), true, 20.0 )
	}
	#endif
...
bool function CastleWall_HasLOSToTarget( entity trophy, entity target, entity castle, vector contactPos )
{
	...
	if ( DEBUG_DRAW_ANTI_GRENADE_DEBUG )
	{
		DebugDrawLineRGB( results.endPos, endOrigin, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 20.0 )
		DebugDrawLineRGB( startOrigin, results.endPos, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 20.0 )
	}
	#endif
