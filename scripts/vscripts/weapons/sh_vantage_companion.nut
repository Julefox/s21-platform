...
OrderPosData function FindEchoOrderPos( entity player )
{
	...
	{
		DebugDrawLineRGB( startTraceInitial, endTraceInitial, 255, 255, 0, false, 5.0 )
		DebugDrawLineRGB( startTraceInitial, trInitial.endPos, 0, 255, 0, false, 5.0 )
		DebugDrawSphereRGB( trInitial.endPos, 3, 0, 255, 0, false, 5.0 )
					...
					if ( !trLedge.startSolid && trLedge.fraction < 0.99 )
					{
						//Found!
							...
							if ( VANTAGE_COMPANION_ORDER_DEBUG_DRAW )
							{
								...
								DebugDrawSphereRGB( trLedge.endPos, 8, int(COLOR_LIGHT_GREEN.x), int(COLOR_LIGHT_GREEN.y), int(COLOR_LIGHT_GREEN.z), false, 5.0 )
							}
						#endif //#if DEVELOPER
	...
	if ( isEchoPerched )
	{
		...
		currentCompanionData.dirToNextPoint = toNextPoint/currentCompanionData.distanceToNextPoint
			...
			if ( VANTAGE_COMPANION_ORDER_DEBUG_DRAW )
			{
				...
				DebugDrawSphereRGB( currentCompanionData.orderRoute[0], 10, int(COLOR_MAGENTA.x), int(COLOR_MAGENTA.y), int(COLOR_MAGENTA.z), false, 5 )
	...
	currentCompanionData.dirToNextPoint = toNextPoint/currentCompanionData.distanceToNextPoint
		...
		if ( VANTAGE_COMPANION_ORDER_DEBUG_DRAW )
		{
			...
			DebugDrawSphereRGB( orderPosData.orderPos, 10, 255, 255, 255, false, 5 )
	...
	if ( trHullDown.fraction < 1.0 )
	{
		...
		if ( VANTAGE_COMPANION_ORDER_DEBUG_DRAW )
		{
			...
			if ( !traceDownHit )
			{
				...
				DebugDrawLineRGB( proposedCompanionPos, trHullDown.endPos, int(drawColor.x), int(drawColor.y), int(drawColor.z), false, drawTime )
			}
			else
			{
				...
				DebugDrawBoxRGB( adjustedPoint, VANTAGE_COMPANION_BOUND_MINS, VANTAGE_COMPANION_BOUND_MAXS, int(drawColor.x), int(drawColor.y), int(drawColor.z), 1, drawTime )
			}
		}
	#endif //#if DEVELOPER
...
void function DebugDrawCompanion( entity echoEnt )
{
	if ( IsValid( echoEnt ) )
	{
		DebugDrawBoxRGB( echoEnt.GetOrigin(), VANTAGE_COMPANION_BOUND_MINS, VANTAGE_COMPANION_BOUND_MAXS, 255, 150, 0, 50, 2*UPDATE_RATE )
		DebugDrawSphereRGB( echoEnt.GetOrigin(), 4, 0, 100, 255, false, 2*UPDATE_RATE )
	}
}
	...
	if ( player.GetPlayerNetInt( VANTAGE_COMPANION_STATE_NETINT ) != eCompanionState.PERCHED  )
	{
		...
		/////////////////////////////////////////////////////////////////////////
			...
			if ( VANTAGE_COMPANION_DEBUG_DRAW || VANTAGE_COMPANION_ORDER_DEBUG_DRAW )
			{
				...
				DebugDrawSphereRGB( echoGoalPos, 2, 255, 0, 0, true, UPDATE_RATE )
				...
				{
					DebugDrawLineRGB( currentCompanionData.prevPos, echoCurrentPos, int(COLOR_CYAN.x), int(COLOR_CYAN.y), int(COLOR_CYAN.z), false, 5.0 )
...
void function SetCompanionPerched(  bool setPerched, entity companion, bool initialSpawn=false )
{
	...
	{
		//companion.Solid // S21DEDI: keep non-solid on deploy -- see NotSolid note in CreateCompanionEntity
		//mover.ClearParent
		//mover.Show
		//companion.Show
	}
	...
	foreach( offset in followCheckOffsets )
	{
		...
		TraceResults trHull = TraceHull( echoPos, traceEnd, VANTAGE_COMPANION_BOUND_MINS, VANTAGE_COMPANION_BOUND_MAXS, ignoreEnts, ECHO_ORDER_TRACE_COL_MASK, TRACE_COLLISION_GROUP_PLAYER, UP_VECTOR, companion)
			...
			if ( VANTAGE_COMPANION_FOLLOW_DEBUG_DRAW )
			{
				...
				DebugDrawBoxRGB( trHull.endPos, VANTAGE_COMPANION_BOUND_MINS, VANTAGE_COMPANION_BOUND_MAXS, r, g, 0, 10, 5.0 )
				DebugDrawLineRGB( echoPos, trHull.endPos, r, g, 0, false, 5.0 )
			}
		#endif //#if DEVELOPER
...
entity function CreateCompanionEntity(  entity player )
{
	...
	companionProp.NotSolid() // S21DEDI: S3 does not honor the S21 SOLID_BBOX + kv.contents combo (bat stayed player-solid, blocking JetDrive flight); S21 bat is never player-solid to begin with
		...
		if( IsValid( vantagePlayer ) )
		{
			if ( vantagePlayer in file.echoData )
			{
				...
				if ( DEBUG_VISIBILITY )
				{
					...
					{
						DebugDrawSphereRGB( echoEnt.GetOrigin(), 10, 0, 255, 50, true, 0.1 )
					}
					else
					{
						DebugDrawSphereRGB( echoEnt.GetOrigin(), 7, 255, 50, 0, true, 0.1 )
