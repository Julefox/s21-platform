...
void function Smokescreen( SmokescreenStruct smokescreen, entity ownerEnt )
{
	...
	if ( SMOKESCREEN_DEBUG )
		DebugDrawCircleRGB( fxInfo.center, <0,0,0>, fxInfo.radius + 240.0, int( COLOR_YELLOW.x ), int( COLOR_YELLOW.y ), int( COLOR_YELLOW.z ), true, smokescreen.lifetime )
#endif
...
void function SmokescreenAffectsEntitiesInArea( SmokescreenStruct smokescreen, SmokescreenFXStruct fxInfo )
{
	...
	while ( Time() - startTime <= smokescreen.lifetime )
	{
		...
		if ( SMOKESCREEN_DEBUG )
		{
			DebugDrawCircleRGB( fxInfo.center, <0,0,0>, smokescreen.damageInnerRadius, int( COLOR_RED.x ), int( COLOR_RED.y ), int( COLOR_RED.z ), true, tickRate )
			DebugDrawCircleRGB( fxInfo.center, <0,0,0>, smokescreen.damageOuterRadius, int( COLOR_RED.x ), int( COLOR_RED.y ), int( COLOR_RED.z ), true, tickRate )
			DebugDrawLineRGB( fxInfo.center, smokescreen.origin, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, tickRate )
		}
#endif
...
#if DEVELOPER
		if ( SMOKESCREEN_DEBUG )
			DebugDrawCircleRGB( position, <0,0,0>, smokescreen.fxXYRadius, int( COLOR_BLUE.x ), int( COLOR_BLUE.y ), int( COLOR_BLUE.z ), true, smokescreen.lifetime )
#endif
