	...
	for( int trace = 0; trace < TRACE_COUNT; trace++ )
	{
		...
		frac = result.fraction
			...
			#endif // SERVER || CLIENT
			{
				...
				DebugDrawLineRGB( perchPosition, result.endPos, 0, 255, 0, true, 3 )
				DebugDrawLineRGB( result.endPos, traceOrigin, 255, 0, 0, true, 3 )
			}
		#endif //DEV
...
void function ClusterDebug()
{
	...
	while( true )
	{
		...
		foreach( cluster in waypointClusterArray )
		{
			...
			if ( nearCluster )
			{
				DebugDrawCircleRGB( cluster.clusterPos, <180,0,0>, 64, 255, 0, 0, true, 0.1 )
			}
			else
			{
				DebugDrawCircleRGB( cluster.clusterPos, <180,0,0>, 64, 128, 0, 255, true, 0.1 )
			}
		...
		foreach( birdCluster in file.birdClusterArray )
		{
			...
			{
				DebugDrawCircleRGB( birdCluster.origin, <180,0,0>, CLUSTER_SPAWN_MAX_RADIUS, 255, 0, 0, true, 0.1 )
				continue
			}
			...
			if ( birdCluster.owner != GetPlayerArray()[0]  )
			{
				...
				if ( birdCluster.triggered )
					DebugDrawCircleRGB( birdCluster.origin + <0,0,8>, <180,0,0>, CLUSTER_SPAWN_MAX_RADIUS, 64, 64, 192, true, 0.1 )
				else
					DebugDrawCircleRGB( birdCluster.origin + <0,0,8>, <180,0,0>, CLUSTER_SPAWN_MAX_RADIUS, 64, 192, 64, true, 0.1 )
			}
			else
			{
				float dist = Distance( birdCluster.owner.GetOrigin(), birdCluster.origin )
				...
				if ( birdCluster.triggered )
					DebugDrawCircleRGB( birdCluster.origin, <180,0,0>, CLUSTER_SPAWN_MAX_RADIUS, 0, 128, 255, true, 0.1 )
				else
					DebugDrawCircleRGB( birdCluster.origin, <180,0,0>, CLUSTER_SPAWN_MAX_RADIUS, 128, 255, 0, true, 0.1 )
			}
		}
