...
void function BreakableCrystals_OnDamaged( entity crystal, var damageInfo )
{
    ...
    if ( crystalData.topSection.state == CrystalSectionState.Broken )
    {
        ...
        crystalData.bottomSection.health -= damage
            ...
            DebugDrawSphereRGB( damagePosition, 25.0, int(COLOR_MAGENTA.x), int(COLOR_MAGENTA.y), int(COLOR_MAGENTA.z), true, 30.0 )
        #endif
    }
    else
    {
        if ( BreakableCrystals_ShouldDamageTop( crystal, damagePosition, crystalData ) )
        {
            ...
            crystalData.topSection.health -= damage
                ...
                DebugDrawSphereRGB( damagePosition, 25.0, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 30.0 )
            #endif
        }
        else
        {
            ...
            crystalData.bottomSection.health -= damage
             ...
             #if DEBUG_CRYSTALS
                 printt( "BC: Bottom new health -", crystalData.bottomSection.health )
                 DebugDrawSphereRGB( damagePosition, 25.0, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 30.0 )
             #endif
        }
    }
