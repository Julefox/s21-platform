...
void function Drone_RotatorThread( DroneData droneData )
{
	...
	while( true )
	{
		...
		vector fwdToLookAhead = FlattenNormalizeVec( lookAheadPos - currentPos )
			...
			{
				DebugDrawLineRGB( currentPos, (currentPos + forwardVec), int(COLOR_WHITE.x), int(COLOR_WHITE.y), int(COLOR_WHITE.z), true, DRONE_BANK_UPDATE_TIME )
				DebugDrawLineRGB( currentPos, (currentPos + fwdToLookAhead), int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, DRONE_BANK_UPDATE_TIME )

				DebugDrawSphereRGB( (currentPos + forwardVec), 8.0, int(COLOR_WHITE.x), int(COLOR_WHITE.y), int(COLOR_WHITE.z), true, DRONE_BANK_UPDATE_TIME )
				DebugDrawSphereRGB( lookAheadPos, 8.0, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, DRONE_BANK_UPDATE_TIME )

				DebugDrawSphereRGB( lastNodeEnt.GetOrigin(), 16.0, int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), true, (DRONE_BANK_UPDATE_TIME * 2) )
				DebugDrawSphereRGB( nextNodeEnt.GetOrigin(), 16.0, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, (DRONE_BANK_UPDATE_TIME * 2) )
			}
		#endif
...
void function DebugDrawSafeDronePathsByPathArray( array< array<entity> > dronePaths, float displayTime = 120, vector color = < 255, 0, 0 > )
{
	foreach ( array < entity > path in GetAllDronePathsInCircle( dronePaths ) )
	{
		...
		for ( int j; j < numNodes; j++ )
		{
			...
			DebugDrawLineRGB( path[j].GetOrigin(), path[ j_next ].GetOrigin(), int(color.x), int(color.y), int(color.z), true, displayTime )
		}
	}
}
	...
	for ( int i = numInitTrackNodes - 1; i >= 0; i-- )
	{
		...
		while ( true )
		{
			...
			if ( !IsValid( nextNode ) )
			{
				...
				DebugDrawSphereRGB( curNode.GetOrigin(), 32, 255, 125, 0, true, 600 )
