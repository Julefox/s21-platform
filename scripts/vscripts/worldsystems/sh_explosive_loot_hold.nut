...
void function ExplosiveHoldAnimation_Thread( entity panel, entity player, ExplosiveHoldGrenadeData data )
{
	...
	foreach ( entity panelEnt in explosiveHold.GetChildren() )
	{
...
void function ExplosiveHoldDoor_DoorResponse_Thread ( entity panel, bool wasExploded )
{
	...
	foreach ( entity door in panel.GetChildren() )
	{
	...
	if ( wasExploded )
	{
		...
		foreach ( entity otherPanel in explosiveHold.GetChildren() )
		{
			if ( otherPanel != panel && otherPanel.GetScriptName() == EXPLOSIVE_HOLD_PANEL_SCRIPTNAME )
			{
				foreach ( entity door in otherPanel.GetChildren() )
				{
					string scriptName = door.GetScriptName()
	...
	if ( wasExploded )
	{
		...
		if( IsValid( panel ) )
		{
			...
			foreach ( entity vfxEntHelper in panel.GetChildren() )
			{
			...
			array< entity > children = panel.GetChildren()
...
void function DEV_LootHold_ShowAll( float timeToShow = 30.0 )
{
	...
	{
		DebugDrawSphereRGB( explosiveHold.GetOrigin(), 256, 255, 0, 0, true, timeToShow )
		...
		if( ExplosiveHold_NoPlaceVols_Enabled() )
		{
			foreach( vol in data.noPlaceVolumes )
			{
				DebugDrawCylinderRGB( vol.GetOrigin(), < -90, 0, 0 >, EXPLOSIVEHOLD_NOPLACEVOL_RADIUS, EXPLOSIVEHOLD_NOPLACEVOL_HEIGHT, 255, 255, 0, true, timeToShow )
			}
		}
		...
		if( ExplosiveHold_BreachTriggers_Enabled() )
		{
			...
			{
				DebugDrawCylinderRGB( trig.GetOrigin(), < -90, 0, 0 >, EXPLOSIVEHOLD_BREACHTRIGGER_RADIUS, EXPLOSIVEHOLD_BREACHTRIGGER_HEIGHT, 255, 128, 0, true, timeToShow )
			}
		}
	}
