...
void function OnWeaponActivate_ability_translocation( entity weapon )
{
	...
	#if CLIENT
		if ( !(InPrediction() && IsFirstTimePredicted()) && !IsValid( GetCurrentTranslocationProjectile( owner, weapon ) ) )
			return
	#endif

	if ( !IsValid( GetCurrentTranslocationProjectile( owner, weapon ) ) )
	{
		#if SERVER
	...
	if ( weapon.w.translocate_lifetimeActive == true )
		return
	weapon.w.translocate_lifetimeActive = true
...
void function OnWeaponDeactivate_ability_translocation( entity weapon )
{
	...
	{
		if ( IsValid( owner ) && IsValid( GetCurrentTranslocationProjectile( owner, weapon ) ) )
			return
...
void function TranslocationLifetimeThread( entity owner, entity weapon )
{
	...
	OnThreadEnd( void function() : ( owner, weapon, rui, haveLockedForToss, ownerName ) {
		if ( IsValid( weapon ) )
			weapon.w.translocate_lifetimeActive = false
		#if TRANSLOCATION_ADDITIONAL_DEBUG
...
#if SERVER || CLIENT
bool function OnWeaponRedirectProjectile_ability_translocation( entity weapon, table params )
{
	entity owner = weapon.GetWeaponOwner()
	entity srcProjectile = expect entity( params.projectile )
	vector srcPos = expect vector( params.projectilePos )
	...
	if ( Time() < srcProjectile.GetProjectileCreationTimeServer() + dropToGroundMinimumTime )
		return false

	if ( srcProjectile.HasWeaponMod( "redirect_mod" ) )
		return false
	...
	#if TRANSLOCATION_ADDITIONAL_DEBUG
		vector ownerPos = owner.GetOrigin(), ownerAng = owner.EyeAngles()
		printf( "TRANSLOCATION:(" + owner.GetPlayerName() + ") Redirect at setpos %f %f %f", srcPos.x, srcPos.y, srcPos.z )
	#endif

	weapon.StartCustomActivity( "ACT_VM_HITCENTER", WCAF_NONE )

	WeaponPrimaryAttackParams attackParams
	attackParams.pos = srcPos
	...
	#if SERVER
		entity projectile = ThrowDeployable( weapon, attackParams, 1, OnProjectilePlanted, null, null, PROJECTILE_NOT_PREDICTED )
	#else
		entity projectile = ThrowDeployable( weapon, attackParams, 1, OnProjectilePlanted, null, null )
	#endif
	...
	if ( projectileSound != "" )
		...
		weapon.w.translocate_projectileThrowPathTaken.append( srcPos )
		owner.SetPlayerNetEnt( "Translocation_ActiveProjectile", projectile )
		...
		thread DropToGroundFXThread( owner, projectile, srcProjectile, srcPos )
		if ( IsValid( srcProjectile ) )
			srcProjectile.Destroy()
		weapon.w.translocate_initialProjectile = null
...
void function TranslocationTossedThread( entity owner, entity weapon )
{
	...
	while( true )
	{
		...
		if ( !IsValid( currentProjectile ) )
		{
			...
			if ( GetLobaTacticalAllowDropToGround() && !weapon.w.translocate_hasProjectilePlanted )
			{
				if ( owner.IsInputCommandPressed( IN_ATTACK ) || owner.IsInputCommandPressed( IN_OFFHAND1 ) )
				{
					table redirectParams
					redirectParams.projectile <- currentProjectile
					redirectParams.projectilePos <- currentProjectile.GetOrigin()
					OnWeaponRedirectProjectile_ability_translocation( weapon, redirectParams )
				}
			}
...
bool function IsLandingPositionValid( vector position, entity player, array<int> realms, entity projectile )
{
	...
	TraceResults tr = TraceLine( position, position+<0, 0, -1000>, [],TRACE_MASK_PLAYERSOLID, TRACE_COLLISION_GROUP_PLAYER )
	if ( tr.fraction == 1.0 )
	{
		printf( "TRANSLOCATION:(" + player.GetPlayerName() + ") Can fall to doom at setpos %f %f %f", position.x, position.y, position.z )

		if ( IsEventFinale() )
		{
			entity tacticalAbility = player.GetOffhandWeapon( OFFHAND_LEFT )
			if ( IsValid( tacticalAbility ) && tacticalAbility.HasMod( "s17cr_p0_cooldown_mod" ) )
			{
				...
				{
					if ( trigger.GetScriptName() == "s17_chr_platform_fall_trigger" )
					{
						return true
					}
				}
	...
	{
		printf( "TRANSLOCATION:(" + player.GetPlayerName() + ") Slip or OOB trigger at setpos %f %f %f", position.x, position.y, position.z )
		return false
	}
		...
		realms, TRACE_MASK_PLAYERSOLID ) )
	{
		...
		{
			// don't allow teleport to or through kill triggers
			printf( "TRANSLOCATION:(" + player.GetPlayerName() + ") Kill trigger between player and setpos %f %f %f", position.x, position.y, position.z )
			return false
		}
	}

	if ( GetMapName() == "mp_rr_salham" )
	{
		foreach ( entity trigger in GetTriggersByClassesInRealms(
			["trigger_multiple"],
			player.GetOrigin(), position,
			realms, TRACE_MASK_PLAYERSOLID ) )
		{
			if ( trigger.GetScriptName() == "s17_respawn_trigger" )
				return false
		}
	}

	entity playerTrident = HoverVehicle_GetVehicleOccupiedByPlayer( player )
	if ( IsValid( playerTrident ) )
	{
		float tridentHeight = playerTrident.GetBoundingMaxs().z - playerTrident.GetBoundingMins().z
		TraceResults heightTrace = TraceLine( position, position + < 0, 0, tridentHeight>, [ player, projectile ], TRACE_MASK_PLAYERSOLID, TRACE_COLLISION_GROUP_PLAYER, null, true )
		if ( heightTrace.fraction < 1.0 )
		{
			entity hitEnt = heightTrace.hitEnt
			if ( IsValid( hitEnt ) && hitEnt == playerTrident )
				return false
		}
	}
...
void function OnProjectilePlanted( entity projectile, DeployableCollisionParams collisionParams )
{
	...
	if( file.canceledTeleports.contains( owner ) )
	{
		...
		projectile.AddGrenadeStatusFlag( GSF_PLANTED )
...
void function TranslocatePlayerThread( entity owner, entity weapon, entity projectile, DeployableCollisionParams collisionParams, vector projectilePos )
{
	...
	EndSignal( projectile, "OnDestroy" )
		...
		DebugDrawSphereRGB( projectilePos, 5.0, int(COLOR_BLACK.x), int(COLOR_BLACK.y), int(COLOR_BLACK.z), false, TRANSLOCATION_DEBUG_TIMEOUT )
		for ( int i = 1; i < numPathPts; ++i )
		{
			...
			DebugDrawSphereRGB( pathPos, 4.0, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), false, TRANSLOCATION_DEBUG_TIMEOUT )
	...
	if ( IsEventFinale() && weapon.HasMod( "s17cr_p0_cooldown_mod" ) )
	{
		foreach ( entity trigger in GetTriggersByClassesInRealms(
			["trigger_multiple"],
			owner.GetOrigin(), projectilePos,
			owner.GetRealms(), TRACE_MASK_PLAYERSOLID ) )
		{
			if ( trigger.GetScriptName() == "s17_chr_platform_fall_trigger" )
			{
				ownerFinalPos = <-10242.84, -8226.31, 36297.1>
			}
		}
	}

	Vehicle_KickPlayer_ForAbility( owner )
	...
	StartParticleEffectInWorldForRealms( GetParticleSystemIndex( TRANSLOCATION_WARP_WORLD_FX ), ownerOrigPos, <0, ang.y, 0>, owner )
	StartParticleEffectInWorldForRealms( GetParticleSystemIndex( TRANSLOCATION_WARP_WORLD_FX ), ownerFinalPos, <0, ang.y, 0>, owner )
...
vector ornull function TryTranslocateSpots( entity owner, entity weapon, entity projectile, array<vector> candidateSpots, vector projectilePos )
{
		...
		{
			DebugDrawSphereRGB( candidateSpots[i], 5.5, 204, 153, 255, false, TRANSLOCATION_DEBUG_TIMEOUT )
		}
	#endif
...
void function DropToGroundFXThread( entity player, entity existingProjectile, entity predictedRedirectedProjectile, vector currentProjectilePos )
{
	...
	#if SERVER
		lineFX.SetOwner( player )
		lineFX.kv.VisibilityFlags = ENTITY_VISIBLE_TO_FRIENDLY | ENTITY_VISIBLE_TO_ENEMY
	#endif
