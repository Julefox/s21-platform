...
const string BLACK_MARKET_ENTITY_CLASS = "prop_loot_grabber"
...
void function LobaUltimateBlackMarket_LevelInit()
{
		...
		RegisterDefaultMinimapPackage( BLACK_MARKET_ENTITY_CLASS, $"", void function( entity ent, var rui ) {} )
...
PlacementInfo function GetPlacementInfo( entity player )
{
	...
	info.angles = AnglesCompose( angles, <0, 180, 0> )
		...
		DebugDrawLineRGB( eyePos + viewVec * min( BLACK_MARKET_PLACEMENT_RANGE_MIN, range ), fwdResults.endPos, 0, 255, 0, true, 1.0 )
		DebugDrawLineRGB( fwdResults.endPos, eyePos + viewVec * range, 255, 0, 0, true, 1.0 )
		DebugDrawLineRGB( fwdResults.endPos, info.origin, 0, 0, 255, true, 1.0 )
		DebugDrawLineRGB( player.GetOrigin(), player.GetOrigin() + (AnglesToForward( angles ) * BLACK_MARKET_PLACEMENT_RANGE_MAX), 0, 255, 0, true, 1.0 )
		DebugDrawLineRGB( eyePos + <0, 0, 8>, eyePos + <0, 0, 8> + (viewVec * BLACK_MARKET_PLACEMENT_RANGE_MAX), 0, 255, 0, true, 1.0 )
	#endif
	...
	if ( info.success )
	{
		...
		#if BLACK_MARKET_DEBUG_DRAW_PLACEMENT
			DebugDrawLineRGB( info.origin, info.origin + (osaRight * 64), 0, 255, 0, true, 1.0 )
			DebugDrawLineRGB( info.origin, info.origin + (osaForward * 64), 0, 0, 255, true, 1.0 )
		#endif
		...
		foreach ( vector testOffset in groundTestOffsets )
		{
			...
			#if BLACK_MARKET_DEBUG_DRAW_PLACEMENT
				DebugDrawLineRGB( testPos + (osaUp * BLACK_MARKET_PLACEMENT_MAX_GROUND_DIST), traceResult.endPos, 255, 0, 0, true, 1.0 )
			#endif
...
void function BlackMarketDeployThread( entity owner, PlacementInfo placementInfo )
{
	...
	entity blackMarket
	{
		...
		blackMarket.SetImpactEffectColorID( COLORID_FX_LOOT_TIER1 )
		...
		if ( blackMarket.GetRealms().len() == 0 )
			blackMarket.AddToRealm( eRealms.DEFAULT )
...
bool function IsBlackMarketDevice( entity ent )
{
	...
	return ent.GetScriptName() == BLACK_MARKET_SCRIPTNAME
}
#endif
...
void function OnCharacterButtonPressed( entity player )
{
	...
	Remote_ServerCallFunction( BLACK_MARKET_TRY_PICKUP_CMD, useEnt )
}
#endif
