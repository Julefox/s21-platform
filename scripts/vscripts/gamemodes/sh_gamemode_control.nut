...
void function Control_OnSpawnedSkydiveLauncherEditorClass( entity prop )
{
	...
	if ( IsValid( prop ) )
	{
		CreateNonExpiringAirdropBadPlace( prop.GetOrigin(), CONTROL_SKYDIVE_LAUNCHER_AIRDROP_BAD_PLACE_RADIUS )
			...
			{
				DebugDrawSphereRGB( prop.GetOrigin(), CONTROL_SKYDIVE_LAUNCHER_AIRDROP_BAD_PLACE_RADIUS, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, CONTROL_DEBUG_DRAW_DISPLAY_TIME )
			}
		#endif // DEV
	}
}
...
void function OnEntitiesDidLoad_MapSetup()
{
	...
	// created, so this cannot be set from Control_Init
...
void function ParseControlPointDataForChosenMap()
{
	...
	foreach ( controlPoint in file.controlPoints)
	{
		...
		CreateNonExpiringAirdropBadPlace( controlPoint.location, radius )
			...
			if ( CONTROL_DISPLAY_DEBUG_DRAWS )
			{
				DebugDrawSphereRGB( controlPoint.location, float( radius ), int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, CONTROL_DEBUG_DRAW_DISPLAY_TIME )
			}
		#endif // DEV
	}
}
														...
														vector overrideColor)
{
	...
	#if UI
	announcementData.startTime = UITime()
	#else
...
{
	// Population fairness only matters when mixtape abandon ban is playlist-on;
	// otherwise true->false nets a false "Abandon Penalty no longer active" toast.
	if ( !GamemodeUtility_GetMixtapeAbandonPenaltyActive() )
		return
	...
	if ( GetGameState() == eGameState.Playing && GetGlobalNetBool( "mixtape_isLeaverPenaltyEnabledForMatch" ) )
	{
		...
		bool stillEnabled = ( !areAlliancesUneven && !isPlayerCountLow )
		if ( stillEnabled != GetGlobalNetBool( "mixtape_isLeaverPenaltyEnabledForMatch" ) )
			SetGlobalNetBool( "mixtape_isLeaverPenaltyEnabledForMatch", stillEnabled )
	}
}
#endif // SERVER
		...
		foreach ( spawnWavePlayer in playersPendingSpawn )
		{
			if ( IsValid( spawnWavePlayer ) )
			{
				...
				if ( isFirstSpawn )
				{
					...
					if ( music.len() > 0 )
						PlayMusicToPlayer( spawnWavePlayer, music )
	...
	foreach ( spawn in spawnList )
	{
		if ( spawn.HasKey( "control_teamnumber" ) )
		{
			...
			if ( int(spawn.kv.control_teamnumber) == ALLIANCE_NONE )
			{
				...
				filteredSpawnList.append( spawn )
					...
					if ( CONTROL_DISPLAY_DEBUG_DRAWS && showSpawnPoints )
						DebugDrawCylinderRGB( spawn.GetOrigin(), spawn.GetAngles(), 20, 128, int( COLOR_YELLOW.x ), int( COLOR_YELLOW.y ), int( COLOR_YELLOW.z ), true, SPAWNPOINT_DISPLAY_TIME )
			...
			if ( int(spawn.kv.control_teamnumber) == playerAlliance )
			{
				...
				#if DEVELOPER
					if ( CONTROL_DISPLAY_DEBUG_DRAWS && showSpawnPoints )
						DebugDrawCylinderRGB( spawn.GetOrigin(), spawn.GetAngles(), 20, 128, int( COLOR_GREEN.x ), int( COLOR_GREEN.y ), int( COLOR_GREEN.z ), true, SPAWNPOINT_DISPLAY_TIME )
			...
			#if DEVELOPER
				if ( CONTROL_DISPLAY_DEBUG_DRAWS && showSpawnPoints )
					DebugDrawCylinderRGB( spawn.GetOrigin(), spawn.GetAngles(), 20, 128, int( COLOR_RED.x ), int( COLOR_RED.y ), int( COLOR_RED.z ), true, SPAWNPOINT_DISPLAY_TIME )
		...
		else
		{
			...
			#if DEVELOPER
				if ( CONTROL_DISPLAY_DEBUG_DRAWS && showSpawnPoints )
					DebugDrawCylinderRGB( spawn.GetOrigin(), spawn.GetAngles(), 20, 128, int( COLOR_BLUE.x ), int( COLOR_BLUE.y ), int( COLOR_BLUE.z ), true, SPAWNPOINT_DISPLAY_TIME )
...
void function Control_SpawnDebugging_Thread( entity player, entity finalSpawn )
{
	...
	OnThreadEnd(
		void function() : ( player, finalSpawn )
		{
			...
			if ( !IsAlive( player ) )
			{
				...
				if ( CONTROL_DISPLAY_DEBUG_DRAWS )
					DebugDrawCylinderRGB( finalSpawn.GetOrigin(), finalSpawn.GetAngles(), 20, 128, int( COLOR_RED.x ), int( COLOR_RED.y ), int( COLOR_RED.z ), true, CONTROL_DEBUG_DRAW_DISPLAY_TIME )
	...
	if ( CONTROL_DISPLAY_DEBUG_DRAWS )
		DebugDrawCylinderRGB( finalSpawn.GetOrigin(), finalSpawn.GetAngles(), 20, 128, int( COLOR_GREEN.x ), int( COLOR_GREEN.y ), int( COLOR_GREEN.z ), true, CONTROL_DEBUG_DRAW_DISPLAY_TIME )
}
#endif // SERVER && DEV
#endif
...
array<entity> function Control_GetValidSpawnpoints( array<entity> spawnpoints, entity player, int amount )
{
	...
	foreach ( spawn in spawnpoints )
	{
		...
		if ( Control_IsSpawnpointValid( spawn, player ) )
		{
			...
			#if DEVELOPER
				if ( CONTROL_DISPLAY_DEBUG_DRAWS && showSpawnPoints )
					DebugDrawCylinderRGB( spawn.GetOrigin(), spawn.GetAngles(), SPAWNPOINT_RADIUS, SPAWNPOINT_HEIGHT, int( COLOR_YELLOW.x ), int( COLOR_YELLOW.y ), int( COLOR_YELLOW.z ), true, SPAWNPOINT_DISPLAY_TIME )
			#endif // DEV
		}
	#if DEVELOPER
		else if ( CONTROL_DISPLAY_DEBUG_DRAWS && showSpawnPoints )
		{
			DebugDrawCylinderRGB( spawn.GetOrigin(), spawn.GetAngles(), SPAWNPOINT_RADIUS, SPAWNPOINT_HEIGHT, int( COLOR_RED.x ), int( COLOR_RED.y ), int( COLOR_RED.z ), true, SPAWNPOINT_DISPLAY_TIME )
		}
	#endif // DEV
	}
...
void function Control_SetupVehicleSummonPlatform( entity summonPlatform )
{
	...
	CreateNonExpiringAirdropBadPlace( summonPlatform.GetOrigin() + <0,0,200>, CONTROL_VEHICLE_AIRDROP_BAD_PLACE_RADIUS )
		...
		if ( CONTROL_DISPLAY_DEBUG_DRAWS )
		{
			DebugDrawSphereRGB( summonPlatform.GetOrigin() + <0,0,200>, CONTROL_VEHICLE_AIRDROP_BAD_PLACE_RADIUS, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, CONTROL_DEBUG_DRAW_DISPLAY_TIME )
		}
	#endif // DEV
...
void function Control_OnInGameMapHide()
{
	...
	//RefreshSoundSystem(); //TODO: confirm this native exists on our S3-modified engine before adopting client's version
}
#endif // CLIENT
...
bool function Control_EvolveWeapon( entity player, entity weapon, int newTier, bool canSecondarySlotWeaponEvo )
{
	...
	if ( IsValid( weapon ) )
	{
		...
		if ( SURVIVAL_Loot_IsRefValid( newWeaponRef ) && SURVIVAL_Loot_GetLootDataByRef( newWeaponRef ).lootType == eLootType.MAINWEAPON )
		{
			...
			bool energized = weapon.IsEnergizeWeapon() && weapon.GetEnergizeState() == ENERGIZE_ENERGIZED
		...
		foreach ( point in file.spawnWaypoints )
		{
			if ( IsValid( point ) )
			{
				...
				if ( Control_IsSpawnWaypointIndexAnObjective( waypointTypeIndex ) )
				{
						...
						if ( CONTROL_DISPLAY_DEBUG_DRAWS )
						{
							DebugDrawSphereRGB( point.GetOrigin(), MIN_DIST_FROM_OBJECTIVES, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 1.0 )
						}
					#endif // DEV
...
int function Control_GetMRBPlacementStateFromHomeBasePositionChecks( int currentPlacementState, bool isEnemyHomebase, float minDistFromHomeBase, array< vector > homebasePositions, vector mrbPosition )
{
	...
	foreach ( position in homebasePositions )
	{
			...
			if ( CONTROL_DISPLAY_DEBUG_DRAWS )
			{
				DebugDrawSphereRGB( position, minDistFromHomeBase, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, 1.0 )
			}
		#endif // DEV
	...
	OnThreadEnd(
		function() : ( player )
		{
			if ( IsValid( player ) )
			{
				...
				{
					Remote_CallFunction_NonReplay( alliancePlayer, "GamemodeUtility_ServerCallback_PlayerJoinedMatchInProgress", player )
					Remote_CallFunction_NonReplay( player, "ServerCallback_Control_ShowSpawnSelection" )
				}
			}
		}
