...
// loadoutselection_rotation_start " 10:00:00 -08:00" ( This should coincide with when your mode goes live, it is used to calculate the loadout slot rotations)
...
global function LoadoutSelection_SetFreeroamForceEnable
global function LoadoutSelection_IsFreeroamForceEnabled
global function LoadoutSelection_FullInit
global function LoadoutSelection_EnsureReadyForMenu
...
global function LoadoutSelection_HasPlayerSelectedLoadout
...
global function LoadoutSelection_IsMenuEnabled
...
struct {
	...
	// Freeroam / training: force-enable stock loadout UI without Control playlist.
	bool freeroamForceEnable = false
	bool didFullInit = false
...
void function LoadoutSelection_Init()
{
	...
	LoadoutSelection_FullInit()
}

// Idempotent full init for playlist enable OR freeroam force-enable.
void function LoadoutSelection_FullInit()
{
	if ( file.didFullInit )
		return
	file.didFullInit = true
	...
	LoadoutSelection_InitWeaponData()
		...
		// only -- must run during BeginRegisteringFunctions, never mid-match FullInit.
	#endif // CLIENT || SERVER

	if ( file.freeroamForceEnable )
		printt( "[LoadoutSelection] FullInit complete (freeroam force enable)" )
	else
		printt( "[LoadoutSelection] FullInit complete" )
}

// Cafe freeroam / aim-trainer: turn on stock loadout selection system + menu.
void function LoadoutSelection_SetFreeroamForceEnable( bool enable )
{
	file.freeroamForceEnable = enable
	if ( enable && !file.didFullInit )
		LoadoutSelection_FullInit()
}

bool function LoadoutSelection_IsFreeroamForceEnabled()
{
	return file.freeroamForceEnable
}

// Force enable + full init + populate loadout rows (safe if already done).
void function LoadoutSelection_EnsureReadyForMenu()
{
	LoadoutSelection_SetFreeroamForceEnable( true )

	#if CLIENT || SERVER
		// First freeroam open often ran FullInit while client loot was empty, so
		// weaponsInLoadout stayed empty and cards showed blank. Rebuild distribution
		// with freeroam trust of mp_weapon_* DT rows, then force re-populate.
		LoadoutSelection_FreeroamClearAndRedistribute()
		file.areLoadoutsPopulated = false
		LoadoutSelection_PopulateLoadouts()
		// maxLoadoutCountRegular is CLIENT/UI-only -- never touch it on SERVER.
		printt( format( "[LoadoutSelection] EnsureReady categories=%d populated=%s",
			file.loadoutCategories.len(),
			file.areLoadoutsPopulated ? "true" : "false" ) )
		#if CLIENT
			printt( format( "[LoadoutSelection] EnsureReady regularCount=%d", file.maxLoadoutCountRegular ) )
		#endif
	#endif

	#if SERVER
		// Seed rotation netvars after populate (maxLoadoutsPerCategory must be set).
		LoadoutSelection_SetUnixTimeSinceEventStarted()
	#endif
}

#if CLIENT || SERVER
// Clear weapons/consumables/equipment lists then re-run distribution (freeroam trust path).
void function LoadoutSelection_FreeroamClearAndRedistribute()
{
	if ( file.loadoutCategories.len() == 0 )
		return

	foreach ( LoadoutSelectionCategory cat in file.loadoutCategories )
	{
		foreach ( name, loadout in cat.loadoutContentsByNameTable )
		{
			loadout.weaponsInLoadout.clear()
			loadout.weaponLoadoutString = ""
			loadout.consumablesInLoadout.clear()
			loadout.consumablesLoadoutString = ""
			loadout.equipmentInLoadout.clear()
...
bool function IsUsingLoadoutSelectionSystem()
{
	if ( file.freeroamForceEnable )
		return true
	if ( GetCurrentPlaylistVarBool( "freeroam_loadout_menu_enable", false ) )
		return true
	return GetCurrentPlaylistVarBool( "loadoutselection_enable_loadouts", false )
}

bool function LoadoutSelection_IsMenuEnabled()
{
	if ( GetCurrentPlaylistVarBool( "freedm_ffa_active", false ) && GetCurrentPlaylistVarBool( "ffa_disable_loadouts", false ) )
		return false
	return IsUsingLoadoutSelectionSystem()
}
...
{
	// ALWAYS register here during _remote_functions_mp BeginRegisteringFunctions.
	// Freeroam force-enables the system later; remotes/netvars cannot be registered mid-match.
	...
	// Server -> UI open/close (used by freeroam_open_loadout and FreeDM/Control).
	...
	foreach ( item in file.loadoutCategories )
	{
		...
		foreach ( name, loadout in item.loadoutContentsByNameTable )
		{
			...
			while ( ( currentRow < numRows && GetDataTableString( distributionTable, currentRow, GetDataTableColumnByName( distributionTable, "loadout" ) ) == "" ) || currentRow == startingRow )
			{
				...
				// Freeroam/bridge: client SURVIVAL loot is often incomplete at force-init.
				// Still accept mp_weapon_* rows from the loadout DT so cards can show.
				if ( !SURVIVAL_Loot_IsRefValid( loadoutItem ) )
				{
					if ( !didOverrideWeapons && LoadoutSelection_IsFreeroamForceEnabled() && loadoutItem.find( "mp_weapon_" ) == 0 )
					{
...
void function LoadoutSelection_InitWeaponData()
{
	...
	int col_weaponRef   = GetDataTableColumnByName( dataTable, "weaponRef" )
		...
		for( int i = 0; i < numRows; ++i )
		{
			...
			if ( weaponRef != "" )
			{
				// Keep the DT row even when SURVIVAL loot is missing (client loot often empty
				// at freeroam force-init). Never call GetBaseMods on invalid refs.
				bool lootOk = SURVIVAL_Loot_IsRefValid( weaponRef )
					...
					if ( !( weaponRef in file.weaponUpgrades ) )
					{
						...
						if( file.weaponUpgrades[ weaponRef ].len() == 0 && lootOk )
						...
						if( defaultOptic != "" && file.weaponUpgrades[ weaponRef ].len() > 0 )
...
LoadoutSelectionItem function LoadoutSelection_GetLoadoutSelectionItemDataFromRef( string ref )
{
	...
	{
		// Freeroam/bridge: client loot may lack rows still listed in loadout DTs.
		item.ref = ref
		string baseWeapon = ref
		array<string> suffixes = [ "_gold", "_purpleset", "_blueset", "_whiteset", "_paintball" ]
		foreach ( string sfx in suffixes )
		{
			if ( baseWeapon.len() > sfx.len() && baseWeapon.slice( baseWeapon.len() - sfx.len() ) == sfx )
			{
				baseWeapon = baseWeapon.slice( 0, baseWeapon.len() - sfx.len() )
				break
			}
		}
		string printName = GetWeaponInfoFileKeyField_GlobalString( baseWeapon, "shortprintname" )
		if ( printName != "" )
			item.name = printName
		else
			item.name = baseWeapon
		item.desc = ""
		item.icon = $""
		return item
	}
...
string function LoadoutSelection_GetWeaponRefByIndex( int loadoutIndex, int weaponIndex )
{
	...
	// In Dev Assert if there is no valid weapon ref, in S21 return a blank ref so we just don't have a weapon but the game won't crash
...
bool function LoadoutSelection_HasPlayerSelectedLoadout( entity player )
{
	if ( !IsValid( player ) )
		return false

	return ( player in file.playerToSelectedLoadoutTable )
}
#endif // SERVER
				...
				if ( LoadoutSelection_GetWeaponCountByLoadoutIndex( loadoutIndex ) > weaponIndex )
				{
					...
					if ( element != null )
					{
						...
						Hud_SetVisible( element, true )
										...
										// Always surface a name even when client loot is missing this ref.
					if ( item.name != "" )
