...
Point function ShadowArmy_GetRevRespawnPoint( entity player, vector deathPos, bool shouldForceUseDeathPos )
{
	...
	if ( legendPlayers.len() > 0 )
	{
		...
		foreach ( array<entity> legendGroup in legendGroups )
		{
			...
			vector center = total / legendGroup.len()
				...
				if ( SHADOWARMY_DISPLAY_PLAYERSPAWN_DEBUG_DRAWS )
				{
					DebugDrawSphereRGB( center, 20, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, SHADOWARMY_DEBUG_DRAW_DISPLAY_TIME )
				}
			#endif
			...
			else
			{
				...
				if ( SHADOWARMY_DISPLAY_PLAYERSPAWN_DEBUG_DRAWS )
				{
					DebugDrawSphereRGB( spawnPoint.origin, spawnRadius, int(COLOR_PINK.x), int(COLOR_PINK.y), int(COLOR_PINK.z), true, SHADOWARMY_DEBUG_DRAW_DISPLAY_TIME )
				}
			#endif
			...
			float closestSqr = DistanceSqr( closestLegend.GetOrigin(), spawnPoint.origin )
				...
				if ( SHADOWARMY_DISPLAY_PLAYERSPAWN_DEBUG_DRAWS )
				{
					DebugDrawSphereRGB( closestLegend.GetOrigin(), 20, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, SHADOWARMY_DEBUG_DRAW_DISPLAY_TIME )
				}
			#endif
	...
	if ( IsValid( livingFullRevPlayer ) && !livingFullRevPlayer.Player_IsFreefalling() )
	{
...
void function OnPlayerPostRespawned( entity player )
{
	...
	switch( respawnForm )
	{
		case eShadowArmyRespawnForm.LIVING_LEGEND:
			...
			if ( SHADOWARMY_DISPLAY_PLAYERSPAWN_DEBUG_DRAWS )
			{
				DebugDrawSphereRGB( player.GetOrigin(), SHADOWARMY_SPAWN_DEBUG_DRAW_RADIUS, int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), true, SHADOWARMY_DEBUG_DRAW_DISPLAY_TIME )
			}
			#endif // DEV
			break
		case eShadowArmyRespawnForm.FULL_REVENANT:
			...
			thread ShadowArmy_FullRev_DelayedStatusEffectHandler( player )
				...
				if ( SHADOWARMY_DISPLAY_PLAYERSPAWN_DEBUG_DRAWS )
				{
					DebugDrawSphereRGB( player.GetOrigin(), SHADOWARMY_SPAWN_DEBUG_DRAW_RADIUS, int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), true, SHADOWARMY_DEBUG_DRAW_DISPLAY_TIME )
				}
			#endif // DEV
			break
		case eShadowArmyRespawnForm.SHADOW:
			...
			Remote_CallFunction_NonReplay( player, "ShadowArmy_ServerCallback_GivePlayerRepeatingEnemyMapScans" )
				...
				if ( SHADOWARMY_DISPLAY_PLAYERSPAWN_DEBUG_DRAWS )
				{
					DebugDrawSphereRGB( player.GetOrigin(), SHADOWARMY_SPAWN_DEBUG_DRAW_RADIUS, int(COLOR_ORANGE.x), int(COLOR_ORANGE.y), int(COLOR_ORANGE.z), true, SHADOWARMY_DEBUG_DRAW_DISPLAY_TIME )
				}
			#endif // DEV
