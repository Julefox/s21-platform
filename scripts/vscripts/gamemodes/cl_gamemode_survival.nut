...
global function Survival_ClientCombatHudInit
global function WaitingForPlayersOverlay_Destroy
...
void function Survival_ClientCombatHudInit()
{
	if ( file.combatHudInited )
		return
	file.combatHudInited = true
	...
	{
		Cl_Survival_AddClient( player )
		if ( IsValid( player.GetCockpit() ) )
			OnPilotCockpitCreated( player.GetCockpit(), player )
		thread InitSurvivalHealthBar()
	}

	// ClGamemodeSurvival_Init owns this for BR. Custom modes never run that
	// function, so armor/helmet/backpack HUD args stay at the RUI default
	// (white tier 0) until something else writes them.
	foreach ( equipSlot, data in EquipmentSlot_GetAllEquipmentSlots() )
	{
		if ( data.trackingNetInt != "" )
			AddCallback_OnEquipSlotTrackingIntChanged( equipSlot, EquipmentChanged )
	...
	entity visPlayer = GetLocalViewPlayer()
	if ( !IsValid( visPlayer ) )
		visPlayer = GetLocalClientPlayer()
	if ( IsValid( visPlayer ) )
	{
		Minimap_SetSizeScale( 1.0 )
		Minimap_UpdateMinimapVisibility( visPlayer )
	}

	printt( "[SUR-HUD] Survival_ClientCombatHudInit" )
}
...
void function Cl_Survival_AddClient( entity player )
{
	...
	thread WaitingForPlayersOverlay_Sync_THREAD()

	thread Survival_PublishLocalCharacter_THREAD()

	if ( GetMapName() == "mp_rr_district_mu1" )
		thread Survival_WorkInProgressMapSplash_THREAD( player )
}

const string WIP_MAP_SPLASH_SUBTEXT = "Custom map ported by @CafeFPS. Expect low fps, crashes, flickering, artifacts."

void function Survival_WorkInProgressMapSplash_THREAD( entity player )
{
	player.EndSignal( "OnDestroy" )
	...
	Announcement_SetSubText( announcement, WIP_MAP_SPLASH_SUBTEXT )
	Announcement_SetTitleColor( announcement, <1, 0.45, 0.05> )
	announcement.duration = 8.0
	announcement.hideOnDeath = false
	// Free roam drops straight into the map, so the mode's own announcements
	// are still queued when this fires.
	announcement.purge = true
	announcement.priority = 100

	printt( "[WIP-MAP] splash firing for " + GetMapName() )
	AnnouncementFromClass( player, announcement )
}

// Dedi pdata does not round-trip character_selection. Push the local slot so
// the body matches the lobby pick instead of the slot default.
void function Survival_PublishLocalCharacter_THREAD()
{
	if ( IsLobby() )
		return
	...
	while ( !LoadoutSlot_IsReady( ToEHI( player ), slot ) )
	{
		WaitFrame()
		if ( !IsValid( player ) )
			return
	...
	ItemFlavor character = LoadoutSlot_GetItemFlavor( ToEHI( player ), slot )
	if ( character.typeIndex != eItemType.character )
		return
	if ( ItemFlavor_GetAsset( character ) == CHARACTER_RANDOM )
		return

	RequestSetItemFlavorLoadoutSlot( ToEHI( player ), slot, character )
	printt( "[CHARSETUP] published local slot '" + ItemFlavor_GetHumanReadableRef( character ) + "' to server" )
}
...
void function SURVIVAL_PopulatePlayerInfoRui( entity player, var rui )
{
	...
	ItemFlavor character = FS_Hud_GetCharacterFlavor( player )
	asset playerIcon      = CharacterClass_GetGalleryPortrait( character )
	printt( "[SUR-HUD] populate character=" + ItemFlavor_GetHumanReadableRef( character ) + " settings=" + string( player.GetPlayerSettings() ) )
	...
	if ( FS_IsFlowstatePlaylist() && player == GetLocalViewPlayer() && file.pilotRui != null )
		Survival_SyncEquipmentHud( player )
}

void function Survival_SyncEquipmentHud( entity player )
{
	if ( !IsValid( player ) || file.pilotRui == null )
		return
	...
	foreach ( equipSlot, data in EquipmentSlot_GetAllEquipmentSlots() )
	{
		if ( data.trackingNetInt == "" )
			continue
		int lootId = player.GetPlayerNetInt( data.trackingNetInt )
		EquipmentChanged( player, equipSlot, lootId )
		if ( equipSlot == "backpack" )
			BackpackChanged( player, equipSlot, lootId )
	}
}
...
void function Survival_MinimapPackage_ObjectiveAreaInit( entity ent, var rui )
{
	...
	switch ( ent.GetTargetName() )
	{
		...
		case SHADOWARMY_MAP_EVAC_AREA:
			RuiSetColorAlpha( rui, "objColor", SrgbToLinear( COLOR_YELLOW / 255.0 ), 0.05 )
			RuiSetColorAlpha( rui, "objBorderColor", SrgbToLinear( COLOR_YELLOW / 255.0 ), 0.15 )
			RuiSetBool( rui, "blink", false )
			RuiSetBool( rui, "borderBlink", false )
			break

		case SHADOWARMY_LEGEND_START_AREA:
			RuiSetColorAlpha( rui, "objColor", SrgbToLinear( COLOR_RED / 255.0 ), 0.05 )
			RuiSetColorAlpha( rui, "objBorderColor", SrgbToLinear( COLOR_RED / 255.0 ), 0.15 )
			RuiSetBool( rui, "blink", false )
			RuiSetBool( rui, "borderBlink", false )
			break
...
void function EquipmentChanged( entity player, string equipSlot, int new )
{
	...
	bool isEvo   = false
	int evoCount = 0
	bool hideEvoHud = Flowstate_IsGame1v1Type()
	...
	if ( player == GetLocalViewPlayer() && file.pilotRui != null )
	{
		...
		if ( armorCapacity >= 0 )
		{
			...
			if ( data.lootType == eLootType.ARMOR )
			{
				...
				if ( EvolvingArmor_IsEquipmentEvolvingArmor( data.ref ) && !hideEvoHud )
				{
				...
				else if( UpgradeCore_IsEquipmentArmorCore( data.ref ) && !hideEvoHud )
				{
...
void function ToggleFireSelect( entity player )
{
	...
	bool canToggleAltfire = DoesModExist( weapon, "altfire" ) && !DoesModExist( weapon, "hopup_selectfire" )
		...
		if ( canToggleAltfire && WeaponIsAutoHemlok( weapon ) && WeaponAutoHemlok_BreachChargeOnCooldown( weapon ) )
		{
			canToggleAltfire = false
...
void function Survival_OnPlayerClassChanged( entity player )
{
	...
	else
	{
		...
		if ( isReady || FS_IsFlowstatePlaylist() )
		{
			thread InitSurvivalHealthBar()
...
void function WaitingForPlayersOverlay_Setup( entity player )
{
	...
	// 0 is both WaitingForCustomStart and "NonRewind not here yet". Do not treat it as waiting.
...
void function WaitingForPlayersOverlay_Sync_THREAD()
{
	entity player = GetLocalClientPlayer()
	if ( IsValid( player ) )
		EndSignal( player, "OnDestroy" )
	...
	while ( GetGameState() <= eGameState.WaitingForPlayers )
	{
		if ( GetGameState() == eGameState.WaitingForPlayers )
		{
			...
			if ( IsValid( lp ) )
				WaitingForPlayersOverlay_Setup( lp )
		}
		WaitFrame()
	}

	bool hadOverlay = ( s_overlayRui != null )
	WaitingForPlayersOverlay_Destroy()
	...
	entity lp = GetLocalClientPlayer()
	if ( IsValid( lp ) )
		Minimap_UpdateMinimapVisibility( lp )

	if ( hadOverlay )
		printt( "[SUR-HUD] retired waiting overlay at gameState=" + GetGameState() )
}
...
void function Survival_RunCharacterSelection_Thread()
{
	...
	float waitReadyStart = Time()
	while( GetGlobalNetBool( "characterSelectionReady" ) == false )
	{
		if ( Time() - waitReadyStart > 1.0 && ( int( Time() ) % 2 == 0 ) )
		WaitFrame()
	}

	entity player = GetLocalClientPlayer()
	float waitLockStart = Time()
	for ( ; ; )
	{
		if ( IsValid( player ) && (player.GetPlayerNetInt( CHARACTER_SELECT_NETVAR_LOCK_STEP_PLAYER_INDEX ) >= 0) )
			break
		if ( Time() - waitLockStart > 1.0 )
		{
			int idx = IsValid( player ) ? player.GetPlayerNetInt( CHARACTER_SELECT_NETVAR_LOCK_STEP_PLAYER_INDEX ) : -999
			waitLockStart = Time()
		}
		WaitFrame()
	}
	...
	CloseCharacterSelectMenu()
	// Always drop menu camera after char select (podium path only used to do this).
	CharacterSelect_RestorePlayerView()
...
void function OnGameStateChanged( int newVal )
{
	...
	if ( gamestate != eGameState.WaitingForPlayers )
		WaitingForPlayersOverlay_Destroy()
...
void function SetVictorySequenceLocationFromInfoNode()
{
	...
	Survival_StoreVictorySequenceLocation( file.victorySequencePosition, file.victorySequenceAngles )
}
...
void function SetVictorySequenceLocation( vector position, vector angles )
{
	...
	Survival_StoreVictorySequenceLocation( position, angles )
}
...
void function ShowMatchStartSequence( int teamOrAlliance, float camera_move_duration = DEFAULT_PODIUM_DURATION, bool placementMode = false, bool isDevTest = false, bool isChampionTeam = false )
{
	...
	if ( IsValid ( player ) )
	{
		...
		RuiSetString( file.introPodiumRui, "mapNameString", GetCurrentMapDisplayName() )
...
{
	if ( file.pilotRui == null || file.dpadMenuRui == null )
		return
