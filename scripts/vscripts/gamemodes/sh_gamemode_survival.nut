...
global function Survival_StoreVictorySequenceLocation
global function GetVictorySequencePosition
global function GetVictorySequenceAngles
...
#if SERVER
global function SetupVictoryPodium
#if DEVELOPER
...
{
	// Playlist wins when present. Convar alone used to force-zero char select / WFP
	// countdown on every survival_br_dev run if a prior session left fast_intro 1.
	if ( GetCurrentPlaylistVarInt( "fast_intro", -1 ) >= 0 )
		return GetCurrentPlaylistVarBool( "fast_intro", false )
...
void function Survival_StoreVictorySequenceLocation( vector position, vector angles )
{
	file.victorySequencePosition = position
	file.victorySequenceAngles = angles
}

vector function GetVictorySequencePosition()
{
	return file.victorySequencePosition
}

vector function GetVictorySequenceAngles()
{
	return file.victorySequenceAngles
}
