...
global function Dev_AssignRandomCharacterToFakePlayer
...
// Origin cache for FR class change and freeroam death/class-change restore
...
void function OnConnectionLost_Reconnect( entity player )
{
	...
	if ( player.GetPartySize() == 1 )
	{
					...
					|| respawnStatus == eRespawnStatus.SQUAD_ELIMINATED ) )
		{
			...
			//player.Forfeit // TODO(s3): Forfeit missing on S3
		}
	}
...
void function EntitiesDidLoad_Survival()
{
...

	{
		...
		//if ( GameModeVariant_IsActive( eGameModeVariants.SURVIVAL_SHADOW_ARMY ) ) // TODO(s3): ShadowArmy plane path missing on S3
		//{
...
array< PlanePathData > function Survival_GenerateSingleRandomPlanePath( bool beQuick, int unusedInt = 0 )
{
	...
	#if DEBUG_PLANE_PATH
		DebugDrawLineRGB( planeStart, planeEnd, 255, 255, 0, true, 10.0 )
	#endif
	...
	if ( PLANE_PATH_DEBUG )
	{
		...
		if ( !dev_numTriesFailed )
			DebugDrawLineRGB( clampedPlaneStart - <0, 0, 10000>, clampedPlaneEnd - <0, 0, 10000>, 0, 255, 0, true, 240.0 )
	#endif
...
void function DEV_SimulatePlanePaths_Internal( int count = 1 )
{
	...
	DebugDrawLineRGB( minPath.startPos, minPath.endPos, int(COLOR_MAGENTA.x), int(COLOR_MAGENTA.y), int(COLOR_MAGENTA.z), true, 30.0 )
	DebugDrawLineRGB( maxPath.startPos, maxPath.endPos, 255, 255, 0, true, 30.0 )
	...
	while( true )
	{
		for ( int i = 0 ; i < 3 ; i++ )
		{
			...
			if ( Distance2D( pointOnPlanePath, mapCenter ) < SURVIVAL_PLANE_DROP_RADIUS_MIN )
			{
				...
				DebugDrawLineRGB( pointOnPlanePath, pointOnPlanePath - <0, 0, 10000>, 255, 0, 0, true, 10.0 )
			...
			if ( trace.fraction < 1.0 && !trace.hitSky && !trace.startSolid )
			{
				...
				#if DEBUG_PLANE_PATH
					DebugDrawLineRGB( traceStart, trace.endPos, 0, 0, 255, true, 10.0 )
					if ( closestTitanNavMesh == null )
					{
						DebugDrawSphereRGB( trace.endPos, 256.0, 255, 0, 0, true, 10.0 )
					}
					else
					{
						expect vector(closestTitanNavMesh)
						DebugDrawLineRGB( trace.endPos, closestTitanNavMesh, 0, 255, 0, true, 10.0 )
						DebugDrawSphereRGB( closestTitanNavMesh, 256.0, 0, 255, 0, true, 10.0 )
					}
				#endif
...
void function OnPlayerKilled( entity victim, entity attacker, var damageInfo )
{
	...
	if ( IsDevGamemode() && IsValid( victim ) )
		Survival_StoreChangeClassPos( victim, false, false )
...
bool function ShouldDoBleedout( entity damagedEnt )
{
	...
	// Bleedout_Init FlagInits this; modes that never run GamemodeSurvival_Init
	// (e.g. fs_1v1) still hit this CodeCallback and must not index a missing flag.
	if ( FlagExists( "BleedoutDebug" ) && Flag( "BleedoutDebug" ) )
		return true
...
void function Survival_GameStartedPlaying_Thread()
{
	...
	entity planeEntPre = Sur_GetPlaneEnt()
...
void function _SpawnPlayersOnGroundWithSquadNearLoot_internal( vector center, float radius, table< int, array< entity > > groupedPlayers, bool debug = false )
{
	...
	if ( debug )
	{
		...
		{
			DebugDrawSphereRGB( spawnPoint, 32.0, int(COLOR_MAGENTA.x), int(COLOR_MAGENTA.y), int(COLOR_MAGENTA.z), true, 10.0 )
	...
	foreach ( int team, array<entity> players in groupedPlayers )
	{
		...
		foreach ( int i, entity player in players )
		{
			...
			if ( debug )
			{
				...
				DebugDrawLineRGB( neighborPoints[i] + <0, 0, 36>, neighborPoints[i] + <0, 0, 36> + (AnglesToForward( angles ) * 128), 255, 0, 0, true, 10.0 )
			}
			else
			{
				ClearPlayerIntroDropSettings( player )
	...
	for ( int i = lootPoints.len() - 1; i >= 0; i-- )
	{
		...
		if ( clampedPoint != null )
		{
			...
			if ( neighborPoints.len() >= numPositions )
			{
				if ( debug )
				{
					DebugDrawSphereColor( lootPoints[i], 4.0, <0, 128, 0>, true, 10.0, 4 )
					DebugDrawLineRGB( lootPoints[i], lootPoints[i] + <0, 0, 2000>, 0, 128, 0, true, 10.0 )
				}
				continue
			}
		}
		...
		if ( debug )
		{
			DebugDrawSphereColor( lootPoints[i], 4.0, <255, 128, 128>, true, 10.0, 4 )
			DebugDrawLineRGB( lootPoints[i], lootPoints[i] + <0, 0, 2000>, 255, 128, 128, true, 10.0 )
		}
		lootPoints.remove( i )
	}
}
...
void function SURVIVAL_SendWinningSquadDataToPlayer( entity player, int winningTeam )
{
	...
	{
		// Clear their local squad data (S3 split: Base + Extra)
		...
		foreach ( int i, GameSummarySquadData data in file.squadData[ winningTeam ] )
		{
			...
			//	i, data.eHandle, data.kills, data.assists, data.knockdowns,
			//	data.damageDealt, data.survivalTime, data.revivesGiven )
			//Remote_CallFunction_NonReplay( player, "ServerCallback_AddWinningSquadData_Extra",
			//	data.respawnsGiven, data.displayData3IsTime,
			//	data.displayData[3], data.displayData[4], data.displayData[5], data.displayData[6],
...
ItemFlavor function Survival_ValidateAndGetCharacterClass( entity player )
{
	...
	// A mode that dictates the legend owns this slot. The unlock check below cannot
	// pass without GRX, and its fallback is a random legend on every respawn.
	LoadoutEntry_CharacterResetOverrideFuncType characterOverride = Loadout_GetCharacterResetOverride()
	if ( characterOverride != null )
	{
		ItemFlavor forcedCharacter = characterOverride( playerEHI )
		SetItemFlavorLoadoutSlot( playerEHI, loadoutCharacter, forcedCharacter )
...
void function SurvivalPlayerRespawnedInit( entity player )
{
	...
	// Deferred SetOrigin: RespawnTitanPilot stomps origin after OnPlayerRespawned.
	if ( Survival_TryDevRestoreCachedOrigin( player ) )
	{ }
	else if ( !player.p.respawnPodLanded && Survival_TryDevPinSpawn( player ) )
	{ }
	else if ( GetGameState() == eGameState.WaitingForPlayers )
	{
...
ItemFlavor function Dev_PickRandomFakePlayerCharacter()
{
	...
	foreach ( ItemFlavor character in GetAllCharacters() )
	{
		...
		if ( characterAsset == CHARACTER_RANDOM || characterAsset == CHARACTER_DUMMIE )
			continue
		...
		playable.append( character )
	}

	if ( playable.len() < 1 )
		return CharacterClass_GetRandomNonGRXCharacter()

	return playable[RandomInt( playable.len() )]
}

void function Dev_AssignRandomCharacterToFakePlayer( entity bot )
{
	if ( !IsValid( bot ) )
		return
	...
	// Validation force-resets bots and sticks the slot to the default class.
	...
	if ( !IsValid( bot ) || !bot.IsPlayer() || !IsAlive( bot ) )
		return

	ItemFlavor character = Dev_PickRandomFakePlayerCharacter()
	...
	{
		printt( format( "[Dev] spawn_fake_player character setup failed name=%s err=%s", bot.GetPlayerName(), charSetupErr ) )
		return
	}

	string characterRef = ItemFlavor_GetHumanReadableRef( character )
	printt( format( "[Dev] spawn_fake_player character=%s name=%s team=%d", characterRef, bot.GetPlayerName(), bot.GetTeam() ) )
	LocalSplashMsgAll( format( "Spawned %s as %s (team %d)", bot.GetPlayerName(), characterRef, bot.GetTeam() ), 3.0 )
}
...
void function Survival_PlayerCharacterSetup( entity player, ItemFlavor character, bool giveDefaultMelee = true )
{
	if ( !IsValid( player ) || !player.IsPlayer() )
		return
	// Read typeIndex directly: ItemFlavor is a struct (never null) and
	// ItemFlavor_GetType asserts on the default-constructed guid.
	if ( character.typeIndex != eItemType.character )
	{
		printt( format( "[CHARSETUP] %s REFUSED: arg is not a character (typeIndex=%d)",
			player.GetPlayerName(), character.typeIndex ) )
		return
	}

	if ( !IsAlive( player ) )
	{
		printt( format( "[CHARSETUP] %s REFUSED: dead, wanted '%s'",
	...
	if ( IsLobby() )
	{
		printt( format( "[CHARSETUP] %s REFUSED: IsLobby, wanted '%s'",
			player.GetPlayerName(), ItemFlavor_GetHumanReadableRef( character ) ) )
		return
	}
	...
	{
		ItemFlavor replacement = CharacterClass_GetRandomPlayableCharacter()
		printt( format( "[CHARSETUP] %s swapped playlist-disabled '%s' -> '%s'",
			player.GetPlayerName(), ItemFlavor_GetHumanReadableRef( character ),
			ItemFlavor_GetHumanReadableRef( replacement ) ) )
	...
	EHI playerEHI = ToEHI( player )
	LoadoutEntry loadoutCharacter = Loadout_Character()
	if ( !LoadoutSlot_IsReady( playerEHI, loadoutCharacter ) || LoadoutSlot_GetItemFlavor( playerEHI, loadoutCharacter ) != character )
		SetItemFlavorLoadoutSlot( playerEHI, loadoutCharacter, character )

	printt( format( "[CHARSETUP] %s APPLIED '%s' setfile=%s readback=%s | slot now '%s'",
		player.GetPlayerName(), ItemFlavor_GetHumanReadableRef( character ), string( setFile ),
		string( player.GetPlayerSettings() ),
		ItemFlavor_GetHumanReadableRef( LoadoutSlot_GetItemFlavor( playerEHI, loadoutCharacter ) ) ) )
...
void function Survival_OnPlayerChangedCharacterClass( EHI playerEHI, ItemFlavor character )
{
	...
	if ( IsAlive( player ) && player.GetPlayerSettings() == CharacterClass_GetSetFile( character ) )
		return
	...
	{
		Survival_StoreChangeClassPos( player, false, false )
	}
	...
	vector pinPos = player.GetOrigin()
	vector pinAng = player.GetAngles()
	Survival_PlayerCharacterSetup( player, character )
	if ( IsDevGamemode() )
	{
		player.SetOrigin( pinPos )
		player.SetAngles( pinAng )
		thread Survival_SetOriginNextFrame_Thread( player, pinPos, pinAng )
	}
}
...
void function FiringRange_SwitchCharacterPresentation_Thread( entity player, ItemFlavor character )
{
	...
	// Pin across setup: S3 SetPlayerSettingsWithMods / safe-spot can still stomp.
	// S21 stays put without this; we only re-apply the cached FR pos.
	vector pinPos = player.GetOrigin()
	vector pinAng = player.GetAngles()
	respawnIn3P.value = false
	if ( EHIToEncodedEHandle( player ) in file.playerChangeClassData )
	{
		PlayerChangeClassData pccd = file.playerChangeClassData[ EHIToEncodedEHandle( player ) ]
		pinPos = pccd.respawnPos
		pinAng = pccd.respawnAngles
		respawnIn3P.value = pccd.respawnIn3P || FRSetting_3rdPerson_IsOn( player )
		delete file.playerChangeClassData[ EHIToEncodedEHandle( player ) ]
	}
	...
	if( !wasAlreadyInShoulderMode.value )
	{
		...
		player.SetTrackEntityOffsetRight( 12.0 )
		player.SetTrackEntityBlendInTimes( 0.2,0.0,0.0 )
		player.SetTrackEntityBlendOutTime( OUTRO_EASE_TIME )
...
void function Survival_StoreChangeClassPos( entity player, bool respawnIn3P, bool onlyIfMissing )
{
	if ( !IsValid( player ) )
		return

	EncodedEHandle handle = EHIToEncodedEHandle( player )
	if ( onlyIfMissing && ( handle in file.playerChangeClassData ) )
		return

	PlayerChangeClassData pccd
	pccd.respawnPos = player.GetOrigin()
	pccd.respawnAngles = player.GetAngles()
	pccd.respawnIn3P = respawnIn3P
	file.playerChangeClassData[handle] <- pccd
}

void function Survival_SetOriginNextFrame_Thread( entity player, vector pos, vector ang )
{
	player.EndSignal( "OnDestroy" )
	WaitEndFrame()
	if ( !IsValid( player ) || !IsAlive( player ) )
...
void function Survival_MarkFreeroamLanded( entity player )
{
	ClearPlayerIntroDropSettings( player )
	PlayerMatchState_Set( player, ePlayerMatchState.NORMAL )
	player.p.survivalLandedOnGround = true
...
// Prefers a live human, then any other alive player.
entity function Survival_GetDevSpawnAnchorPlayer( entity player )
{
	array<entity> alivePlayers = GetPlayerArray_Alive()

	foreach ( entity other in alivePlayers )
	{
		if ( other != player && IsValid( other ) && !other.IsBot() )
			return other
	}

	foreach ( entity other in alivePlayers )
	{
		if ( other != player && IsValid( other ) )
			return other
	}
...
bool function Survival_IsDevPinSpawnMode()
{
	return IsDevGamemode() || GetCurrentPlaylistVarBool( "is_practice_map", false )
}

// Freeroam: restore cached origin after engine spawn stomp (including DevRespawn respawnPodLanded).
bool function Survival_TryDevRestoreCachedOrigin( entity player )
{
	if ( !IsDevGamemode() )
		return false

	EncodedEHandle handle = EHIToEncodedEHandle( player )
	if ( !( handle in file.playerChangeClassData ) )
		return false

	PlayerChangeClassData pccd = file.playerChangeClassData[handle]

	if ( !player.p.hasMatchParticipationStarted )
		OnPlayerMatchParticipationStarted( player )

	Survival_MarkFreeroamLanded( player )
	thread Survival_RestoreCachedOriginNextFrame_Thread( player, pccd.respawnPos, pccd.respawnAngles, pccd.respawnIn3P )
	return true
}

void function Survival_RestoreCachedOriginNextFrame_Thread( entity player, vector pos, vector ang, bool respawnIn3P )
{
	player.EndSignal( "OnDestroy" )
	WaitEndFrame()
...
// First human spawn / every bot spawn: pin next to another alive player.
bool function Survival_TryDevPinSpawn( entity player )
{
	if ( !Survival_IsDevPinSpawnMode() )
		return false

	if ( !player.IsBot() && player.p.respawnCount > 1 )
		return false

	entity anchorPlayer = Survival_GetDevSpawnAnchorPlayer( player )
	if ( !IsValid( anchorPlayer ) )
		return false
	...
	Survival_MarkFreeroamLanded( player )
	thread Survival_DevPinSpawnNearAnchor_Thread( player, anchorPlayer )
	return true
}

void function Survival_DevPinSpawnNearAnchor_Thread( entity player, entity anchorPlayer )
{
	player.EndSignal( "OnDestroy" )
	WaitEndFrame()
	...
	if ( !IsValid( anchorPlayer ) || !IsAlive( anchorPlayer ) )
		anchorPlayer = Survival_GetDevSpawnAnchorPlayer( player )
	...
	if ( player.IsBot() )
	{
		float yaw = RandomFloatRange( 0.0, 360.0 )
		offset = AnglesToForward( <0, yaw, 0> ) * RandomFloatRange( 48.0, 96.0 )
	}

	player.SetOrigin( anchorPlayer.GetOrigin() + offset )
	player.SetAngles( <0.0, anchorPlayer.GetAngles().y, 0.0> )
	Survival_MarkFreeroamLanded( player )
}
...
void function Survival_OnClientConnected( entity player )
{
	...
	// Dev pin is in SurvivalPlayerRespawnedInit via Survival_TryDevPinSpawn.
	// Never DecideRespawnPlayer here -- stock CodeCallback_OnClientConnectionCompleted does it.
}
...
void function RespawnTeamAtRingEdge( int team )
{
	...
	DebugDrawLineRGB( p.origin, p.origin + <0, 0, 10000>, 255, 255, 255, true, 10.0 )
...
void function GameSummary_MatchStart( entity player )
{
	...
	//if( player.IsUsingSteam ) // TODO(s3): IsUsingSteam missing on S3
	//{
	//	data.platformUid = player.GetPINNucleusId // TODO(s3): GetPINNucleusId missing on S3
	//}
	data.pid                      = "" //player.GetPINNucleusPid // TODO(s3): GetPINNucleusPid missing on S3
	data.nucleus                  = "" //player.GetPINNucleusId // TODO(s3): GetPINNucleusId missing on S3
	data.hardware                 = "" //player.GetUnspoofedHardware // TODO(s3): GetUnspoofedHardware missing on S3
	data.isUsingSteam             = false //player.IsUsingSteam // TODO(s3): IsUsingSteam missing on S3
...
GameSummarySquadData function GameSummary_GetPlayerData( entity player )
{
	...
	if ( !( team in file.squadData ) || !( index in file.squadData[ team ] ) )
		GameSummary_MatchStart( player )
...
void function CodeCallback_ReportPlayerCustomerService( entity player, int hardwareID, string uid, string eaid, string reason )
{
	...
	string reporterHardware = "" //player.GetUnspoofedHardware // TODO(s3): GetUnspoofedHardware missing on S3
