...
void function TDM_Init()
{
	#if SERVER
		if ( FreeDM_IsFFA() )
			printt( "[FreeDM] FFA active" )

		if ( GameMode_AreRoundsEnabled() )
			SetRoundBased( true )

		if ( FreeDM_IsFFA() )
			Spawn_SetSpawnpointRatingFunc( RateSpawnpoints_FFA )
		else
...
void function TDM_ScoreboardSetup()
{
	...
	if ( FreeDM_IsFFA() )
	{
		Teams_AddCallback_Header( TDM_FFA_ScoreboardUpdateHeader )
		Teams_AddCallback_GetTeamColor( TDM_FFA_GetTeamColor )
		return
	}
...
#if CLIENT
void function TDM_FFA_ScoreboardUpdateHeader( var headerRui, var frameRui, int team )
{
	if ( headerRui == null )
		return
	...
	RuiSetInt( headerRui, "score", GameRules_GetTeamScore( team ) )
	RuiSetInt( headerRui, "maxScore", maxScore )
	RuiSetInt( headerRui, "gameState", GetGameState() )
}
#endif // CLIENT

#if CLIENT
vector function TDM_FFA_GetTeamColor( int team )
{
	return Squads_GetSquadColor( Squads_GetSquadUIIndex( team ) )
}
#endif // CLIENT
