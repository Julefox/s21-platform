...
void function ShDevUtility_Init()
{
	...
	// respawn / set_respawn_override register in _clientcommands.gnut (always-loaded).
	// Do not re-register here -- When:DEV + non-DEV double-register is a no-op/overwrite trap.
...
// DEV_SendDevMenuStateToUI lives in client/cl_devmenu_cheats.nut (always CLIENT).
// Do not define it here under When:DEV — non-dev UI DevMenu would fail RunClientScript.
...
//////////////////////////////
//////////////////////////////
// Respawn client commands + DevRespawnGetPlayerEliminationOverride live in
// _clientcommands.gnut (always-loaded). Kept here: DEV-only helpers that still
// reference the shared override via DevRespawnGetPlayerEliminationOverride.
#if SERVER
...
void function DEV_PreviewWorldRUI( asset ruiAsset, float width = 100, float height = 100 )
{
	...
	DebugDrawLineRGB( pos, pos + 50.0 * right, 255, 120, 120, true, 5 )
	DebugDrawLineRGB( pos, pos + 50.0 * down, 120, 255, 120, true, 5 )
...
void function DEV_PlayerDebugLinesTool_Thread( entity localClientPlayer )
{
	...
	while( true )
	{
		...
		#endif // SERVER || CLIENT
		{
			DebugDrawLineRGB( player.GetOrigin(), player.EyePosition(), 10, 80, 65, true, 0.0 )
			DebugDrawLineRGB( player.EyePosition(), player.EyePosition() + 40.0 * AnglesToForward( player.EyeAngles() ), 30, 255, 220, false, 0.0 )
...
void function DEV_DrawBoundingBox()
{
	...
	while ( cornerIdx < 4 )
	{
		...
		if ( cornerIdx == 0 )
		{
			fourCorners[0] = EyeTraceVec( player )
		...
		else if ( cornerIdx >= 1 )
		{
			if ( cornerIdx == 1 )
			{
				fourCorners[1] = EyeTraceVec( player )
			}
			DebugDrawLineRGB( fourCorners[0], fourCorners[1], int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), false, 0.0 )
			DebugDrawLineRGB( fourCorners[0], fourCorners[1], 0, 120, 0, true, 0.0 )
			...
			if ( cornerIdx >= 2 )
			{
				...
				DebugDrawLineRGB( fourCorners[0], fourCorners[2], int(COLOR_RED.x), int(COLOR_RED.y), int(COLOR_RED.z), false, 0.0 )
				DebugDrawLineRGB( fourCorners[0], fourCorners[2], 120, 0, 0, true, 0.0 )
				...
				if ( cornerIdx == 3 )
				{
					...
					DebugDrawLineRGB( fourCorners[0], fourCorners[3], int(COLOR_BLUE.x), int(COLOR_BLUE.y), int(COLOR_BLUE.z), false, 0.0 )
					DebugDrawLineRGB( fourCorners[0], fourCorners[3], 0, 0, 120, true, 0.0 )
				}
			}
		}
	...
	DebugDrawRotatedBoxRGB( boxCenter, boxMins, boxMaxs, boxAng, int( COLOR_WHITE.x ), int( COLOR_WHITE.y ), int( COLOR_WHITE.z ), true, 20.0 )
...
void function DEV_ShowPlayerInvulnerableState( entity player, float showTime = 30.0 )
{
	...
	thread function() : (player, showTime)
	{
		...
		while(( timePassed < showTime ) && IsValidPlayer( player ))
		{
			...
			DebugDrawSphereRGB( player.GetOrigin(), 128, 255, 0, 0, true, TIME_SLICE )
			wait TIME_SLICE
...
void function DEV_ShowAllPlayerLocs( float duration = 10 )
{
	...
	foreach(player in allPlayers )
	{
		...
		DebugDrawCylinderRGB( playerLoc, < -90, 0, 0 >, 32, 64, 255, 0, 0, true, duration )
