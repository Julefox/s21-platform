...
global const float WORLD_SETORIGIN_LIMIT = 65536.0
...
void function TableDump( table Table, int depth = 0 )
{
	...
	foreach ( k, v in Table )
	{
		...
		// S21: no int→bool (S21 CLIENT rejects bare depth)
...
bool function ArePlayersAllies( entity playerA, entity playerB )
{
	...
	// S21: IsTeamInAlliance engine native (bridge implements on S3).
...
{
	// S21 CLIENT: no int→bool; playlist int must be compared
	return GetCurrentPlaylistVarInt( "npc_short_titles", 0 ) != 0
}
...
array<vector> function GetAllPointsOnBezier( array<vector> points, int numSegments, float debugDrawTime = 0.0 )
{
	...
	// Debug draw the points used for the curve (S21: no float→bool)
	// DebugDrawLine: S3 SERVER rgb ints; S21 CLIENT colorVec (S21 form)
	if ( debugDrawTime != 0 )
	{
		for ( int i = 0; i < points.len() - 1; i++ )
			DebugDrawLineColor( points[i], points[i + 1], <150, 150, 150>, true, debugDrawTime )
	}
	...
	if ( debugDrawTime != 0 )
	{
		for ( int i = 0; i < curvePoints.len() - 1; i++ )
			DebugDrawLineColor( curvePoints[i], curvePoints[i + 1], <200, 0, 0>, true, debugDrawTime )
	}
...
GravityLandData function GetGravityLandData( vector startPos, vector parentVelocity, vector objectVelocity, float timeLimit, bool bDrawPath = false, int traceMask = TRACE_MASK_NPCWORLDSTATIC, float bDrawPathDuration = 0.0, array<int> pathColor = [ 255, 255, 0 ] )
{
	...
	while ( returnData.elapsedTime <= timeLimit )
	{
		...
		if ( bDrawPath )
		{
			// array<int> indices are int; dual-arity via DebugDrawLineColor
			DebugDrawLineColor( traceStart, traceEnd, <pathColor[0], pathColor[1], pathColor[2]>, false, bDrawPathDuration )
		}
...
{
	int aVal = expect int( a.GetPlayerGameStat( PGS_ASSAULT_SCORE  ) )
	int bVal = expect int( b.GetPlayerGameStat( PGS_ASSAULT_SCORE  ) )
...
{
	int aVal = expect int( a.GetPlayerGameStat( PGS_SCORE  ) )
	int bVal = expect int( b.GetPlayerGameStat( PGS_SCORE  ) )
...
{
	int aVal = expect int( a.GetPlayerGameStat( PGS_DEFENSE_SCORE  ) )
	int bVal = expect int( b.GetPlayerGameStat( PGS_DEFENSE_SCORE  ) )
...
int function CompareCP( entity a, entity b )
{
...

	{
		...
		aVal += expect int( a.GetPlayerGameStat( PGS_DEFENSE_SCORE  ) )
		bVal += expect int( b.GetPlayerGameStat( PGS_DEFENSE_SCORE  ) )
	...
	// 3) Pilot Kills
	{
		int aVal = expect int( a.GetPlayerGameStat( PGS_KILLS  ) )
		int bVal = expect int( b.GetPlayerGameStat( PGS_KILLS  ) )
	...
	// 3) Titan Kills
	{
		int aVal = expect int( a.GetPlayerGameStat( PGS_TITAN_KILLS  ) )
		int bVal = expect int( b.GetPlayerGameStat( PGS_TITAN_KILLS  ) )
	...
	// 4) Deaths
	{
		int aVal = expect int( a.GetPlayerGameStat( PGS_DEATHS  ) )
		int bVal = expect int( b.GetPlayerGameStat( PGS_DEATHS  ) )
...
int function CompareCTF( entity a, entity b )
{
	...
	// 3) Titan Kills
	aVal = expect int( a.GetPlayerGameStat( PGS_TITAN_KILLS  ) )
	bVal = expect int( b.GetPlayerGameStat( PGS_TITAN_KILLS  ) )
	...
	// 4) Deaths
	aVal = expect int( a.GetPlayerGameStat( PGS_DEATHS  ) )
	bVal = expect int( b.GetPlayerGameStat( PGS_DEATHS  ) )
...
{
	int aVal = expect int( a.GetPlayerGameStat( PGS_DETONATION_SCORE  ) )
	int bVal = expect int( b.GetPlayerGameStat( PGS_DETONATION_SCORE  ) )
...
bool function IsHitEffectiveVsTitan( entity victim, int damageType )
{
	...
	// S21 style: IsBitFlagSet returns bool (mask & flag) — no int→bool / !int
	if ( !IsBitFlagSet( damageType, DF_CRITICAL ) && ( IsBitFlagSet( damageType, DF_BULLET ) || IsBitFlagSet( damageType, DF_MAX_RANGE ) ) )
		return false
...
// SetOrigin fatals the host if any axis is outside ±(1<<16). MAP_EXTENTS is a
// different, tighter overlay bound -- do not ClampToMap a dummy plant dest.
bool function IsLegalWorldOrigin( vector pos )
{
	if ( pos.x != pos.x || pos.y != pos.y || pos.z != pos.z )
		return false
	return fabs( pos.x ) < WORLD_SETORIGIN_LIMIT && fabs( pos.y ) < WORLD_SETORIGIN_LIMIT && fabs( pos.z ) < WORLD_SETORIGIN_LIMIT
}

bool function TrySetOriginInWorld( entity ent, vector pos )
{
	if ( !IsValid( ent ) )
		return false
	if ( !IsLegalWorldOrigin( pos ) )
		return false
	ent.SetOrigin( pos )
	return true
}
...
void function DebugDrawLineFromEntToPos( entity ent, vector pos, int r, int g, int b, bool throughGeo, float duration )
{
	...
	while( Time() <= endTime )
	{
		DebugDrawLineRGB( ent.GetOrigin(), pos, r, g, b, throughGeo, 0.1 )
		wait 0.05
	}
}
...
// S3 SERVER polyfill. S21 CLIENT: engine-bound (do not redeclare).
#if SERVER
...
void function GivePlayerSettingsMods( entity player, array<string> additionalMods )
{
...
#endif
	{
		...
		{
			// S21 CLIENT / S21: (mod). S3 SERVER: (settingsName, mod).
			#if CLIENT
...
// S3 SERVER polyfill; S21 CLIENT engine-bound.
#if SERVER
...
// S3 / bridge-only string; S21 CLIENT engine-bound.
#if SERVER
...
int function ScoreCompare_Freelance( entity a, entity b )
{
	...
	aVal = expect int( a.GetPlayerGameStat( PGS_SCORE  ) )
	bVal = expect int( b.GetPlayerGameStat( PGS_SCORE  ) )
...
string function GetEnumStringFromValue( int match, table< string, int > enumTable )
{
	foreach ( string key, int value in enumTable )
	{
		if ( value == match )
		{
			return key
...
#if SERVER
void function ApplyUpdraftModUntilTouchingGround( entity player )
{
	if ( HasPlayerSettingMod( player, "fissure_updraft" ) )
		return
	...
	thread function() : (player)
	{
		...
		GivePlayerSettingsMods( player, ["fissure_updraft"] )
		OnThreadEnd( function() : (player)
		{
			if ( IsValid( player ) )
				TakePlayerSettingsMods( player, ["fissure_updraft"] )
		} )

		while( !IsValid( player.GetGroundEntity() ) )
			WaitFrame()
	}()
}
#endif
...
#if SERVER
void function StunWhenInterruptable( entity ent, var damageInfo )
{
	if ( Electricity_ShouldDamageStunNPC( ent, damageInfo ) )
	{
		ent.EndSignal("OnDeath")
		while( !ent.IsInterruptable() )
		{
			WaitFrame()
		}

		// S3 native is 3-arg (name, bool, blend); S21 is 4-arg.
