...
void function OnItemFlavorRegistered_LootMainWeapon( ItemFlavor weaponFlavor )
{
	// skins
	{
		...
		entry.DEV_name     = GetCleanAssetPathForItemFlavor( weaponFlavor ) + " Skin"
		#if DEVELOPER
	...
	// charms
	{
		...
		charmEntry.DEV_name     = GetCleanAssetPathForItemFlavor( weaponFlavor ) + " Charm"
		#if DEVELOPER
...
{
	// Live apply for Alter Loadout / loadouts_devset (sv_cheats). Must not be DEVELOPER-only —
	// without this, the loadout slot updates but held weapons keep the old skin/charm.
	string weaponClassName = WeaponItemFlavor_GetClassname( weaponFlavor )
...
void function WeaponCosmetics_Apply( entity ent, ItemFlavor ornull skinOrNull, ItemFlavor ornull charmOrNull )
{
	...
	{
		ItemFlavor ornull weaponItemOrNull = GetWeaponItemFlavorByClass( ent.GetWeaponClassName() )
		if ( weaponItemOrNull != null )
		{
			ItemFlavor weaponFlav = expect ItemFlavor( weaponItemOrNull )
			EHI ownerEHI = ToEHI( owner )
			if ( skinOrNull != null )
			{
				ItemFlavor skin = expect ItemFlavor( skinOrNull )
				LoadoutEntry skinSlot = Loadout_WeaponSkin( weaponFlav )
				if ( !IsItemFlavorUnlockedForLoadoutSlot( ownerEHI, skinSlot, skin ) )
					skinOrNull = GetDefaultItemFlavorForLoadoutSlot( ownerEHI, skinSlot )
			}
			if ( charmOrNull != null )
			{
				ItemFlavor charm = expect ItemFlavor( charmOrNull )
				LoadoutEntry charmSlot = Loadout_WeaponCharm( weaponFlav )
				if ( !IsItemFlavorUnlockedForLoadoutSlot( ownerEHI, charmSlot, charm ) )
					charmOrNull = GetDefaultItemFlavorForLoadoutSlot( ownerEHI, charmSlot )
			}
		}
	}
#endif
	...
	if ( charmOrNull != null )
	{
		...
		ent.e.charmItemFlavorGUID = ItemFlavor_GetGUID( charm )
			...
			if ( ent.GetNetworkedClassName() == "weaponx" )
			{
				...
				// The whole charm path is unimplemented here: the GUID native is a
				// registered stub, and SetWeaponCharm/ClearWeaponCharm have no
				// native at all.
...
void function WeaponCosmetics_ApplyModelAndSkin( entity ent, ItemFlavor skin )
{
	...
	// SetLegendaryModelIndex after SetModel; required for weaponx skin apply.
	if ( ent.GetNetworkedClassName() == "weaponx" )
