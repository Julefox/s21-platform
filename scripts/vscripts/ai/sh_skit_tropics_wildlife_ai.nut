...
entity function SpawnWildlifeAI( WildlifeCampData campData, int npcType, float assaultScale, vector origin, vector angles )
{
	...
	// entitystructs default ai.npcType = -1; loot PIN (SetItemSpawnSource) and
	// GetLootParams key off this eNPC value — must stamp what we just spawned as.
	npc.ai.npcType = npcType
		...
		thread function() : ()
		{
			for ( ;; )
			{
				...
				foreach ( WildlifeCampData campData in s_wildlifeCampDatas )
				{
					...
					DebugDrawTriggerRGB( origin, radius, 255, 0, 102, 1.0, true )
				}
			}
		}()
	}
	...
	if ( s_isDebugDrawActivationRadiiEnabled == true )
	{
		...
		thread function() : ()
		{
			for ( ;; )
			{
				...
				foreach ( WildlifeCampData campData in s_wildlifeCampDatas )
				{
					DebugDrawTriggerRGB( campData.activationOrigin, campData.activationRadius, 255, 255, 255, 1.0, true )
				}
			}
		}()
	}
