...
void function DEV_ActiveApexScreenDebugThread()
{
	...
	while ( file.DEV_activeScreenDebug )
	{
		...
		foreach ( ApexScreenState screen in file.staticScreenList )
		{
			...
			{
				activeCount += 1
				DebugDrawRotatedBoxRGB( <0, 0, 0>, screen.spawnMins + <-1, -1, -3>, screen.spawnMaxs + <-1, -1, -3>, <0, 0, 0>, 140, 185, 255, true, interval + 0.1 )
			}
			else
			{
				DebugDrawRotatedBoxRGB( <0, 0, 0>, screen.spawnMins + <-1, -1, -3>, screen.spawnMaxs + <-1, -1, -3>, <0, 0, 0>, 25, 25, 80, true, interval + 0.1 )
			}
		}
