...
#if SERVER
global function Freelance_SendTeamCanRespawn
//
...
{
	#if SERVER || CLIENT
	PrecacheModel( $"mdl/dev/editor_ref.rmdl" ) //used to test if a proposed spawn point is touching a zone trigger
	#endif // SERVER || CLIENT

	// dialogue:
	//RegisterCSVDialogue( $"datatable/dialogue/freelance_common.rpak" )
	//RegisterCSVDialogue( $"datatable/dialogue/freelance_mission_proto.rpak" )
...
#if SERVER
void function Freelance_SendTeamCanRespawn( int team, int teamCanRespawnState )
{
	foreach ( entity player in GetPlayerArrayOfTeam( team ) )
	{
		if ( player.p.latestSentTeamCanRespawnState == teamCanRespawnState )
			continue
		Remote_CallFunction_NonReplay( player, FUNCNAME_UpdateTeamCanRespawn, teamCanRespawnState )
		player.p.latestSentTeamCanRespawnState = teamCanRespawnState
	}
}
...
{
	return s_latestHUDTracker
}
#endif // DEV

entity function SquadHUDTracker_Create( float revealTime )
{
...
void function SquadHUDTracker_UpdateSquadProgress( entity wp, int squad, float progress )
{
	//float currentProgress = wp.GetWaypointFloat( squad )
	//if ( currentProgress > progress )
	//	return

	wp.SetWaypointFloat( squad, progress )
}
