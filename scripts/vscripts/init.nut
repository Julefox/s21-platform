...
{
#if SERVER
	// S3 native GRX: int rows, not typed client structs
	array< int > items
...
// Bridge/product helper (not engine-ordered). Append-only so native type ordinals stay bak/S21.
...
// Declared for the dedicated server, which does not provide them; the client
// does, so these values must be the client's or they shadow it with a different
// numbering and every breach result is misread.
global const int BREACH_TRACE_RESULT_SUCCESS = 0
global const int BREACH_TRACE_RESULT_FAILURE = 1
global const int BREACH_TRACE_RESULT_INVALID_END_POINT = 2
global const int BREACH_TRACE_RESULT_WALL_TOO_THIN = 3
global const int BREACH_TRACE_RESULT_WALL_TOO_THICK = 4
...
{
#if DEVELOPER
	// hasscriptdocs() is a later-engine native; S3 dedi has no binding.
	// Const-table clone for docs tooling is skipped when the native is absent.
#endif
}
