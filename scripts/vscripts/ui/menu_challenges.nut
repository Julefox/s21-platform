...
void function JumpToChallenges( string link )
{
	...
	// Bridge: ChallengesPanel may be a hidden tab (not on the strip). Never ActivateTab(-1).
	if ( index < 0 || index >= lobbyTabData.tabDefs.len() )
	{
		printt( "JumpToChallenges: ChallengesPanel not registered as lobby tab; aborting" )
		return
	}
