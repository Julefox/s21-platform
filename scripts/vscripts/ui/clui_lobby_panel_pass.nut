...
void function ShowBattlePassItem_Voucher( ItemFlavor item, float scale )
{
	...
	else if ( Voucher_GetEffectBattlepassLevels( item ) > 0 )
...
void function InitPassPanel( var panel )
{
	...
	HudElem_SetRuiArg( file.giftButton, "buttonNxTextFontOffset", -8.0 )
	...
	AddCallback_UI_FeatureTutorialDialog_PopulateTabsForMode( BattlePass_PopulateAboutText, FEATURE_EVENT_BOOSTED_TUTORIAL )
	AddCallback_UI_FeatureTutorialDialog_SetTitle( BattlePass_PopulateAboutTitle, FEATURE_EVENT_BOOSTED_TUTORIAL )
	...
	AddPanelFooterOption( panel, LEFT, BUTTON_X, true, "#X_BATTLEPASS_BREAKOUT_REWARDS", "#BATTLEPASS_BREAKOUT_REWARDS", BattlePass_OpenBoostedBreakdown, BattlePass_ShouldShowBreakoutRewards )
...
bool function BattlePass_ShouldShowBreakoutRewards()
{
	return RewardCampaign_GetActiveRewardCampaign() != null && !BattlePass_CanBuyUpToFocusedItem() && !BattlePass_IsFocusedItemEquippable()
}
...
void function BattlePass_UpdatePurchaseButton( var button, bool showUpsell = true )
{
	...
	if ( GRX_IsItemOwnedByPlayer( activeBattlePass ) )
	{
		...
		else
		{
			...
			Hud_SetLocked( button, false )
			ToolTipData toolTipData
			toolTipData.titleText = "#BUTTON_BATTLE_PASS_GOTO_PRESALE_TOOLTIP"
...
void function UpdatePassPurchaseButtons()
{
	...
	bool hasPremiumPass  = DoesPlayerOwnBattlePass( GetLocalClientPlayer(), activeBattlePass )
	bool canPurchasePass = !hasPremiumPass
	...
	bool hasSufficientPurchasableLevels = GetPlayerBattlePassPurchasableLevels( ToEHI( GetLocalClientPlayer() ), activeBattlePass ) >= 25
	bool canPurchaseBundle = !hasPremiumPass && hasSufficientPurchasableLevels
	...
	Hud_SetLocked( s_passPurchaseMenu.passPurchaseButton, !canPurchasePass )

	fileLevel.isPremiumPassPurchasable = canPurchasePass
	fileLevel.isBundlePassPurchasable = canPurchaseBundle

	if ( !hasSufficientPurchasableLevels )
	{
		ToolTipData toolTipData
