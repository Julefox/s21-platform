...
{
	// Season lobby tab is unregistered on the bridge client.
	TabData lobbyTabData = GetTabDataForPanel( GetMenu( "LobbyMenu" ) )
	int seasonIdx = Tab_GetTabIndexByBodyName( lobbyTabData, "SeasonPanel" )
	if ( seasonIdx < 0 )
		return
	...
	ActivateTab( lobbyTabData, seasonIdx )
