...
void function ToggleInventoryOrOpenOptions()
{
	...
	if ( (UITime() >= endTIme && InputIsButtonDown( BUTTON_START )) || (InputIsButtonDown( KEY_ESCAPE ) && !SURVIVAL_IsAnInventoryMenuOpened()) )
	{
		...
		if( GameModeVariant_IsActive( eGameModeVariants.SURVIVAL_FIRING_RANGE ) && !IsControllerModeActive() && !InputIsButtonDown( KEY_TAB ) && !Flowstate_IsGame1v1Type() )
		{
...
void function InitMenus()
{
	...
	// Pre-connect public list (main-menu Browse). Distinct from lobby ServerBrowserPanel.
	...
	AddPanel( lobbyMenu, "GamemodesPanel", InitGamemodesPanel )
	var createPanel = AddPanel( lobbyMenu, "CreatePanel", InitCreatePanel )
	AddPanel( createPanel, "R5RPlaylistPanel", InitR5RPlaylistPanel )
	AddPanel( createPanel, "R5RMapPanel", InitR5RMapPanel )
	AddPanel( createPanel, "R5RVisPanel", InitR5RVisPanel )
	AddPanel( createPanel, "R5RNamePanel", InitR5RNamePanel )
	AddPanel( createPanel, "R5RDescPanel", InitR5RDescPanel )
	...
	var labPanel = AddPanel( inventoryMenu, "LabPanel", InitLabPanel )
	...
	AddPanel( leaderboardMenu, "LeaderboardScrollPanel", Init_LeaderboardScrollPanel )
