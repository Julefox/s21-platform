...
void function OnUILanguageSettingChanged( var button )
{
	// bridge_ui_language change already reloaded disk tables in client.dll.
	// Re-paint this row's title/desc from the new table so Settings updates live.
	SetButtonRuiText( button, Localize( "#BRIDGE_UI_LANGUAGE" ) )
	SettingsButton_SetDescription( button, Localize( "#BRIDGE_UI_LANGUAGE_DESC" ) )
	EmitUISound( "UI_Menu_Select" )
}
...
void function InitHudOptionsPanel( var panel )
{
	...
	// Instant UI/HUD language (disk tables). ConVar bridge_ui_language drives C++ reload.
	SetupSettingsButton( Hud_GetChild( contentPanel, "SwitchUILanguage" ), "#BRIDGE_UI_LANGUAGE", "#BRIDGE_UI_LANGUAGE_DESC", $"rui/menu/settings/settings_hud" )
	AddButtonEventHandler( Hud_GetChild( contentPanel, "SwitchUILanguage" ), UIE_CHANGE, OnUILanguageSettingChanged )
	...
	// Usage Sharing / PIN opt-in removed from HUD settings (no analytics path).
	SetConVarBool( "pin_opt_in", false )
	...
	// Communication Filter removed -- leave chat open to everyone.
	SetConVarInt( "cl_comms_filter", -1 )
	...
	file.conVarDataList.append( CreateSettingsConVarData( "hud_setting_healthbar_style", eConVarType.INT ) )
...
void function OnHudOptionsPanel_Show( var panel )
{
	...
	// Keep PIN/usage-sharing permanently off (surveys / PIN gate on pin_opt_in).
	SetConVarBool( "pin_opt_in", false )
	// Communication filter removed from UI; everyone can chat.
	SetConVarInt( "cl_comms_filter", -1 )
	if ( GetConVarInt( "hud_setting_chat" ) == 1 )
		SetConVarInt( "hud_setting_chat", 0 )

#if !PC_PROG_NX_UI
		// Crossplay hidden on PC bridge; pin chain AnonymousMode -> NetGraph.
		HudOptionsHideButton( contentPanel, "SwitchCrossplay", "SwitchAnonymousMode", "SwitchNetGraph" )
#else
		...
		{
			HudOptionsShowButton( contentPanel, "SwitchCrossplay", "SwitchAnonymousMode", "SwitchNetGraph" )
		}
