...
bool function GroundList_IsLootGrabber( entity deathBox )
{
	if ( !IsValid( deathBox ) )
		return false
...
void function UIToClient_SurvivalGroundListOpened( var menu )
{
	...
	bool isBlackMarket = GroundList_IsLootGrabber( deathBox )
...
void function UIToClient_SurvivalGroundListClosed()
{
	...
	entity deathBox = Survival_GetDeathBox()
	if ( GroundList_IsLootGrabber( deathBox ) )
	{
...
void function SendItemActionCommand( DeathBoxEntryData entryData, int count, entity preferredLootEnt, int actionType, bool isAltAction, int ornull backpackSwapSlot )
{
	...
	if ( IsValid( boxEnt ) )
	{
		string boxClass = expect string( boxEnt.GetNetworkedClassName() )
		printt( "[FR-WEAP] UI SendItemAction ref=" + entryData.lootFlav.ref +
			" idx=" + entryData.lootFlav.index + " count=" + count +
			" boxClass=" + boxClass +
			" prefValid=" + IsValid( preferredLootEnt ) +
			" flags=" + pickupFlags + " actionType=" + actionType )
		if ( boxEnt.GetTargetName() == DEATH_BOX_TARGETNAME || boxClass == "prop_death_box" )
		{
		...
		else if ( boxClass == "prop_loot_grabber" )
		{
		...
		{
			printt( "[FR-WEAP] UI FAIL unknown box class=" + boxClass )
			Assert( false, "Attempted to pick up from non known box entity of type: " + boxClass )
		}
	}
	else
	{
		printt( "[FR-WEAP] UI FAIL boxEnt invalid eeh=" + fileLevel.currentDeathBoxEEH +
			" ref=" + entryData.lootFlav.ref )
	}
