...
void function OnLobbyMenu_Open()
{
	...
	{
		// Lobby tabs live on the launcher now. Strip is Play | Armory.
		{
			...
			SetTabBaseWidth( tab, LOBBY_PLAY_TAB_WIDTH )
		}
		{
		...
		// Bridge: Challenges stays off the tab strip but remains ActivateTab-reachable
		// from AllChallengesButton / JumpToChallenges (visible=false, enabled=true).
		{
		...
		// Keep S21 CTA width for capstone layout (selected tab frame / spacing).
		// Strings are cleared in RefreshTabsGRXData / BannerUpdate so no "ends in".
#if PC_PROG_NX_UI
		...
		// Seed FLOWSTATE into GRXData before first ActivateTab paint.
...
void function OnGRXStateChanged()
{
	...
	SetPanelTabEnabled( GetPanel( "ArmoryPanel" ), ready )
	...
	// Bridge: Challenges is a hidden ActivateTab target only — never put it on the strip.
	// Invisible active tab = no selected/featured state on the strip.
	int challengesIdx = Tab_GetTabIndexByBodyName( tabData, "ChallengesPanel" )
	if ( challengesIdx >= 0 )
	{
		tabData.tabDefs[challengesIdx].visible = false
		if ( tabData.activeTabIdx == challengesIdx && !IsTabPanelActive( GetPanel( "ChallengesPanel" ) ) )
			ActivateTab( tabData, GetLobbyDefaultTabIndex() )
	}
...
// S21 Event tab is gone. Keep the export so event panel scripts still link.
...
void function UpdateCornerButtons()
{
	...
	// Bridge: nuke News + XP (progression modifiers) corner buttons.
	Hud_SetVisible( file.newsButton, false )
	Hud_SetVisible( file.newsButtonStatusIcon, false )
	Hud_SetEnabled( file.newsButton, false )
	...
	Hud_SetEnabled( file.progressionModifiersButton, false )
...
{
	// Bridge: lobby chat enter line is removed — do not steal focus onto it.
	return
}
