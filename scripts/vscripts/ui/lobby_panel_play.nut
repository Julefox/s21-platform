...
void function InitPlayPanel( var panel )
{
	...
	// Bridge: no matchmaking fill squad — keep the control dead for the whole session.
	Hud_SetVisible( file.fillButton, false )
	Hud_SetEnabled( file.fillButton, false )
	Hud_Hide( file.fillButton )
	...
	// Bridge: challenge chrome is optional — res may omit or fail-parse these.
	// Never Hud_GetChild without HasChild (throws and aborts InitPlayPanel).
	file.allChallengesButton = null
	if ( Hud_HasChild( panel, "AllChallengesButton" ) )
	{
	...
	if ( Hud_HasChild( panel, "ChallengesNextBPReward" ) )
	{
		...
		Hud_SetVisible( nextBPRewardButton, false )
		Hud_Hide( nextBPRewardButton )
	}

	file.eventPrizeTrackButton = null
	if ( Hud_HasChild( panel, "EventPrizeTrackButton" ) )
	{
		file.eventPrizeTrackButton = Hud_GetChild( panel, "EventPrizeTrackButton" )
		Hud_SetVisible( file.eventPrizeTrackButton, false )
		Hud_SetEnabled( file.eventPrizeTrackButton, false )
		Hud_Hide( file.eventPrizeTrackButton )
	}

	file.eventShopButton = null
	if ( Hud_HasChild( panel, "EventShopButton" ) )
	{
		file.eventShopButton = Hud_GetChild( panel, "EventShopButton" )
		Hud_SetVisible( file.eventShopButton, false )
		Hud_SetEnabled( file.eventShopButton, false )
		Hud_Hide( file.eventShopButton )
	}

	file.storyPrizeTrackButton = null
	if ( Hud_HasChild( panel, "StoryPrizeTrackButton" ) )
	{
		file.storyPrizeTrackButton = Hud_GetChild( panel, "StoryPrizeTrackButton" )
		Hud_SetVisible( file.storyPrizeTrackButton, false )
		Hud_SetEnabled( file.storyPrizeTrackButton, false )
		Hud_Hide( file.storyPrizeTrackButton )
	}

	if ( Hud_HasChild( file.panel, "PopupMessage" ) )
		Hud_AddEventHandler( Hud_GetChild( file.panel, "PopupMessage" ), UIE_CLICK, OnClickCallToActionPopup )

	file.challengeCategorySelection = null
	file.challengesLeftButton = null
	file.challengesRightButton = null
	if ( Hud_HasChild( file.panel, "ChallengeCatergorySelection" ) )
		file.challengeCategorySelection = Hud_GetChild( file.panel, "ChallengeCatergorySelection" )
	if ( Hud_HasChild( file.panel, "ChallengeCatergoryLeftButton" ) )
	{
		file.challengesLeftButton = Hud_GetChild( file.panel, "ChallengeCatergoryLeftButton" )
		Hud_AddEventHandler( file.challengesLeftButton, UIE_CLICK, IsRTL() ? ChallengeSwitchRight_OnClick : ChallengeSwitchLeft_OnClick )
	}
	if ( Hud_HasChild( file.panel, "ChallengeCatergoryRightButton" ) )
	{
		file.challengesRightButton = Hud_GetChild( file.panel, "ChallengeCatergoryRightButton" )
		Hud_AddEventHandler( file.challengesRightButton, UIE_CLICK, IsRTL() ? ChallengeSwitchLeft_OnClick : ChallengeSwitchRight_OnClick )
	}
	// Bridge: hard-kill entire weekly challenge strip when present.
	...
	// Bridge: kill lobby chat enter/input line (history can stay).
	var chatInputLine = Hud_GetChild( file.chatBox, "ChatInputLine" )
	Hud_SetVisible( chatInputLine, false )
	Hud_SetEnabled( chatInputLine, false )
	Hud_Hide( chatInputLine )
	var chatTextEntry = Hud_GetChild( chatInputLine, "ChatInputTextEntry" )
	Hud_SetVisible( chatTextEntry, false )
	Hud_SetEnabled( chatTextEntry, false )
	Hud_Hide( chatTextEntry )
...
void function PlayPanel_OnShow( var panel )
{
	...
	UpdateLobbyButtons()
	// Bridge: re-nuke challenge chrome every show (category strip had been surviving).
	HideLobbyChallengeMenu()
	...
	UI_ClearRespawnOverlay() 
		...
		// Solo bridge lobby: trio-centered PLAY cam (not 4-stack PLAY_QUADS).
	...
	// Bridge: stub hides LobbyDebugText permanently (no date/time thread).
...
{
	// Bridge: hard-disable LobbyDebugText (date/time dev strip above SelfButton).
	var textLabel = Hud_GetChild( file.panel, "LobbyDebugText" )
	Hud_SetText( textLabel, "" )
	Hud_SetVisible( textLabel, false )
}
...
void function UpdateLastSquadDpadNav()
{
	...
	var buttonRightOfLastSquadPanel = file.selfButton
...
void function UpdateFriendButtons()
{
	...
	// Bridge solo lobby: no party invite / friend slots — only SelfButton.
	...
	Hud_SetVisible( file.inviteLastPlayedUnitFrame2, false )

	if( IsDialog( GetActiveMenu() ) )
		return

	// SelfButton layout is LIVE-style: pin PanelFrame TOP (play.res) — do not force screen center.

	Hud_SetNavDown( file.selfButton, file.gamemodeSelectButton )
	Hud_SetNavLeft( file.selfButton, file.gamemodeSelectButton )
	Hud_SetNavRight( file.selfButton, file.miniPromoButton )
	if ( file.miniPromoButton != null )
		Hud_SetNavLeft( file.miniPromoButton, file.selfButton )
	if ( file.eventPrizeTrackButton != null )
		Hud_SetNavLeft( file.eventPrizeTrackButton, file.selfButton )
	if ( file.eventShopButton != null )
		Hud_SetNavLeft( file.eventShopButton, file.selfButton )
	if ( file.challengesLeftButton != null )
		Hud_SetNavLeft( file.challengesLeftButton, file.selfButton )

	// Self only — never populate friend/invite slots.
	Party party = GetParty()
	string playerUID = GetPlayerUID()
	bool foundSelfInParty = false
	foreach ( PartyMember partyMember in party.members )
	{
		if ( partyMember.uid != playerUID )
			continue

		foundSelfInParty = true
	...
	if ( !foundSelfInParty )
	{
		string selfHardware = GetUnspoofedPlayerHardware()
		PartyMember selfStub
		selfStub.name              = GetPlayerName()
		selfStub.uid               = playerUID
		selfStub.hardware          = selfHardware
		selfStub.ready             = false
		selfStub.present           = true
		selfStub.eaid              = ""
		selfStub.boostCount        = 0
		selfStub.unspoofedHardware = selfHardware
		selfStub.unspoofedUID      = playerUID

		ToolTipData stubToolTipData
		stubToolTipData.tooltipStyle = eTooltipStyle.BUTTON_PROMPT
		stubToolTipData.actionHint1  = "#A_BUTTON_INSPECT"
		Hud_SetToolTipData( file.selfButton, stubToolTipData )

		var friendRui = Hud_GetRui( file.selfButton )
		RuiSetBool( friendRui, "canViewStats", true )
		UpdateFriendButton( friendRui, selfStub, false )
		Hud_SetVisible( file.selfButton, true )
	}
}
...
void function UpdateLowerLeftButtonPositions()
{
	...
	// Solo lobby: nav into SelfButton only.
	Hud_SetNavRight( rankedBadge, file.selfButton )
	Hud_SetNavRight( aboutButton, file.selfButton )
	Hud_SetNavRight( file.fillButton, file.selfButton )
	...
	if ( v3PlaylistSelect )
	{
		...
		Hud_SetNavUp( file.gamemodeSelectButton, file.selfButton )
...
void function ClientToUI_PartyMemberJoinedOrLeft( string leftSpotUID, string leftSpotEAID, string leftSpotHardware, string leftSpotUnspoofedUID, string leftSpotUnspoofedHardware, string leftSpotName, bool leftSpotInMatch,
												  string rightSpotUID, string rightSpotEAID, string rightSpotHardware, string rightSpotUnspoofedUID, string rightSpotUnspoofedHardware, string rightSpotName, bool rightSpotInMatch,
												  string fourthSpotUID, string fourthSpotEAID, string fourthSpotHardware, string fourthSpotUnspoofedUID, string fourthSpotUnspoofedHardware, string fourthSpotName, bool fourthSpotInMatch)
	{
		...
		file.friendInLeftSpot.unspoofedHardware = leftSpotUnspoofedHardware
		...
		file.friendInRightSpot.unspoofedHardware = rightSpotUnspoofedHardware
		...
		file.friendInFourthSpot.unspoofedHardware = fourthSpotUnspoofedHardware
...
void function UpdateModeButton()
{
	...
	if ( useGamemodeSelect && playlistName != "" )
	{
		...
		else
		{
			...
			string modeDescText = playlistName == PLAYLIST_NEW_PLAYER_ORIENTATION ? Localize( "#PL_ORIENTATION_MATCH" ) : ""
...
void function UpdateFillButton()
{
	...
	// Bridge: force-hide Fill Squad (S21 re-shows it when playlist supports no-fill).
	...
	Hud_SetVisible( file.fillButton, false )
	Hud_SetEnabled( file.fillButton, false )
	Hud_SetLocked( file.fillButton, true )
	Hud_Hide( file.fillButton )
}
...
{
	if ( !Hud_HasChild( file.panel, "MiniPromo" ) )
		return
	...
	if ( file.eventPrizeTrackButton != null )
		pinCandidates.append( file.eventPrizeTrackButton )
	if ( file.eventShopButton != null )
		pinCandidates.append( file.eventShopButton )
	if ( file.challengesLeftButton != null )
		pinCandidates.append( file.challengesLeftButton )
