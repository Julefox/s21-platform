...
{
	string header = ""
	// Bridge default — BannerUpdate can paint before first RefreshTabsGRXData.
	string title = "FLOWSTATE"
...
void function BannerUpdate_Capstone( TabData tabData )
{
	...
	bool isLogoSmall = false
	if ( tabData.activeTabIdx >= 0 && tabData.activeTabIdx < tabData.tabDefs.len() )
		isLogoSmall = tabData.tabDefs[tabData.activeTabIdx].isBannerLogoSmall
	RuiSetBool( tabBackgroundRUI, "isLogoSmall", isLogoSmall )
	...
	{
		// Bridge brand title. Keep S21 callToActionWidth for capstone layout
		// (width 0 collapsed selected/featured framing).
		string brandTitle = GRXData.title
		if ( brandTitle == "" )
			brandTitle = "FLOWSTATE"
		RuiSetString( tabBackgroundRUI, "title", brandTitle )
		RuiSetString( tabBackgroundRUI, "header", GRXData.header )
		RuiSetString( tabBackgroundRUI, "callToActionHeader", "" )
		RuiSetString( tabBackgroundRUI, "callToActionTitle", "" )
		RuiSetFloat( tabBackgroundRUI, "callToActionWidth", tabData.callToActionWidth )
	}
	else
	{
...
{
	// BRIDGE offline lobby branding — not season-date dependent.
	// S21 wrote Season_GetShortName here; we always brand FLOWSTATE.
	// Do NOT zero callToActionWidth — that breaks selected/featured tab framing.
	GRXData.title = "FLOWSTATE"
	GRXData.header = ""
	GRXData.callToActionHeader = ""
	GRXData.callToActionTitle = ""
...
void function ActivateTab( TabData tabData, int tabIndex )
{
	...
	{
		printt( "ActivateTab: invalid tabIndex", tabIndex, "defs", tabData.tabDefs.len() )
		return
	}
...
void function DeactivateTab( TabData tabData )
{
	...
	if( tabData.tabDefs[tabData.activeTabIdx].panel == null )
		return
...
void function UpdateMenuTabs()
{
	...
	foreach ( tabButtonPanel in tabButtonPanels )
	{
		...
		Lobby_AdjustScreenFrameToMaxSize( tabData.background )
		...
		for ( int tabIndex = 0; tabIndex < MAX_TABS; tabIndex++ )
		{
			...
			RuiSetBool( tabButtonRUI, "isBeta", tabDef.isBeta )
