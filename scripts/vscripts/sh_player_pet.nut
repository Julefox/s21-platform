...
void function DebugPlayerPet( entity npc, entity owner )
{
	...
	while( true )
	{
		DebugDrawLineRGB( npc.GetOrigin(), owner.GetOrigin() + <0, 0, 36>, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 0.1 )
		DebugDrawSphereRGB( npc.GetOrigin(), 16, int(COLOR_GREEN.x), int(COLOR_GREEN.y), int(COLOR_GREEN.z), true, 0.1 )
		wait 0.05
	}
